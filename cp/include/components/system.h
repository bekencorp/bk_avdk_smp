...
typedef enum {
    ...
    RESET_SOURCE_SLEEP_GPIO = 0x1a,
    RESET_SOURCE_SLEEP_RTC = 0x1b,
    RESET_SOURCE_SLEEP_WIFI = 0x1c,
    RESET_SOURCE_SLEEP_BT = 0x1d,
    RESET_SOURCE_SLEEP_TOUCH = 0x1e,
    RESET_SOURCE_SLEEP_VAD = 0x1f,
    RESET_SOURCE_DEEPPS_VAD = 0x20,
 ...
 * @brief  GPIO LOG API
 *
 * These log functions output log information via GPIO by simulating the SPI protocol.
 * The log is transmitted through GPIO pins using a software-implemented SPI, which is useful for outputting logs when standard UART or other interfaces are unavailable.
 * Functions in this section allow for initialization, formatted printing, and raw data transmission over the GPIO-based SPI log channel.
 */
