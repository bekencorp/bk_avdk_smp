...
#define AUD_DUMP_INLINE              static inline
...
AUD_DUMP_INLINE void set_aud_dump_bitmap_bit(uint8_t dump_type)
{
...
AUD_DUMP_INLINE void clr_aud_dump_bitmap_bit(uint8_t dump_type)
{
    if(DUMP_TYPE_MAX <= dump_type)
    {
        BK_LOGE("aud_dump","%s,%d,dump_type:%d is invalid!\n", __func__, __LINE__,dump_type);
...
AUD_DUMP_INLINE bool is_aud_dump_valid(uint8_t dump_type)
{
    if(DUMP_TYPE_MAX <= dump_type)
    {
        BK_LOGE("aud_dump","%s,%d,dump_type:%d is invalid!\n", __func__, __LINE__,dump_type);
...
AUD_DUMP_INLINE uint16_t get_aud_dump_bitmap(void)
{
    return g_aud_data_dump_bitmap;
}

AUD_DUMP_INLINE void clr_aud_dump_bitmap(void)
{
