...
static void aon_pmu_drv_set_wlp_power_down_wrapper(uint32_t v)
{
	aon_pmu_drv_set_wlp_power_down(v);
}
...
static void wifi_mac_phy_power_on_wrapper(void)
{
	...
	bk_pm_module_vote_power_ctrl(PM_POWER_SUB_DOMAIN_MAC, PM_POWER_MODULE_STATE_ON);
	bk_pm_module_vote_power_ctrl(PM_POWER_SUB_DOMAIN_PHY, PM_POWER_MODULE_STATE_ON);
...
{
	#if 0//(CONFIG_ROSC_COMPENSATION) //TODO
...
static void bk_task_wdt_feed_wrapper(void)
{
#if (CONFIG_TASK_WDT)
	bk_task_wdt_feed();
#endif
...
__attribute__((section(".dtcm_sec_data "))) wifi_os_funcs_t g_wifi_os_funcs = {
	...
	#if(CONFIG_SOC_BK7236XX || CONFIG_SOC_BK7256XX || CONFIG_SOC_BK7259)
	...
	._bk_task_wdt_feed = bk_task_wdt_feed_wrapper,
...
__attribute__((section(".dtcm_sec_data "))) wifi_os_variable_t g_wifi_os_variable = {
    ...
    ._sys_sys_debug_config1_addr = 0,//TODO
	...
	._pm_power_module_name_wifip_mac = PM_POWER_SUB_DOMAIN_MAC,
