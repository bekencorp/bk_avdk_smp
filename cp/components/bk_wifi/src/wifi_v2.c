...
__IRAM_SEC static int wifi_lowvol_enter_cb(uint64_t expected_time_ms, void *args)
{
	rwnx_lowvol_enter_callback();
	return 0;
}
__IRAM_SEC static int wifi_lowvol_exit_cb(uint64_t expected_time_ms, void *args)
{
...
bk_err_t bk_wifi_disable(void)
{
    ...
    ret = bk_pm_module_vote_power_ctrl(PM_POWER_SUB_DOMAIN_MAC, PM_POWER_MODULE_STATE_OFF);
    if(BK_OK != ret)
    {
        g_wifi_funcs->_wifi_notify_state_to_bt(1);
        g_wifi_enable_flag = true;
...
bk_err_t bk_wifi_enable(void)
{
    ...
    ret = bk_pm_module_vote_power_ctrl(PM_POWER_SUB_DOMAIN_MAC, PM_POWER_MODULE_STATE_ON);
