...
///Wi-Fi coex event types
enum coex_wifi_event{
    COEX_WIFI_SCAN                  = 1 << 0,
    COEX_WIFI_TBTT                  = 1 << 1,
    COEX_WIFI_TBTT_DATA             = 1 << 2,
    COEX_WIFI_TX                    = 1 << 3,
    COEX_WIFI_AP_MODE               = 1 << 4,
    COEX_WIFI_STA_CONNECTING        = 1 << 5,
    COEX_WIFI_STA_CONNECTED         = 1 << 6,
    COEX_WIFI_STA_MODE              = 1 << 7,

    COEX_WIFI_MAC_ACTIVE            = 1 << 8,
...
typedef struct {
	...
	void (*_sys_drv_set_cpu_power_sleep_wakeup_pwd_ofdm)(uint32_t v);
	uint32_t (*_sys_drv_get_cpu_power_sleep_wakeup_pwd_ofdm)(void);
	uint32_t (*_sys_drv_wifi_wrls_power_is_on)(void);
	uint32_t (*_sys_drv_get_cpu_device_clk_enable_wlss_cken)(void);
	uint32_t (*_sys_drv_get_cpu_device_clk_enable_mac_cken)(void);
	...
	void (*_aon_pmu_drv_set_wlp_power_down)(uint32_t v);
	...
	uint64_t (*_rtc_tick_to_us)(uint64_t tick);
	uint64_t (*_rtc_us_to_tick)(uint64_t us);
