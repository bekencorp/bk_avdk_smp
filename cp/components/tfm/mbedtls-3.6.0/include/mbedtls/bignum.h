...
#if defined(ARM_CE_DUBHE_ACA)
/* Beken Dubhe TrustEngine ACA hardware-accelerated MPI helpers. These are
 * implemented in the trustengine dubhe_lib/bignum.c and consumed across
 * dubhe_lib. The upstream mbedtls-3.6.0 does not declare them, so mirror the
 * Beken-patched prototypes here to keep the accelerator self-consistent. */
