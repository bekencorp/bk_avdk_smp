                          ...
                          unsigned char *output)
{
    ...
    if (mode == MBEDTLS_AES_DECRYPT) {
        unsigned char padding_value = output[length - 1];
        unsigned char padding_ok = 0x01;
        size_t i;
        
        for (i = 0; i < padding_value; i++) {
            unsigned char index = length - 1 - i;
            unsigned char expected = padding_value;
            padding_ok &= (output[index] == expected);
        }
        
        unsigned char mask = padding_ok - 1;
        for (i = 0; i < 16; i++) {
            output[length - 16 + i] &= mask;
        }
    }
