                        ...
                        unsigned char *output)
{
    ...
    /* Replace non-constant-time comparison with constant-time comparison */
    unsigned char check_result_bin[MBEDTLS_MPI_MAX_SIZE];
    unsigned char input_blinded_bin[MBEDTLS_MPI_MAX_SIZE];

    MBEDTLS_MPI_CHK(mbedtls_mpi_write_binary(&check_result_blinded, check_result_bin, ctx->len));
    MBEDTLS_MPI_CHK(mbedtls_mpi_write_binary(&input_blinded, input_blinded_bin, ctx->len));

    if (mbedtls_ct_memcmp(check_result_bin, input_blinded_bin, ctx->len) != 0) {
