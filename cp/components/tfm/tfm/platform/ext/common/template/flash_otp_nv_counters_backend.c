...
enum tfm_plat_err_t read_otp_nv_counters_flash(uint32_t offset, void *data, uint32_t cnt)
{
    volatile int32_t err = TFM_PLAT_ERR_SYSTEM_ERR;
    ...
    if (remaining_cnt) {
        ...
        verify_size = read_items * data_width;
        ...
        if (err < 0) {
            ...
            if (memcmp(data, verify_buffer, verify_size) != 0) {
                return TFM_PLAT_ERR_SYSTEM_ERR;
            }
        } else {
            ...
            while (verify_offset < verify_size) {
                chunk_size = verify_size - verify_offset;
                if (chunk_size > sizeof(verify_buffer)) {
                    chunk_size = sizeof(verify_buffer);
                }
                chunk_items = chunk_size / data_width;
                err = TFM_PLAT_ERR_SYSTEM_ERR;
                err = OTP_NV_COUNTERS_FLASH_DEV.ReadData(
                                                TFM_OTP_NV_COUNTERS_AREA_ADDR + offset + verify_offset,
                                                verify_buffer,
                                                chunk_items);
                if (err < 0) {
                    return TFM_PLAT_ERR_SYSTEM_ERR;
                }
                if (memcmp((uint8_t *)data + verify_offset, verify_buffer, chunk_size) != 0) {
                    return TFM_PLAT_ERR_SYSTEM_ERR;
                }
                verify_offset += chunk_size;
            }
        }
    }
    ...
    if (remaining_cnt) {
        ...
        if (memcmp(temp_buffer, verify_buffer, data_width) != 0) {
            return TFM_PLAT_ERR_SYSTEM_ERR;
        }
...
enum tfm_plat_err_t init_otp_nv_counters_flash(void)
{
    volatile enum tfm_plat_err_t err = TFM_PLAT_ERR_SYSTEM_ERR;
    uint32_t init_value;
    uint32_t swap_count;
