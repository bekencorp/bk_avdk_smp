                                  ...
                                  uint32_t arg)
{
    ...
    switch (control & ARM_USART_CONTROL_Msk) {
        ...
        case ARM_USART_MODE_ASYNCHRONOUS:
            if(bk_uart_set_baud_rate(id, arg) != BK_OK) {
...
{
#if DOMAIN_NS == 1U
    /* UART0 is already initialized in Secure side before jumping to NS */
    /* In NS side, we only need to set driver state, skip all hardware initialization */
    ARG_UNUSED(cb_event);
    
    /* Initialize driver framework if not already done */
    /* Skip GPIO init as it's already done in Secure side */
    sys_drv_init();
    bk_uart_driver_init();
    
    /* Hardware is already configured by Secure side, only set driver state */
    /* without re-initializing hardware */
    if (bk_uart_mark_initialized(UART_ID_0) == BK_OK) {
        return ARM_DRIVER_OK;
    }
    return ARM_DRIVER_ERROR;
#else
...
{
#if DOMAIN_NS == 1U
    /* UART0 hardware is already configured in Secure side */
    /* Skip operations that would access system registers */
    
    switch (control & ARM_USART_CONTROL_Msk) {
#ifdef UART_TX_RX_CONTROL_ENABLED
        case ARM_USART_CONTROL_TX: {
            uart_id_t id = USART0_DEV.id;
            /* Only enable/disable TX, no system register access */
            if (arg == 0) {
                bk_uart_set_enable_tx(id, false);
            } else if (arg == 1) {
                if (bk_uart_set_enable_tx(id, true) != BK_OK) {
        ...
        case ARM_USART_CONTROL_RX: {
            uart_id_t id = USART0_DEV.id;
            /* Only enable/disable RX, no system register access */
            if (arg == 0) {
                bk_uart_set_enable_rx(id, false);
            } else if (arg == 1) {
                if (bk_uart_set_enable_rx(id, true) != BK_OK) {
...
#endif
        case ARM_USART_MODE_ASYNCHRONOUS:
            /* Skip baud rate setting as it accesses system registers */
            /* Hardware is already configured in Secure side */
            /* Just return success */
            return ARM_DRIVER_OK;
        default:
            return ARM_DRIVER_ERROR_UNSUPPORTED;
    ...
    /* Skip other control operations that might access system registers */
    return ARM_DRIVER_OK;
#else
