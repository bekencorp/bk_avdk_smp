...
/* armino_config.h is generated by CMake configure_file() from the external
 * Armino/SDK Kconfig (CONFIG_CPU0_SPE_RAM_SIZE -> CONFIG_TFM_RAM_SIZE). It
 * provides CONFIG_TFM_RAM_SIZE, which drives S_DATA_SIZE and the NS RAM window
 * below. Including it here makes that value available to every consumer of
 * region_defs.h (board/target_cfg.c SAU config, bk7259_s.ld / bk7259_ns.ld),
 * so the SPE RAM size is configured only in the external defconfig. */
 ...
 * window is therefore 0x3C01C000..0x3C05FFFF (physical SMEM offset 0x1C000..,
 * right after the 0x1C000 secure SPE carve-out = CONFIG_CPU0_SPE_RAM_SIZE).
 * Mark exactly that window Non-Secure in the SAU so NS data accesses are
 * allowed while the secure SPE keeps 0x28000000..0x2801BFFF. This boundary
 * (CONFIG_TFM_RAM_SIZE) drives the RAM MPC split (bk_tfm_mpc.c) and the NS app
 * base (bk7259_bsp.ld), so derive the NS window from it instead of hardcoding.
 */
