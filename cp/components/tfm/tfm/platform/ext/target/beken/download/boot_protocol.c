 ...
 * Used only when CONFIG_BL2_UPDATE_WITH_PC: protocol-side gate beside
 * bl_forbid_operate_boot_partition(). Without UPDATE_WITH_PC the real
 * [0, app) protect lives in bl_forbid_operate_boot_partition().
 */
...
// 01 e0 fc 01 00
// 04 0e 05 01 e0 fc 01 00
extern bk_err_t bk_flash_set_protect_type_none(void);
...
#if CONFIG_BL2_UPDATE_WITH_PC
	if (s_record_dl_flag == 0) {
		if ((addr >= CONFIG_PRIMARY_MANIFEST_PHY_PARTITION_OFFSET) &&
		    (addr < DOWNLOAD_PROTECT_END_OFFSET)) {
...
static u32 flash_cmd_reg_read_handler(u8 * cmd_param, u16 param_len)
{
	...
	{
		++sr_read_times;
		bk_flash_set_protect_type_none();   // unprotect flash once after handshake, before any erase/write
	}
