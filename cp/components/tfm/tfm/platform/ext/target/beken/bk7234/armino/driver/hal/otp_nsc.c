...
otp_privilege_t otp_read_permission(uint8_t map_id, uint32_t item)
{
	...
	otp_privilege_t permission = read_permission_fun[map_id-1](location);
	otp_privilege_t mask = read_mask_fun[map_id-1](location);
...
{
	//check the parameters again after active
	if((map_id > 2) || (map_id == 1 && item >= OTP1_MAX_ID) || (map_id == 2 && item >= OTP2_MAX_ID))
	{
		return BK_ERR_OTP_INDEX_WRONG;
	}

	uint32_t location = otp_map[map_id-1][item].offset / 4;
	int32_t size = otp_map[map_id-1][item].allocated_size;
...
bk_err_t otp_read(uint8_t map_id, uint32_t item, uint8_t* buf, uint32_t size)
{
	...
	while(size > 0) {
		...
		//printf("location = 0x%x value = 0x%x  ",location, value);
		//printf("size = 0x%x start = 0x%x\r\n",size, start);
...
__tfm_psa_secure_gateway_no_naked_attributes__ bk_err_t bk_otp_read_nsc(uint32_t item)
{
	...
	uint8_t* buf = (uint8_t*)os_malloc(size*(sizeof(uint8_t)));
	if(buf == NULL)
...
__tfm_psa_secure_gateway_no_naked_attributes__ bk_err_t bk_otp_write_mask_nsc(uint8_t map_id, uint32_t item, uint32_t permission)
{
	...
	OTP_ACTIVE();
	otp_write_mask(map_id, item, permission);
	otp_sleep();
	return BK_OK;
}
