...
/* Same as FLASH0_SIZE */
#define FLASH_TOTAL_SIZE                (0x00800000) /* 8 MB (bk7259) */

/* Flash layout info for BL2 bootloader.
 * BK7259 flash XIP/execution base = SOC_FLASH_MEM_BASE = 0x04000000 (BootROM
 * reg_base.h). The legacy 0x02000000 (bk7234 ROM base) made BL2/TF-M link/run
 * at the wrong address, so the BootROM XIP jump to the verified bl2 ran off
 * into garbage and produced no log. */
...
/*ota area start*/
#define FLASH_AREA_PRIMARY_PARTITION_ID    (FLASH_AREA_SCRATCH_ID + 1)
#define FLASH_AREA_PRIMARY_MANIFEST_ID     (FLASH_AREA_PRIMARY_PARTITION_ID + 1)
#define FLASH_AREA_PRIMARY_BL2_ID          (FLASH_AREA_PRIMARY_MANIFEST_ID + 1)
#define FLASH_AREA_SYS_PS_ID               (FLASH_AREA_PRIMARY_BL2_ID + 1)
#define FLASH_AREA_SYS_ITS_ID              (FLASH_AREA_SYS_PS_ID + 1)
#define FLASH_AREA_PRIMARY_TFM_S_ID        (FLASH_AREA_SYS_ITS_ID + 1)
...
/* BK7259 flash XIP base = 0x04000000 (secure alias); NS alias = +0x10000000. */
#define S_ROM_ALIAS_BASE  (0x04000000)
#define NS_ROM_ALIAS_BASE (0x14000000)
...
#define FLASH_AREA_IMAGE_SECONDARY(x)   (((x) == FLASH_IMAGE_ALL_ID) ? FLASH_AREA_SECONDARY_ALL_ID : \
					 ...
					 (((x) == FLASH_IMAGE_PRIMARY_BL2_ID) ? FLASH_AREA_PRIMARY_BL2_ID : 255 \
					  )))

#define FLASH_AREA_IMAGE_SCRATCH        FLASH_AREA_SCRATCH_ID

#define TFM_HAL_PS_FLASH_AREA_ADDR      0x00200000
#define TFM_HAL_ITS_FLASH_AREA_ADDR     0x00210000

#define TFM_OTP_NV_COUNTERS_AREA_ADDR   0x00220000
#define TFM_OTP_NV_COUNTERS_AREA_SIZE   0x1000

#define TFM_OTP_NV_COUNTERS_BACKUP_AREA_ADDR 0x00230000
