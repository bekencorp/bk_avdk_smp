...
int flash_map_init(void)
{
	...
	id = PARTITION_BOOT_PARAM;
	flash_map[FLASH_MAP_IMAGE_BOOT_PARAM].fa_off = partition_get_phy_offset(id);
	flash_map[FLASH_MAP_IMAGE_BOOT_PARAM].fa_size = partition_get_phy_size(id);
	return 0;
}
