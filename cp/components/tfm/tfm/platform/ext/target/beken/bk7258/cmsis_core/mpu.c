...
static const ARM_MPU_Region_t mpu_regions[] = {
    /* MPU region 0: Flash before primary_all (app) — RO + executable.
     * BK7258 flash XIP base is 0x02000000. Covers bl1_control/boot_flag/
     * partition/manifest/bl2 up to primary_all; attr[4] is WT+RA (no WA).
     */
    { ARM_MPU_RBAR(0x02000000UL, ARM_MPU_SH_NON, 1, 1, 0),
      ARM_MPU_RLAR((0x02000000UL + CONFIG_PRIMARY_ALL_PHY_PARTITION_OFFSET - 0x10), 4)},
...
static const uint8_t mpu_attrs[] = {
    ...
    ARM_MPU_ATTR(0xa, 0xa)  // Normal memory, cacheable write through, read allocate, (RO) no write allocate
  };
