...
/* BL2 early bring-up debug markers disabled (raw UART0 SystemInit progress).
 * The markers and their busy-wait delay added per-boot UART overhead; with the
 * boot chain validated they are turned into no-ops. Keep the call sites so they
 * can be re-enabled by defining BL2_SI_MARK_DEBUG if early SystemInit ever needs
 * to be re-traced. */
#if defined(BL2_SI_MARK_DEBUG) && defined(BL2) && !defined(__DOMAIN_NS)
static inline void si_mark(const char *s)
{
	volatile unsigned char  *u0b = (volatile unsigned char  *)0x44820000;
	volatile unsigned int   *u0  = (volatile unsigned int   *)0x44820000;
	for (; *s; ++s) { while (u0[0x18 / 4] & (1u << 16)) { } u0b[0x1C] = (unsigned char)*s; }
}
#define SI_MARK(s) si_mark(s)
#else
#define SI_MARK(s) do {} while (0)
#endif
