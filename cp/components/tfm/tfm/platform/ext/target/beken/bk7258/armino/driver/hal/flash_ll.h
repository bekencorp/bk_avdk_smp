...
/* NOR flash status register bit definitions.
 * WIP/WEL are volatile, read-only status bits; SRP0/SRP1 are the
 * status-register-protect bits (SRP0 = byte1 bit7, SRP1 = byte2 S8). */
#define FLASH_SR_WIP_POS    (0)
#define FLASH_SR_WEL_POS    (1)
#define FLASH_SR_SRP0_POS   (7)
#define FLASH_SR_SRP1_POS   (8)

#define FLASH_SR_WIP        (1u << FLASH_SR_WIP_POS)   /* Write In Progress */
#define FLASH_SR_WEL        (1u << FLASH_SR_WEL_POS)   /* Write Enable Latch */
#define FLASH_SR_SRP0       (1u << FLASH_SR_SRP0_POS)  /* Status Register Protect 0 */
#define FLASH_SR_SRP1       (1u << FLASH_SR_SRP1_POS)  /* Status Register Protect 1 (S8) */

/* LB[3:0] security-register lock bits (S13..S10, OTP) and SUS suspend status
 * (S15, volatile read-only) are not part of the value programmed by WRSR. */
#define FLASH_SR_LB_MASK    (0xFu << 10)               /* S10..S13 lock bits */
#define FLASH_SR_SUS        (1u << 15)                 /* Suspend status (S15) */
...
static inline uint32_t flash_ll_get_wp_value(flash_hw_t *hw)
{
	return hw->op_ctrl.wp_value;
}

static inline void flash_ll_set_wp_value(flash_hw_t *hw, uint32_t wp_value)
{
	hw->op_ctrl.wp_value = wp_value;
}
...
{
	/* TH25Q64 cannot use controller WRSR2 (timeout on download unprotect).
	 * Keep 0x01 + 0x31 split for TH/GD25Q32C when width==2. Also drive WP# high
	 * like middleware — otherwise SR/QE writes can silently fail. */
	flash_ll_wait_op_done(hw);
	hw->cmd_cfg.v = 0;
