...
/* A/B bootloader (BL2) version tracking + bootloader-update-from-PC. bk7259
 * uses a single BL2 (DIRECT_XIP, no "B" partition), so CONFIG_BL2_B_* /
 * CONFIG_PARTITION_B_* / BOOT_RECORD_VERSION don't exist here. This block is
 * only meaningful with the A/B update-with-PC feature and has no caller in the
 * bk7259 download flow, so gate it out. */
#if CONFIG_BL2_UPDATE_WITH_PC
