...
uint32_t rtos_disable_int(void)
{
	...
	__asm volatile ("MRS %0, primask" : "=r" (primask_val) );
...
{
	__asm volatile ("MSR primask, %0" : : "r" (int_level) : "memory");
}
...
__bk_weak void rtos_exit_critical(uint32_t flags)
{
}

__bk_weak uint32_t rtos_enter_critical(void)
{
	return 0;
}
