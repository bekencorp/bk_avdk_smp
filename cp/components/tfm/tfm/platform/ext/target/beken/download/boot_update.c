...
/*
 * First downloadable app payload. No TFM on this product → primary_all.
 * Fall back to TFM_S for layouts that still ship a secure image.
 */
#if defined(CONFIG_PRIMARY_ALL_PHY_PARTITION_OFFSET)
#define DOWNLOAD_PROTECT_END_OFFSET  CONFIG_PRIMARY_ALL_PHY_PARTITION_OFFSET
#elif defined(CONFIG_PRIMARY_TFM_S_PHY_PARTITION_OFFSET)
#define DOWNLOAD_PROTECT_END_OFFSET  CONFIG_PRIMARY_TFM_S_PHY_PARTITION_OFFSET
#endif
...
#else
	/*
	 * Bootloader upgrade via PC disabled: protect [0, app).
	 * Only the partition-table window remains writable (re-flashed with all-app).
	 */
#ifndef DOWNLOAD_PROTECT_END_OFFSET
#error "download protect needs PRIMARY_ALL or PRIMARY_TFM_S phy offset"
#endif
...

	{
		...
		uint32_t protect_end_addr = DOWNLOAD_PROTECT_END_OFFSET;
		...
		if (op_start_addr < protect_end_addr) {
			...
			if (!((op_start_addr >= part_tbl_start) &&
				...
				printf("Forbid operating flash before app partition, addr: 0x%x, len: 0x%x, boundary: 0x%x\r\n",
...
/* Bootloader self-version management maintains a second bootloader slot
 * (bl2_B / partition_B), which only exists when in-place bootloader upgrade is
 * used. It is compiled out otherwise so a single-bootloader layout builds. */
#if CONFIG_BL2_UPDATE_WITH_PC
...
#endif /* CONFIG_BL2_UPDATE_WITH_PC */
