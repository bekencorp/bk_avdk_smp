...
int flash_bootloader_area_protect(int32_t addr,uint32_t len)
{
	flash_boot_flag_e  boot_flag = 0;
...
static uint32_t flash_area_erase_app_image(const struct flash_area *area, uint32_t off)
{
	...
	if(s_erase_flash_address <= (erase_size -FLASH_AREA_IMAGE_SECTOR_SIZE) )
	{
		...
		{
			flash_area_erase(area, s_erase_flash_address, FLASH_AREA_IMAGE_SECTOR_SIZE);
                                ...
                                const void *src, uint32_t len)
{
	...
	if((area->fa_id == FLASH_AREA_PRIMARY_ALL_ID)||(area->fa_id == FLASH_AREA_SECONDARY_ALL_ID))
	{
		...
		flash_area_erase_app_image(area, off); //erase app strategy.
...
#if CONFIG_BL2_UPDATE_WITH_PC 	/*check the area address whether valid*/
...
int flash_area_erase(const struct flash_area *area, uint32_t off, uint32_t len)
{
	...
	if(flash_check_area_addr_valid(area,off,len) != 0)
	{
		printf("Erase area address invalid:0x%x\r\n",s_area_addr_valid);
		return -1;
	}
#endif
