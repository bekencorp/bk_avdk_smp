...
{
    /* BK7259: enable the real hash abort so the driver-wrapper hash context is
     * cleaned up (paired with the now-enabled setup/update/finish above). */
...
psa_status_t psa_hash_setup(psa_hash_operation_t *operation,
                            psa_algorithm_t alg)
{
    /* BK7259: this thin PSA core shipped with the hash ops stubbed out
     * (#if 0 ... return 0), so SHA never ran and MCUboot saw an uninitialised
     * hash buffer (calc=aaaaaaaa) -> "image not valid". Enable the real driver-
     * wrapper hash path (SHA-256 via mbedtls/dubhe) so EC-P256 image
     * verification gets a correct digest. */
    psa_status_t status;
