...
/* The Dubhe accelerator config defines MBEDTLS_PLATFORM_NO_STD_FUNCTIONS +
 * MBEDTLS_PLATFORM_SNPRINTF_ALT, so dubhe_lib expects mbedtls_snprintf via the
 * ALT function pointer. Declare _SNPRINTF_ALT here too so the non-accelerator
 * bl2_mbedcrypto TUs (lms.c/error.c/md5.c ...) agree on the same ALT contract
 * (matches the existing _PRINTF_ALT). */
