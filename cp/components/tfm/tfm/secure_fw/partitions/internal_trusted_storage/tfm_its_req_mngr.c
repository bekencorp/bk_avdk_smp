...
static psa_status_t tfm_its_get_info_req(const psa_msg_t *msg)
{
    ...
    if (status == PSA_SUCCESS) {
        if (psa_write(msg->handle, 0, &info, sizeof(info)) != sizeof(info)) {
            return PSA_ERROR_PROGRAMMER_ERROR;
        }
    }
...
size_t its_req_mngr_write(const uint8_t *buf, size_t num_bytes)
{
    return psa_write(handle, 0, buf, num_bytes);
}
#endif
