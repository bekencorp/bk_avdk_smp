...
uint64_t ipc_schedule(uint32_t exc_return)
{
    ...
    if (pth_next != NULL && p_part_curr != p_part_next) {
        /* Validate partition pointer validity */
        if (p_part_next == NULL) {
            tfm_core_panic();
        }
        
        /* Validate partition load info pointer validity */
        if (p_part_next->p_ldinf == NULL) {
            tfm_core_panic();
        }
        
        /* Validate partition load info integrity (Magic number check) */
        if ((p_part_next->p_ldinf->psa_ff_ver & PARTITION_INFO_MAGIC_MASK)
            != PARTITION_INFO_MAGIC) {
            tfm_core_panic();
        }
        
        /* Validate partition ID validity */
        if (p_part_next->p_ldinf->pid < 0) {
            tfm_core_panic();
        }
        
        /* Validate Client ID range validity (if NS Agent) */
        if (IS_NS_AGENT(p_part_next->p_ldinf)) {
            if (p_part_next->p_ldinf->client_id_base > 
                p_part_next->p_ldinf->client_id_limit) {
                tfm_core_panic();
            }
        }
        
        /* Validate stack pointer range (stricter check) */
        uint32_t current_psp = __get_PSP();
        if (p_curr_ctx->sp_limit >= current_psp) {
            tfm_core_panic();
        }
        if ((p_curr_ctx->sp_limit + sizeof(struct tfm_additional_context_t)) > current_psp) {
            tfm_core_panic();
        }
        
        /* Validate context pointer validity */
        if (p_curr_ctx == NULL) {
            tfm_core_panic();
        }
        if (pth_next->p_context_ctrl == NULL) {
            tfm_core_panic();
        }
        
        /* Validate stack pointer alignment */
        if ((current_psp & 0x7) != 0) {
            tfm_core_panic();
        }
        
        
        /* Validate boundary value validity */
        if (p_part_curr != NULL && p_part_curr->boundary == 0) {
            /* Current partition boundary should not be 0 (unless initial state) */
        }
        if (p_part_next->boundary == 0 && !IS_PSA_ROT(p_part_next->p_ldinf)) {
            /* Non-privileged partitions must have valid boundary value */
            tfm_core_panic();
        }
        ...
        if (fih_not_eq(fih_bool, fih_int_encode(false))) {
            /* Re-validate partition info before boundary switch */
            if (p_part_next->p_ldinf == NULL) {
                tfm_core_panic();
            }
            ...
            /* Verify partition info not tampered after boundary switch */
            if ((p_part_next->p_ldinf->psa_ff_ver & PARTITION_INFO_MAGIC_MASK)
                != PARTITION_INFO_MAGIC) {
                tfm_core_panic();
            }
        ...
        /* Protection 13: Flush FP context */
        ARCH_FLUSH_FP_CONTEXT();
        
        /* Protection 14: Validate thread pointer validity */
        if (pth_next == NULL) {
            tfm_core_panic();
        }
    ...
    /* Validate partition metadata pointer validity */
    if (p_part_next->p_metadata == NULL) {
        tfm_core_panic();
    }
    
    /* Validate metadata pointer alignment */
    if (((uintptr_t)p_part_next->p_metadata & 0x3) != 0) {
        tfm_core_panic();
    }
    
    /* Validate partition info integrity (re-check) */
    if ((p_part_next->p_ldinf->psa_ff_ver & PARTITION_INFO_MAGIC_MASK)
        != PARTITION_INFO_MAGIC) {
        tfm_core_panic();
    }
