...
#if defined(ARM_CE_DUBHE_ACA)
/* Beken Dubhe TrustEngine ACA hardware-accelerated MPI helpers. Implemented in
 * trustengine dubhe_lib/bignum.c and consumed across dubhe_lib. Upstream
 * mbedtls-3.6.0 does not declare them, so mirror the Beken-patched prototypes
 * here (this interface copy is on the include path before mbedtls-3.6.0). */
