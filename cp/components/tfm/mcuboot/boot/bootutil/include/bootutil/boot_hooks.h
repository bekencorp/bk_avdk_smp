...
/** Hook to override which slot MCUboot activates for an image.
 *
 * Called right after find_slot_with_highest_version(). Lets a platform (e.g. an
 * A/B boot_param policy) pick the slot instead of pure highest version. The
 * preferred slot is only used if available; otherwise MCUboot keeps the default
 * and still falls back on a failed signature check.
 *
 * @param img_index the index of the image pair
 * @param slot      in: default (highest-version) slot; out: preferred slot
 *
 * @retval 0: *slot holds the preferred slot to try first
 *         BOOT_HOOK_REGULAR: no preference, keep the default slot
 */
