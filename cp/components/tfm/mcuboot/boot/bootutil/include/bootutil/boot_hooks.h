 ...
 *
 * Called from find_slot_with_highest_version(). Lets a platform A/B policy
 * (boot_param) pick the preferred slot. The preferred slot is only used if
 * available; MCUboot still falls back on a failed signature check.
 * On BK7234, reboot_times CRC-hang recovery may still force a swap after this.
 *
 * @param img_index the index of the image pair
 * @param slot      in: default candidate; out: preferred slot
 *
