...
fih_ret boot_fih_memequal(const void *s1, const void *s2, size_t n)
{
    ...
    // return bk_sca_ct_memcmp(s1, s2, n);
    return bk_sca_memcmp_nontrivial(s1, s2, n);
#else
    return memcmp(s1, s2, n);
#endif /* CONFIG_SCA_DEFENSE */
}
#else
fih_ret boot_fih_memequal(const void *s1, const void *s2, size_t n)
{
...
#ifdef CONFIG_SCA_DEFENSE
    // if (!mbedtls_ct_memcmp(s1_p, s2_p, n)) {
    // if (!bk_sca_ct_memcmp(s1_p, s2_p, n)) {
    if (!bk_sca_memcmp_nontrivial(s1_p, s2_p, n)) {
        ret = FIH_SUCCESS;
        goto out;
    }
#else
