...
typedef enum
{
	SLOT0_MAGIC_IS_ERROR = 1,
	SLOT1_MAGIC_IS_ERROR = 2,
	SLOT_MAGIC_IS_RIGHT = 3,
} slot_status_e;
...
#define  RECORD_REBOOT_LIMIT_TIME   (AB_TRY_MAX_DEFAULT)
#define  WDT_PERIOD_MAX_VALUE       (0xFFFF)//WDT_PERIOD_MAX_VALUE is 0xFFFF
...
fill_rsp(struct boot_loader_state *state, struct boot_rsp *rsp)
{
    ...
    flash_set_excute_enable(active_slot == BOOT_PRIMARY_SLOT ? 0 : 1);
                     ...
                     int slot)
{
    ...
    if (hdr->ih_magic != IMAGE_MAGIC) {
        BOOT_LOG_ERR("s%d invalid img magic: %x\r\n", slot, hdr->ih_magic);
        return false;
    }

    if (!boot_u32_safe_add(&size, hdr->ih_img_size, hdr->ih_hdr_size)) {
        BOOT_LOG_ERR("s%d invalid size: size=%x, img_size=%d, hdr_size=%x\r\n",
                     slot, size, hdr->ih_img_size, hdr->ih_hdr_size);
        return false;
    }

    if (size >= flash_area_get_size(fap)) {
        BOOT_LOG_ERR("s%d invalid size: size=%x, area_size=%x\r\n",
                     slot, size, flash_area_get_size(fap));
        return false;
    }
...
#if 0//defined(MCUBOOT_DIRECT_XIP)
/**
                   ...
                   struct boot_status *bs)
{
    ...
    /* Reject invalid headers before hashing to avoid long hash on empty/garbage slots. */
    if (!boot_is_header_valid(hdr, fap, slot)) {
#if !defined(__BOOTSIM__)
        BOOT_LOG_ERR("before validate, Image in the %s slot is not valid!",
    ...
    if (!boot_is_header_valid(hdr, fap, slot) || fih_not_eq(fih_rc, FIH_SUCCESS)) {
...
boot_get_slot_usage(struct boot_loader_state *state)
{
    ...
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        ...
        rc = boot_read_image_headers(state, false, NULL);
        if (rc != 0) {
            BOOT_LOG_ERR("Failed reading all image headers. rc=%d", rc);
            return rc;
        }
        ...
        for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
            ...
            if (boot_is_header_valid(hdr, BOOT_IMG_AREA(state, slot), (int)slot)) {
        ...
        /* Both headers unusable: drop try in NORMAL only. In TRIAL, leave the
         * counter alone so try can still reach try_max+1 and decide_slot can
         * rollback (boot may still fail if exec header is also bad). */
        {
            bool any = false;
            ab_flag_record_t bp_rec;
            bool in_trial;
            ...
            in_trial = (boot_param_get_latest_record(&bp_rec) == 0 &&
                        bp_rec.boot_state == AB_STATE_TRIAL);
            if (!any && !in_trial) {
                boot_param_pmu_try_clear();
                BOOT_LOG_FORCE("both slots invalid, try cleared\r\n");
            }
        }
...
static uint32_t  find_slot_to_swap_verify(uint32_t origin_slot)
{
	...
	if(record_slot_status == SLOT_MAGIC_IS_RIGHT)
	{
		...
		{
			BOOT_LOG_FORCE("pre-swap from slot0 to slot1 \r\n");
			candidate_slot = CANDIDATE_SLOT_1; //change from slot0 to slot1
		}
		else
		{
			BOOT_LOG_FORCE(" slot0  slot1 all fail \r\n");
			candidate_slot = NO_ACTIVE_SLOT;
		}
	}
	else if(record_slot_status == SLOT0_MAGIC_IS_ERROR) //not exsit slot0
	{
		BOOT_LOG_FORCE(" pre-swap verfy slot1 \r\n");
		candidate_slot =CANDIDATE_SLOT_1;
	}
	else if(record_slot_status == SLOT1_MAGIC_IS_ERROR) //not exsit slot1
	{
		BOOT_LOG_FORCE(" pre-swap verify slot0 \r\n");
		candidate_slot = CANDIDATE_SLOT_0;
	}
	else
	{
		BOOT_LOG_FORCE("do nothing \r\n");
	}
...
find_slot_with_highest_version(struct boot_loader_state *state)
{
    ...
    int32_t record_reboot_times = -1;
    int rc;

    /* BK7259-style when version-based selection is enabled. */
    for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
        if (state->slot_usage[BOOT_CURR_IMG(state)].slot_available[slot]) {
    ...
    /* Let boot_param steer slot choice over the highest-version default, but only
     * when the preferred slot is available (bad-signature fallback preserved). */
    if (candidate_slot != NO_ACTIVE_SLOT) {
        uint32_t pref = candidate_slot;
        int hrc = BOOT_HOOK_CALL(boot_get_active_slot_hook, BOOT_HOOK_REGULAR,
                                 BOOT_CURR_IMG(state), &pref);
        /* pref_in_range MUST gate every use of pref as an array index below,
         * so a bad slot number from the hook can never cause an OOB access. */                         
        bool hook_decided  = (hrc != BOOT_HOOK_REGULAR);
        bool pref_in_range = (pref < BOOT_NUM_SLOTS);
        bool pref_usable   = pref_in_range &&
            state->slot_usage[BOOT_CURR_IMG(state)].slot_available[pref];

        if (hook_decided && pref_in_range && pref_usable &&
            pref != candidate_slot) {
            BOOT_LOG_FORCE("bp override: img %d slot %u->%u\r\n",
                           BOOT_CURR_IMG(state),
                           (unsigned)candidate_slot, (unsigned)pref);
            candidate_slot = pref;
        }
    }

    /* Best CRC hang recovery without extra AON bits (NORMAL only):
     *  1) try == LIMIT  -> sticky-swap once to the other available slot
     *     (covers preferred header-OK / body hang, other image good).
     *  2) try >  LIMIT  -> clear try and keep BP preferred
     *     (covers preferred good, sticky target hung: do not sticky again).
     * TRIAL is excluded; decide_slot owns try_max rollback.
     * Never swap to an unavailable slot. */
    if (candidate_slot != NO_ACTIVE_SLOT) {
        ab_flag_record_t bp_rec;
        bool in_trial = (boot_param_get_latest_record(&bp_rec) == 0 &&
                         bp_rec.boot_state == AB_STATE_TRIAL);

        record_reboot_times = (int32_t)boot_param_pmu_try_get();
        if (!in_trial) {
            if (record_reboot_times > (int32_t)RECORD_REBOOT_LIMIT_TIME) {
                boot_param_pmu_try_clear();
                record_reboot_times = 0;
                /* No sticky; leave candidate as already chosen (version/BP).
                 * Preferred may still be unavailable — do not force it here. */
                BOOT_LOG_FORCE("try>%d cleared, no sticky (cand=%u)\r\n",
                               (int)RECORD_REBOOT_LIMIT_TIME,
                               (unsigned)candidate_slot);
            } else if (record_reboot_times ==
                       (int32_t)RECORD_REBOOT_LIMIT_TIME) {
                uint32_t swapped = find_slot_to_swap_verify(candidate_slot);

                if (swapped != NO_ACTIVE_SLOT &&
                    swapped < BOOT_NUM_SLOTS &&
                    state->slot_usage[BOOT_CURR_IMG(state)].slot_available[swapped]) {
                    BOOT_LOG_FORCE(" verified exceed %d times \r\n",
                                   record_reboot_times);
                    candidate_slot = swapped;
                }
            }
        }
    }

    BOOT_LOG_FORCE("candidate_slot :0x%x \r\n ", candidate_slot);
    return candidate_slot;
}
...
/* No CONFIG_DIRECT_XIP boot_select_or_erase(copy_done): aligned with BK7259 BP.
 * Incomplete-OTA / trial settle are owned by boot_param, not XIP trailer. */
...
boot_load_and_validate_images(struct boot_loader_state *state)
{
    ...
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        ...
        while (true) {
            ...
            if (active_slot == NO_ACTIVE_SLOT) {
                BOOT_LOG_ERR("No slot to load for image %d",BOOT_CURR_IMG(state));
                FIH_RET(FIH_FAILURE);
            }
        ...
        if (state->img_mask[BOOT_CURR_IMG(state)]) {
            ...
            /* Same gate as BK7259: only when MCUBOOT_DIRECT_XIP_REVERT is on.
             * Product BP path does not erase on missing trailer copy_done. */
#ifdef MCUBOOT_DIRECT_XIP_REVERT
