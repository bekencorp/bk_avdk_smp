 ...
 * Copyright (c)     2023-2028 Nordic Semiconductor ASA
 *
 * Original license:
 *
...
fill_rsp(struct boot_loader_state *state, struct boot_rsp *rsp)
{
    ...
    /* Position-independent A/B: slot B (secondary) executes at slot A's VA via the
     * HW XIP remap; enable remap for B, disable for A before handing off. */
...
/* Gated with the call site (MCUBOOT_DIRECT_XIP_ROM_FIXED_CHECK, never defined)
 * so both compile out together for the position-independent A/B XIP scheme. */
#if defined(MCUBOOT_DIRECT_XIP) && defined(MCUBOOT_DIRECT_XIP_ROM_FIXED_CHECK)
/**
                   ...
                   struct boot_status *bs)
{
    ...
    FIH_DECLARE(fih_rc, FIH_FAILURE);
    int rc;
    bk_sw_fih_set_data(FIH_SW_INDEX8);
    area_id = flash_area_id_from_multi_image_slot(BOOT_CURR_IMG(state), slot);
    rc = flash_area_open(area_id, &fap);
    if (rc != 0) {
        FIH_RET(fih_rc);
...
#endif
    /* Validate the header BEFORE hashing: boot_image_check() hashes
     * hdr_size+img_size bytes from flash, so a garbage header (empty slot ->
     * bogus huge ih_img_size) would hash for seconds and trip the watchdog.
     * boot_is_header_valid() bounds size to the slot, so it fails fast here. */
...
#endif /* !CONFIG_OTA_OVERWRITE: boot_copy_region is overridden by
        * common/bl2/decompress_bl2.c for the compressed-overwrite scheme. */
                              ...
                              struct boot_status *bs)
{
    ...
    /* Compressed-overwrite: no swap journal, install armed only by
     * OVERWRITE_CONFIRM. No partial swap to resume; just log primary and let
     * boot_validated_swap_type() pick TEST vs NONE.
     */
...
/* secureboot_overwrite function start */
fih_ret
context_boot_go(struct boot_loader_state *state, struct boot_rsp *rsp)
{
    ...
    /* Slot sectors are defined here (not at file scope) so they aren't allocated
     * for non-boot-loader apps (old gcc ignores -fdata-sections). */

    has_upgrade = false;

    /* Pass 1: determine swap type per image and finish any aborted swap. */
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        /* Open primary + secondary areas for this call. */
        for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
            ...
            if (rc != 0) {
                BOOT_LOG_ERR("Failed to open flash area ID %d (image %d slot %d): %d, "
                             "cannot continue", fa_id, BOOT_CURR_IMG(state), (int8_t)slot, rc);
                FIH_PANIC;
            }
        }
    ...
    /* Pass 2: swap types are set; perform the required update for each image. */
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
...
find_slot_with_highest_version(struct boot_loader_state *state)
{
    ...
    if (candidate_slot != NO_ACTIVE_SLOT) {
        ...
        bool pref_differs  = pref_in_range && (pref != candidate_slot);
...
boot_load_and_validate_images(struct boot_loader_state *state)
{
    ...
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        ...
        if (state->img_mask[BOOT_CURR_IMG(state)]) {
            ...
            /* ROM-address-fix rejection DISABLED for position-independent A/B XIP.
             * Slot B reuses slot A's ciphertext (ih_load_addr = A's offset) and
             * runs at A's VA via HW XIP remap, so a ROM_FIXED image would always be
             * wrongly rejected for B. Gated by MCUBOOT_DIRECT_XIP_ROM_FIXED_CHECK
             * (never defined) so call + definition compile out together, like
             * MCUBOOT_DIRECT_XIP_REVERT below. Our packer never sets --rom-fixed. */
#ifdef MCUBOOT_DIRECT_XIP_ROM_FIXED_CHECK
            int rc = boot_rom_address_check(state);
            if (rc != 0) {
...
#endif /* MCUBOOT_DIRECT_XIP_ROM_FIXED_CHECK */
