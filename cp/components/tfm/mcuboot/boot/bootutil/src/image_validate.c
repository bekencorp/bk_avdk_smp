...
#ifdef EXPECTED_SIG_TLV
extern bool efuse_is_secureboot_enabled(void);

/* Whether BL2 must authenticate the image signature. When the secure boot
 * efuse (bit3) is burned, signature authentication is forced regardless of the
 * sig_verify_en csv setting. Otherwise it follows CONFIG_SIG_VERIFY_EN. The
 * SHA256 image hash is always checked in either case. */
static bool bootutil_sig_verify_required(void)
{
    if (efuse_is_secureboot_enabled()) {
        return true;
    }
#if defined(CONFIG_SIG_VERIFY_EN) && (CONFIG_SIG_VERIFY_EN == 0)
    return false;
#else
    return true;
#endif
                      ...
                      int seed_len, uint8_t *out_hash)
{
    ...
    bool sig_required = bootutil_sig_verify_required();
    int key_id = -1;
    ...
    while (true) {
        ...
        if (type == IMAGE_TLV_SHA256) {
            ...
            /* Authenticate the signature only when required. When skipped the
             * image integrity is still guaranteed by the SHA256 hash check. */
            if (sig_required) {
    ...
    if (sig_required) {
        ...
        /* Signature authentication skipped: image accepted on SHA256 hash. */
