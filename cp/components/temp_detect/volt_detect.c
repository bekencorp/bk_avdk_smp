...
static int _volt_detect_get_adc_data(adc_chan_t adc_chan, uint16_t *adc_buf)
{
    ...
    err = bk_adc_read_raw(adc_buf, ADC_TEMP_BUFFER_SIZE, ADC_READ_SEMAPHORE_WAIT_TIME);
    if (BK_OK != err)
    {
...
void volt_daemon_polling_handler(void)
{
	...
	TEMPD_LOGV("v-cnt=%d, interval=%d, last=%d, cur=%d, thr=%d\r\n",
