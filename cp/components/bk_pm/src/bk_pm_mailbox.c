...
/* bk_pm_module_vote_psram_ctrl is provided by:
 * - client build: `src/bk_pm_client.c`
 * - server build with PSRAM: `src/services/bk_pm_psram.c`
 * Keep a stub only when PSRAM is disabled to avoid multiple definition.
 */
#if (!CONFIG_PSRAM)
