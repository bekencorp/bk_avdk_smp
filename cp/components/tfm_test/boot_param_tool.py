#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""boot_param_tool.py — 手动构造 / 解析 / 修改 BK7259 的 boot_param 分区。

用于在 OTA 代码就绪前，手动摆布 A/B 启动记录来测试阶段3 的各种 case
（正常启动、指定 exec_slot、TRIAL 试启动、回滚、ping-pong 选新等）。

on-flash 布局是 32 字节的 ab_flag_record_t，单一事实源为固件侧
  cp/.../common/bl2/boot_param.h
以及打包器 beken_utils/scripts/partition.py:process_boot_param()。
本脚本必须与它们逐字节一致；改布局时三处一起改并 bump struct_ver。

boot_param 分区 = 8K = 两个 4K ping-pong sector，各放一份记录。
固件启动时读两个 sector，取 magic/ver/size/crc 校验通过且 seq 最大的一份。
提交(commit)时写“另一个” sector（seq+1），因此掉电不会毁掉当前有效副本。

分区物理地址(bk7259, 见 partitions_gen.h)：
  offset = 0x6d5000 , size = 0x2000 (8K)
...
PART_PHY_OFFSET     = 0x6d5000        # bk7259 boot_param 物理烧录地址
...
def cmd_build(a):
    ...
    print(f'\n烧录到设备: 物理地址 0x{PART_PHY_OFFSET:x}')
    print(f'  (例) bk_loader/串口工具 download {a.out} @ 0x{PART_PHY_OFFSET:x}')
...
def main():
    p = argparse.ArgumentParser(
        description='BK7259 boot_param 分区构造/解析/修改工具 (A/B 测试用)',
    ...
    b.add_argument('--try-count', type=lambda x: int(x, 0), default=0, help='当前试启动计数, 默认0')
...
EXAMPLES = """\
示例 (阶段3 A/B 测试):
  # 1) 出厂/正常态: 从 A 启动 (等价打包器预置记录)
  boot_param_tool.py build --exec-slot A --state normal --out bp_A.bin

  # 2) 强制从 B 启动 (验证 secondary 槽/remap 能跑)
  boot_param_tool.py build --exec-slot B --state normal --out bp_B.bin

  # 3) TRIAL 试启动 B: 每次上电 try_count++, 到 try_max 回滚到 exec_slot A
  boot_param_tool.py build --exec-slot A --update-slot B --state trial \\
      --try-max 3 --try-count 0 --out bp_trial.bin

  # 4) TRIAL 即将耗尽(下次上电触发回滚)
  boot_param_tool.py build --exec-slot A --update-slot B --state trial \\
      --try-max 3 --try-count 2 --out bp_trial_last.bin

  # 5) ping-pong: 让 sector1 seq 更大, 固件应选 sector1
  boot_param_tool.py build --sector 1 --seq 100 --exec-slot B --out bp_pp.bin

  # 解析设备读回的 8K 分区镜像, 看两个扇区+固件会选谁
  boot_param_tool.py dump bp_trial.bin

  # 就地把某扇区改成从 B 启动 (seq 自动+1, 其余字段保留)
  boot_param_tool.py edit bp_A.bin --exec-slot B --update-slot B

烧录: 把生成的 8K bin 写到物理地址 0x6d5000 (boot_param 分区)。
"""
