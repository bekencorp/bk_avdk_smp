...
/* RFC2131 / BOOTP: UDP payload should be at least 300 bytes. */
...
static bool dhcp_client_on_bound_iface(const uint8_t *mac, void *netif_handle)
{
	struct netif *netif = (struct netif *)netif_handle;
	int vif_idx;

	if (!mac || !netif || !netif->state)
		return false;

	vif_idx = wifi_netif_vif_to_vifid(netif->state);
	if (vif_idx < 0 || vif_idx == INVALID_VIF_IDX)
		return false;

	return sta_mgmt_get_staid((uint8_t)vif_idx, mac) != INVALID_STA_IDX;
}
...
static int send_dns_response(int sock, struct sockaddr *addr, char *msg, int len)
{
	int nb;
	unsigned int sent = 0;
...
static int process_dns_message(char *msg, int len, struct sockaddr_in *fromaddr)
{
	...
	send_dns_response(dhcps.dnssock, (struct sockaddr *)fromaddr,
		      msg, endp - msg);
...
static int process_dhcp_message(char *msg, int len)
{
	...
	if (!dhcp_client_on_bound_iface(hdr->chaddr, dhcps.prv)) {
		dhcp_d("ignore DHCP for STA not on this iface\r\n");
		return 0;
	}
...
/* Allocate a free instance slot for intrfc_handle. */
