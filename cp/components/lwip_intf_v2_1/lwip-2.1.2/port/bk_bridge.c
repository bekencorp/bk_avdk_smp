...
#if LWIP_AUTOIP
  /* RFC 3927 section 2.5: link-local source => link-layer broadcast. */
 ...
 * clone+linkoutput when want_clone, or pass pbuf through when want_clone==0.
 */
                          ...
                          struct netif *tx_if, int want_clone)
{
  ...
  return tx_if->linkoutput(tx_if, p);
}
