...
{
    return (ieee802154_frame_get_type(frame) == IEEE802154_FRAME_TYPE_COMMAND);
}
...
{
    return IEEE802154_FRAME_FCF_SIZE + (is_dsn_present(frame) ? IEEE802154_FRAME_DSN_SIZE : 0);
}
...
{
    if (!is_suported_frame_type(ieee802154_frame_get_type(frame))) {
        return IEEE802154_FRAME_INVALID_OFFSET;
    }
...
{
    if (!is_suported_frame_type(ieee802154_frame_get_type(frame))) {
        return IEEE802154_FRAME_INVALID_ADDR_MODE;
    }
    uint8_t offset = ieee802154_frame_address_offset(frame);
    uint8_t dst_mode = dst_addr_mode(frame);
    uint8_t addr_size;

    if (!(dst_mode == IEEE802154_FRAME_DST_MODE_SHORT || dst_mode == IEEE802154_FRAME_DST_MODE_EXT)) {
        return dst_mode;
    }
