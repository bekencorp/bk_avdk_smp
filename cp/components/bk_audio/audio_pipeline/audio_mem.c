...
{
    void *ret_data = os_malloc(size);
#ifdef ENABLE_AUDIO_MEM_TRACE
    BK_LOGD(TAG, "hsram malloc ret:%p, size:%d, called:0x%08x \r\n",
        ret_data, size, (intptr_t)__builtin_return_address(0) - 2);
#endif
    return ret_data;
}
...
#ifdef ENABLE_AUDIO_MEM_TRACE
    BK_LOGD(TAG, "hsram free ret:%p, called:0x%08x \r\n",
        ptr, (intptr_t)__builtin_return_address(0) - 2);
#endif
}
