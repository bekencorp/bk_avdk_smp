	...
	switch (decompressor) {
...
#if (CONFIG_LZMA1900 | CONFIG_LZMA2301)
		{
			...
			if (ret) {
				...
				/* Return NULL on failure so the BL2 overwrite installer
				 * (boot_copy_region in decompress_bl2.c, the only caller of
				 * this cp/BL2 variant, which checks `r == NULL`) aborts the
				 * install instead of writing a garbage block to primary_all
				 * and then failing image validation in a brick loop. */
				return NULL;
			}
