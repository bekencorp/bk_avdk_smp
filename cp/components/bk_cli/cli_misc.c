...
{
	CLI_LOGD("\r\ncache {mode|flush [addr] [size]}\n");
	CLI_LOGD("  Show cache configuration informaion or control dcache.\n");
	CLI_LOGD("  -mode<int><optional>: Enable or disable dcache. If this parameter is not filled in, it will show cache configuration information.\n");
	CLI_LOGD("  -flush<str><optional>: Flush cache. If addr and size are not provided, flush all cache.\n");
	CLI_LOGD("  -addr<hex><optional>: Address to flush (e.g., 0x12345678).\n");
	CLI_LOGD("  -size<hex><optional>: Size to flush (e.g., 0x1000).\n");
	CLI_LOGD("  example1: cache\n");
	CLI_LOGD("  example2: cache 0\n");
	CLI_LOGD("  example3: cache 1\n");
	CLI_LOGD("  example4: cache flush\n");
	CLI_LOGD("  example5: cache flush 0x12345678 0x1000\n");
}
#if (CONFIG_EFUSE)
...
void cli_cache_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (!os_strcmp(argv[1], "flush")) {
		if (argc == 2) {
			BK_LOGD(NULL, "cache flush all.\n");
			flush_all_dcache();
			BK_LOGD(NULL, "cache flush all done.\n");
		} else if (argc == 4) {
			uint32_t addr = os_strtoul(argv[2], NULL, 0);
			uint32_t size = os_strtoul(argv[3], NULL, 0);
			BK_LOGD(NULL, "cache flush addr=0x%x, size=0x%x.\n", addr, size);
			flush_dcache((void *)addr, (long)size);
			BK_LOGD(NULL, "cache flush done.\n");
		} else {
			cli_misc_cache_help();
			return;
		}
