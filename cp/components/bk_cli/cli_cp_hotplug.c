...
#if CONFIG_DBG_PROBE
/* Ver4 dbg_probe stress: two same-priority tasks pinned to core0/core1 each
 * burst dbg_probe_rt_raw frames. In SHARED mode both target UART1 and serialize
 * via the probe spinlock, so a single-wire (COM10) capture must show only whole
 * 4-byte frames tagged core0(0x8x,mod 0x11) / core1(0x9x,mod 0x22), never byte-
 * interleaved. Burst is bunched (8 back-to-back, then 2 ms) to force contention
 * yet span a few seconds for capture. */
...
/* Dump one RAM ring as text: a BEGIN line with the header fields, then the data
 * region hex (logical order oldest->newest), then END. Parsed offline by
 * dbg_parse.py --ram-dump. Uses cli_printf so it lands on the system
 * console (COM23), separate from the probe wire (COM10/COM11).
 *
...
static void cli_dbg_probe_dump_ring(uint8_t core, uint8_t bucket)
{
	...
	uint32_t wr, wrap, seq0, start, count;
	char line[129];
	uint32_t col = 0;
	...
	if (wrap == 0u) {
		start = 0u;
		count = wr;             /* not yet wrapped: [0, wr) */
	} else {
		start = wr;
		count = size;           /* wrapped: full buffer, oldest at wr */
	}
	cli_printf("==DBGRING BEGIN core=%u bucket=%u hdr_ver=%u fmt=%u size=%u wr=%u wrap=%u seq=%u==\r\n",
	...
	cli_printf("==DBGRING END seq_end=%u==\r\n", (unsigned)h->seq);
}
...
static uint32_t cli_cpu_hp_gov_wait_online(uint32_t want_online, uint32_t timeout_ms)
{
	for (uint32_t waited = 0; waited <= timeout_ms; waited += CPU_HP_GOV_STRESS_POLL_MS) {
		if (bk_cpu_hp_is_online(CPU1_CORE_ID) == want_online) {
			return 1;
		}
		rtos_delay_milliseconds(CPU_HP_GOV_STRESS_POLL_MS);
	}

	return (bk_cpu_hp_is_online(CPU1_CORE_ID) == want_online);
}
...
static void cli_cpu_hp_governor_cmd(int argc, char **argv)
{
	...
	if (os_strcmp(argv[2], "on") == 0) {
		...
		bk_cpu_hp_governor_status_t st;
		bk_cpu_hp_governor_get_status(&st);
		CLI_LOGI("cpu gov: enabled=%u cpu1_online=%u load0=%u%% load1=%u%% up_cnt=%u down_cnt=%u online=%u offline=%u\r\n",
...
static void cli_dbg_probe_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (argc >= 2 && os_strcmp(argv[1], "v5") == 0) {
		/* Run the Ver5 frame-kind self-test pinned to core1 so the frames hit
		 * UART2/COM11 (the runtime-live port). Host decoder validates 4B/8B,
		 * full-ts/short-ts and the TASK 4-ASCII payload. */
		int core = SMP_CORE1_ID;
		BaseType_t r;

		if (argc >= 3 && os_strcmp(argv[2], "0") == 0) {
			core = SMP_CORE0_ID;   /* optional: emit on core0 (U1, silent at rt) */
		}
	...
	CLI_LOGI("dbgp {mod|sink|etest [loops]|stest [loops]|shared|excl|v5 [0]|dump}\r\n");
}
#endif /* CONFIG_DBG_PROBE */
