...
static void cli_mem_print_static_regions(void)
{
	...
	BK_DUMP_OUT("%-22s %-10s %-10s %-10s\r\n", "------", "----------", "----------", "----------");
	cli_mem_print_region_row("cp_ram", ram_start, heap_end);
	cli_mem_print_region_row("cp_ram_non_heap", ram_start, heap_start);
...
#if defined(CONFIG_CP_PSRAM_HEAP_ADDR) && (CONFIG_CP_PSRAM_HEAP_SIZE > 0)
	cli_mem_print_region_row("cp_psram_heap",
		CONFIG_CP_PSRAM_HEAP_ADDR,
		CONFIG_CP_PSRAM_HEAP_ADDR + CONFIG_CP_PSRAM_HEAP_SIZE);
#endif
#if (CONFIG_CP_PSRAM_SECTION_ADDR)
	cli_mem_print_region_row("cp_psram_data_used",
		(uint32_t)&__psram_data_start__, (uint32_t)&__psram_data_end__);
	cli_mem_print_region_row("cp_psram_bss_used",
...
void cli_memory_free_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	cli_mem_print_heap_row("cp_psram_heap", total_size, free_size, mini_size);
#endif
        ...
        if (compare_result != 0) {
            ...
            for (uint32_t i = 0; i < copy_size; i++) {
                if (src_ptr[i] != dst_ptr[i]) {
                    bk_printf("  First mismatch at offset %u: src=0x%02X, dst=0x%02X\r\n", 
