...
#if WIFI_AGC_TEST
			if ((rx_param->addr & SOC_AGCMEM_REG_BASE) == SOC_AGCMEM_REG_BASE)
			{
				uint32_t riu_agcfsm = REG_READ(SOC_RIUKARST_REG_BASE + 0x390);
				uint32_t crm_phyclk = REG_READ(SOC_PHY_CRM_REG_BASE + 0xC);

				REG_WRITE(SOC_RIUKARST_REG_BASE + 0x390, riu_agcfsm | (1 << 12)); //riu_agcfsmreset_setf(0x1);
				REG_WRITE(SOC_PHY_CRM_REG_BASE + 0xC, crm_phyclk | (1 << 29)); //crm_agcmemclkforce_setf(0x1);
				...
				REG_WRITE(SOC_RIUKARST_REG_BASE + 0x0390, riu_agcfsm & ~(1 << 12)); //riu_agcfsmreset_setf(0x1);
				REG_WRITE(SOC_PHY_CRM_REG_BASE + 0x0C, crm_phyclk & ~(1 << 29)); //crm_agcmemclkforce_setf(0x0);
			}
#endif
			if(rx_param->addr)
				REG_WRITE(rx_param->addr, rx_param->value);
...
#endif
#if WIFI_AGC_TEST
		else if ((rx_param->addr & SOC_AGCMEM_REG_BASE) == SOC_AGCMEM_REG_BASE)
		{
			uint32_t riu_agcfsm = REG_READ(SOC_RIUKARST_REG_BASE + 0x0390);
			uint32_t crm_phyclk = REG_READ(SOC_PHY_CRM_REG_BASE + 0x0C);

			REG_WRITE(SOC_RIUKARST_REG_BASE + 0x0390, riu_agcfsm | (1 << 12)); //riu_agcfsmreset_setf(0x1);
			REG_WRITE(SOC_PHY_CRM_REG_BASE + 0x0C, crm_phyclk | (1 << 29)); //crm_agcmemclkforce_setf(0x1);
