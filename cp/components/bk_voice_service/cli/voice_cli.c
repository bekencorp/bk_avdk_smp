...
void cli_voice_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    if (os_strcmp(argv[1], "start") == 0)
    {
        ...
        else if (dec_type == AUDIO_DEC_TYPE_USER)
        {
            ...
            #else
            gl_g711_dec_cfg.buf_sz = spk_samp_rate*20/1000;
            gl_g711_dec_cfg.out_block_size = (gl_g711_dec_cfg.buf_sz<<1);
            voice_cfg.write_pool_size = gl_g711_dec_cfg.buf_sz;
            #endif
        ...
        if (spk_type == SPK_TYPE_ONBOARD)
        {
            ...
            for(uint32_t i = 0; i < AUD_DAC_SOURCE_MAX; i++)
            {
                ...
                onboard_spk_cfg.frame_size[i] = spk_samp_rate*20/1000*2;
            }
            #else
        ...
        {
            LOGE("%s, %d, uac speaker stream is not supported\n", __func__, __LINE__);
            goto fail;
        }
#endif
