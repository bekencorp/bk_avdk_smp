...
static void bt_gatt_service_init(void)
{
	...
	const struct bt_gatt_service_static *srv[2] = {&_1_gatt_svc, &_2_gap_svc};
	uint8_t i = 0;
	for (i = 0; i < sizeof(srv) / sizeof(srv[0]); i++)
	{
		last_static_handle += srv[i]->attr_count;
	}
#else
			       ...
			       bt_gatt_attr_func_t func, void *user_data)
{
	...
	if (start_handle <= last_static_handle) {
		...
		for (i = 0; i < sizeof(srv) / sizeof(srv[0]); i++)
		{
			...
			for (j = 0; j < static_svc->attr_count; j++, handle++) {
				if (gatt_foreach_iter(&static_svc->attrs[j],
								handle, start_handle,
								end_handle, uuid,
