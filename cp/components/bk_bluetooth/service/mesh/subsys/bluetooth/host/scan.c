			...
			struct net_buf *buf, uint8_t len)
{
	...
	BT_VER("%s event %u, len %u, rssi %d dBm", bt_addr_le_str(addr),
	...
	/* id_addr is consumed synchronously by the callbacks below and the
	 * caller does not use info->addr after this function returns, so the
	 * stack-local address is safe here. */
...
void bt_hci_le_adv_ext_report(struct net_buf *buf)
{
	...
	BT_VER("Adv number of reports %u",  num_reports);
