...
{
	struct bt_l2cap_le_chan *chan = NULL;
	struct bt_conn *conn = NULL;

	chan = LE_CHAN_RTX(work);
	conn = chan->chan.conn;
...
void bt_l2cap_connected(struct bt_conn *conn)
{
	...
	const struct bt_l2cap_fixed_chan *gfchan[ ] = {&le_fixed_chan,&att_fixed_chan,&smp_fixed_chan};
	for(uint8_t iterator = 0; iterator < sizeof(gfchan) / sizeof(gfchan[0]); iterator++)
	{
		const struct bt_l2cap_fixed_chan *fchan = gfchan[iterator];
	#else
