                           ...
                           const mbedtls_operation_t operation )
{
    ...
    if( ( ctx->cipher_info->flags & MBEDTLS_CIPHER_VARIABLE_KEY_LEN ) == 0 &&
        (int) mbedtls_cipher_info_get_key_bitlen(ctx->cipher_info) != key1_bitlen )
    {
        return( MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA );
    }
    ...
    return mbedtls_cipher_get_base(ctx->cipher_info)->set_derived_key_func( ctx->cipher_ctx, key_type,
