...
{
	/* expected: lin_cfg {master|slave} {channel} {checksum} {rate} {bus_inactiv_time} {wup_repeat_time} */
	if (argc < 7) {
		cli_lin_help();
		return;
	}
...
void cli_lin_master_stress_test(void)
{
	...
	while (1) {
		...
		os_memset(rx_buf, 0 ,LIN_DATA_LEN_MAX);
		sprintf((char *)tx_buf, "%08u", rand_count);
		CLI_LOGI("tx_buf:[%s]\r\n", tx_buf);
...
DRV_CLI_CMD_EXPORT static const struct cli_command s_lin_commands[] = {
