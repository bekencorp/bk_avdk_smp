...
#define DMA_DEV_I2S   DMA_DEV_I2S0    // add temporary for compatibility
#endif
...
#if CONFIG_SOC_BK7259
#define I2S3_RETURN_ON_NOT_INIT() do {\
		if (!s_i2s3_driver_is_init) {\
			return BK_ERR_I2S_NOT_INIT;\
		}\
	} while(0)

#define I2S4_RETURN_ON_NOT_INIT() do {\
		if (!s_i2s4_driver_is_init) {\
			return BK_ERR_I2S_NOT_INIT;\
		}\
	} while(0)
#endif
...
#if CONFIG_SOC_BK7259
static bool s_i2s3_driver_is_init = false;
static bool s_i2s4_driver_is_init = false;
#endif
...
#if CONFIG_SOC_BK7259
static i2s_callback_t s_i2s3_isr[SOC_I2S_ISR_NUM] = {NULL};
static i2s_callback_t s_i2s4_isr[SOC_I2S_ISR_NUM] = {NULL};
#endif
static i2s_drv_info_t *i2s_drv_info = NULL;
static i2s_drv_info_t *i2s1_drv_info = NULL;
static i2s_drv_info_t *i2s2_drv_info = NULL;
#if CONFIG_SOC_BK7259
static i2s_drv_info_t *i2s3_drv_info = NULL;
static i2s_drv_info_t *i2s4_drv_info = NULL;
#endif
...
#if 0//CONFIG_SOC_BK7259
static void i2s_init_gpio(i2s_gpio_group_id_t id)
{
	switch (id) {
		case I2S_GPIO_GROUP_0:
			gpio_dev_unmap(GPIO_9);
			gpio_dev_map(GPIO_9, GPIO_DEV_I2S1_DIN);
			gpio_dev_unmap(GPIO_10);
			gpio_dev_map(GPIO_10, GPIO_DEV_I2S1_DOUT);  /// GPIO10/GPIO11 used for DL_UART0
			gpio_dev_unmap(GPIO_11);
			gpio_dev_map(GPIO_11, GPIO_DEV_I2S1_CLK);
			gpio_dev_unmap(GPIO_12);
			gpio_dev_map(GPIO_12, GPIO_DEV_I2S1_SYNC);
			gpio_dev_unmap(GPIO_29);
			gpio_dev_map(GPIO_29, GPIO_DEV_I2S1_MCLK);
			break;

		case I2S_GPIO_GROUP_1:
			gpio_dev_unmap(GPIO_13);
			gpio_dev_map(GPIO_13, GPIO_DEV_I2S2_DIN);
			gpio_dev_unmap(GPIO_14);
			gpio_dev_map(GPIO_14, GPIO_DEV_I2S2_DOUT);
			gpio_dev_unmap(GPIO_15);
			gpio_dev_map(GPIO_15, GPIO_DEV_I2S2_CLK);
			gpio_dev_unmap(GPIO_16);
			gpio_dev_map(GPIO_16, GPIO_DEV_I2S2_SYNC);
			gpio_dev_unmap(GPIO_29);
			gpio_dev_map(GPIO_29, GPIO_DEV_I2S1_MCLK);
			break;
		...
		case I2S_GPIO_GROUP_2:
			...
			gpio_dev_map(GPIO_17, GPIO_DEV_I2S3_DIN);
			gpio_dev_unmap(GPIO_18);
			gpio_dev_map(GPIO_18, GPIO_DEV_I2S3_DOUT);
			gpio_dev_unmap(GPIO_19);
			gpio_dev_map(GPIO_19, GPIO_DEV_I2S3_CLK);
			gpio_dev_unmap(GPIO_20);
			gpio_dev_map(GPIO_20, GPIO_DEV_I2S3_SYNC);
			gpio_dev_unmap(GPIO_29);
			gpio_dev_map(GPIO_29, GPIO_DEV_I2S1_MCLK);
			break;

		case I2S_GPIO_GROUP_3:
			gpio_dev_unmap(GPIO_21);
			gpio_dev_map(GPIO_21, GPIO_DEV_I2S4_DIN);
			gpio_dev_unmap(GPIO_22);
			gpio_dev_map(GPIO_22, GPIO_DEV_I2S4_DOUT);
			gpio_dev_unmap(GPIO_23);
			gpio_dev_map(GPIO_23, GPIO_DEV_I2S4_CLK);
			gpio_dev_unmap(GPIO_24);
			gpio_dev_map(GPIO_24, GPIO_DEV_I2S4_SYNC);
			gpio_dev_unmap(GPIO_29);
			gpio_dev_map(GPIO_29, GPIO_DEV_I2S1_MCLK);
			break;

		case I2S_GPIO_GROUP_4:
			gpio_dev_unmap(GPIO_25);
			gpio_dev_map(GPIO_25, GPIO_DEV_I2S4_DIN);
			gpio_dev_unmap(GPIO_26);
			gpio_dev_map(GPIO_26, GPIO_DEV_I2S4_DOUT);
			gpio_dev_unmap(GPIO_27);
			gpio_dev_map(GPIO_27, GPIO_DEV_I2S4_CLK);
			gpio_dev_unmap(GPIO_28);
			gpio_dev_map(GPIO_28, GPIO_DEV_I2S4_SYNC);
			gpio_dev_unmap(GPIO_29);
			gpio_dev_map(GPIO_29, GPIO_DEV_I2S1_MCLK);
			break;
		default:
...
bk_err_t bk_i2s_driver_deinit(void)
{
	...
	bk_pm_clock_ctrl(PM_CLK_ID_I2S1, CLK_PWR_CTRL_PWR_DOWN);
	...
	if (i2s_index == 0) {
		...
		I2S3_RETURN_ON_NOT_INIT();
		sys_drv_i2s3_clock_en(CLK_PWR_CTRL_PWR_DOWN);
		sys_drv_i2s3_int_en(0);
		int_config_table.int_src = INT_SRC_I2S3;
		s_i2s3_driver_is_init = false;
	} else if (i2s_index == 4) {
		I2S4_RETURN_ON_NOT_INIT();
		sys_drv_i2s4_clock_en(CLK_PWR_CTRL_PWR_DOWN);
		sys_drv_i2s4_int_en(0);
		int_config_table.int_src = INT_SRC_I2S4;
		s_i2s4_driver_is_init = false;
#endif
	} else {
		return BK_FAIL;
	}
...
bk_err_t bk_i2s_init(i2s_gpio_group_id_t id, const i2s_config_t *config)
{
	...
	switch (id) {
		...
		case I2S_GPIO_GROUP_3:
			...
			i2s_hal_set_cfg_index(3);
			sys_drv_i2s3_clock_en(CLK_PWR_CTRL_PWR_UP);
			i2s_hal_soft_reset();
			sys_drv_i2s3_int_en(1);
			os_memset(&s_i2s3_isr, 0, sizeof(s_i2s3_isr));
			int_config_table.int_src = INT_SRC_I2S3;
			int_config_table.isr = i2s_isr;
			s_i2s3_driver_is_init = true;
			if (i2s3_drv_info == NULL) {
				i2s3_drv_info = (i2s_drv_info_t *)os_malloc(sizeof(i2s_drv_info_t));
				if (!i2s3_drv_info) {
					return BK_FAIL;
				}
				i2s3_drv_info->chl1_cfg = NULL;
				i2s3_drv_info->chl2_cfg = NULL;
				i2s3_drv_info->chl3_cfg = NULL;
			}
			drv_info = i2s3_drv_info;
			break;
		...
		case I2S_GPIO_GROUP_4:
			...
			i2s_hal_set_cfg_index(4);
			sys_drv_i2s4_clock_en(CLK_PWR_CTRL_PWR_UP);
			i2s_hal_soft_reset();
			sys_drv_i2s4_int_en(1);
			os_memset(&s_i2s4_isr, 0, sizeof(s_i2s4_isr));
			int_config_table.int_src = INT_SRC_I2S4;
			int_config_table.isr = i2s_isr;
			s_i2s4_driver_is_init = true;
			if (i2s4_drv_info == NULL) {
				i2s4_drv_info = (i2s_drv_info_t *)os_malloc(sizeof(i2s_drv_info_t));
				if (!i2s4_drv_info) {
					return BK_FAIL;
				}
				i2s4_drv_info->chl1_cfg = NULL;
				i2s4_drv_info->chl2_cfg = NULL;
				i2s4_drv_info->chl3_cfg = NULL;
			}
			drv_info = i2s4_drv_info;
			break;
#endif
	...
	switch (chl_id) {
		case I2S_CHANNEL_1:
			if (i2s_index == 0) {
				...
				*i2s_data_addr = I2S3_PCM_DAT_ADDR;
			} else if (i2s_index == 4) {
				*i2s_data_addr = I2S4_PCM_DAT_ADDR;
#endif
			}
			break;
		...
		case I2S_CHANNEL_2:
			if (i2s_index == 0) {
				...
				*i2s_data_addr = I2S3_PCM_DAT2_ADDR;
			} else if (i2s_index == 4) {
				*i2s_data_addr = I2S4_PCM_DAT2_ADDR;
#endif
			}
			break;
		...
		case I2S_CHANNEL_3:
			if (i2s_index == 0) {
				...
				*i2s_data_addr = I2S3_PCM_DAT3_ADDR;
			} else if (i2s_index == 4) {
				*i2s_data_addr = I2S4_PCM_DAT3_ADDR;
#endif
			}
			break;
...
bk_err_t bk_i2s_set_samp_rate(i2s_samp_rate_t samp_rate)
{
	...
	if (apll_clk == 98304000)
		sys_drv_apll_cal_val_set(0x8973CA70);
	else if (apll_clk == 90316800)
		sys_drv_apll_cal_val_set(0x88AF2ECA);
	else
		sys_drv_apll_cal_val_set(0x8973CA70);
	sys_drv_apll_config_set(0xC2A06AA6);
#else
	if (apll_clk == 98304000)
		sys_drv_apll_cal_val_set(0x8973CA6F);
	else if (apll_clk == 90316800)
	...
	sys_drv_apll_config_set(0xC2A0AE86);
#endif
	sys_drv_i2s0_ckdiv_set(3);   // Support???
	sys_drv_apll_spi_trigger_set(1);
	delay(10);
	sys_drv_apll_spi_trigger_set(0);
	...
	if (type == I2S_TXRX_TYPE_RX) {
		switch (chl_id) {
			case I2S_CHANNEL_1:
				if (i2s_index == 0) {
#if CONFIG_SOC_BK7259
					dma_config.src.dev = DMA_DEV_I2S0_RX;
#else
					...
					dma_config.src.dev = DMA_DEV_I2S3_RX;
				} else if (i2s_index == 4) {
					dma_config.src.dev = DMA_DEV_I2S4_RX;
#endif
				}
				break;
			case I2S_CHANNEL_2:
		...
		I2S_LOGD("RX: dma_config.src.start_addr=%p, dma_config.src.end_addr=%p, dma_config.dst.start_addr=%p, dma_config.dst.end_addr=%p\n", 
            dma_config.src.start_addr, dma_config.src.end_addr, dma_config.dst.start_addr, dma_config.dst.end_addr);
	} else {
		...
		switch (chl_id) {
			case I2S_CHANNEL_1:
				if (i2s_index == 0) {
#if CONFIG_SOC_BK7259
					dma_config.dst.dev = DMA_DEV_I2S0;
#else
					...
					dma_config.dst.dev = DMA_DEV_I2S3;
				} else if (i2s_index == 4) {
					dma_config.dst.dev = DMA_DEV_I2S4;
#endif
				}
				break;
			case I2S_CHANNEL_2:
        ...
        I2S_LOGD("TX: dma_config.src.start_addr=%p, dma_config.src.end_addr=%p, dma_config.dst.start_addr=%p, dma_config.dst.end_addr=%p\n", 
            dma_config.src.start_addr, dma_config.src.end_addr, dma_config.dst.start_addr, dma_config.dst.end_addr);
	}
