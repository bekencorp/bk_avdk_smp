...
static void alarm_dump_node(alarm_node_t *node_p)
{
	...
	ANA_RTC_LOGV("node_p=0x%x\r\n", node_p);
	if (node_p)
	{
		ANA_RTC_LOGV("next=0x%x\r\n", node_p->next);
		ANA_RTC_LOGV("name=%s\r\n", node_p->name);
		ANA_RTC_LOGV("period_tick=0x%x\r\n", node_p->period_tick);
		ANA_RTC_LOGV("period_cnt=%d\r\n", node_p->period_cnt);
		ANA_RTC_LOGV("start_tick=0x%x\r\n", node_p->start_tick);
		ANA_RTC_LOGV("expired_tick=0x%x\r\n", node_p->expired_tick);
	}
...
static int32_t alarm_insert_node(aon_rtc_id_t id, alarm_node_t *node_p)
{
	...
	if (cur_p == NULL)
	{
		...
		ANA_RTC_LOGV("insert first node 0x%x,name=%s\r\n", node_p, node_p->name);
	...
	if (next_p == NULL)
	{
		...
		// TODO:log debug
		ANA_RTC_LOGV("list total has two nodes\r\n");
...
static alarm_node_t *alarm_remove_node(aon_rtc_id_t id, uint8_t *name_p)
{
	...
	{
		ANA_RTC_LOGV("%s:can't find %s alarm\r\n", __func__, name_p);
	}
 ...
 */
#if 0
				ANA_RTC_LOGV("last alarm:free=0x%x,name=%s\r\n", cur_p, cur_p->name);
				os_free(cur_p);
#endif
				ana_rtc_release_node(id, cur_p);
			}
			...
			else
			{
				...
				else // only self
				{
					...
					cur_p->expired_tick += cur_p->period_tick;
					ANA_RTC_LOGV("%s update next expired time %d \r\n", cur_p->name, cur_p->expired_tick);
				}
			}
		}
		else // no expired
...
static void __BK_IRQ ana_rtc1_isr_handler(void)
{
...
void ana_rtc_update(void)
{
	...
	s_high_tick++;
	#else
	// default value for timersel
	s_ana_rtc_base_tick += 0x100000;
	#endif
	#endif
}
...
#if CONFIG_CLI && CONFIG_AON_RTC_TEST
...
bk_err_t bk_alarm_register(aon_rtc_id_t id, alarm_info_t *alarm_info_p)
{
	...
	{
		ANA_RTC_LOGV("period_tick should not smaller then %d\r\n", AON_RTC_PRECISION_TICK);
		return BK_FAIL;
	}
...
bool bk_alarm_is_registered(aon_rtc_id_t id, const uint8_t *name_p)
{
	...
	if (list_corrupt) {
		ANA_RTC_LOGW("%s: alarm list corrupt or too long (>%d), name=%s\r\n",
...
bk_err_t bk_rtc_gettimeofday(struct timeval *tv, void *ptz)
{
	...
	if (tv != NULL)
	{
		...
		ANA_RTC_LOGV("s_boot_time_us:h=0x%x,l=0x%x \r\n", s_boot_time_us >> 32, (uint32_t)s_boot_time_us);
		ANA_RTC_LOGV("%s sec=%d,us:%d\n", __func__, tv->tv_sec, tv->tv_usec);
	}
	else
