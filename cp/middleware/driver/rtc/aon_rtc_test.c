...
static void cli_aon_rtc_help(void)
{
	...
	CLI_LOGD("aon_rtc <id> create <tick> <period>\r\n");
	CLI_LOGD("aon_rtc <id> destroy\r\n");
	CLI_LOGD("aon_rtc_int <id> tick\r\n");
	CLI_LOGD("aon_rtc_int <id> upper\r\n");
#endif
...
#if 0	//remove it, only one HW can't be used for many APPs.
static void __BK_IRQ cli_aon_rtc_tick_isr(aon_rtc_id_t id, uint8_t *name_p, void *param)
{
	CLI_LOGD("aon_rtc_tick_isr(%d)\n", id);
}

static void __BK_IRQ cli_aon_rtc_upper_isr(aon_rtc_id_t id, uint8_t *name_p, void *param)
{
...
static void cli_aon_rtc_time_of_day(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
...
#if 0  // requires RTC to be configured and enabled
	struct timeval tv;
	struct timezone tz;
	...
	else if (os_strcmp(argv[1], "get_deepsleep_time") == 0)
	{
		...
		CLI_LOGD("%s deepsleep_second = %ds ret: %d\r\n", __func__, time, ret);
	}
	else
		cli_aon_rtc_help();
#endif
...
DRV_CLI_CMD_EXPORT static const struct cli_command s_aon_rtc_commands[] = {
...
int bk_ano_rtc_register_cli_test_feature(void)
{
	...
	return cli_register_module_test_feature(s_aon_rtc_commands, AON_RTC_CMD_CNT);
}
