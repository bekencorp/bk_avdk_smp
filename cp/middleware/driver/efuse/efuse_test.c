...
DRV_CLI_CMD_EXPORT static const struct cli_command s_efuse_commands[] = {
	{"efuse_driver", "efuse_driver {init|deinit}", cli_efuse_driver_cmd},
	{"efuse_test", "efuse_test {write|read}", cli_efuse_cmd}
};

int bk_efuse_register_cli_test_feature(void)
{
	return cli_register_module_test_feature(s_efuse_commands, EFUSE_CMD_CNT);
}
// eof
