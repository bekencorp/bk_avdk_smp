...
/* USR_CONFIG @ 0x4E7000 — CP slots start after AP's peer at 0x4EB000. */
#ifndef CONFIG_FLASH_CONC_NS_ADDR
#define CONFIG_FLASH_CONC_NS_ADDR     0x4EC000
#endif
#ifndef CONFIG_FLASH_CONC_S_ADDR
#define CONFIG_FLASH_CONC_S_ADDR      0x4ED000
#endif
#ifndef CONFIG_FLASH_CONC_SMP0_ADDR
#define CONFIG_FLASH_CONC_SMP0_ADDR   0x4EE000
#endif
#ifndef CONFIG_FLASH_CONC_SMP1_ADDR
#define CONFIG_FLASH_CONC_SMP1_ADDR   0x4EF000
#endif
#ifndef CONFIG_FLASH_CONC_PEER_ADDR
#define CONFIG_FLASH_CONC_PEER_ADDR   0x4F0000
#endif
...
static flash_conc_worker_t s_workers[FLASH_CONC_WORKER_MAX] = {
	[FLASH_CONC_IDX_NS] = {
		.name = "fc_ns", .tag = 0xA0, .path = FLASH_CONC_PATH_NS,
	...
	[FLASH_CONC_IDX_S] = {
		.name = "fc_s", .tag = 0xB0, .path = FLASH_CONC_PATH_S,
		.core = FLASH_CONC_CORE_ANY, .addr = CONFIG_FLASH_CONC_S_ADDR,
		.size = CONFIG_FLASH_CONC_SIZE, .interval_ms = CONFIG_FLASH_CONC_INTERVAL_MS,
	},
	[FLASH_CONC_IDX_SMP0] = {
		.name = "fc_smp0", .tag = 0xC0, .path = FLASH_CONC_PATH_NS,
		.core = FLASH_CONC_CORE_0, .addr = CONFIG_FLASH_CONC_SMP0_ADDR,
		.size = CONFIG_FLASH_CONC_SIZE, .interval_ms = CONFIG_FLASH_CONC_INTERVAL_MS,
	},
	[FLASH_CONC_IDX_SMP1] = {
		.name = "fc_smp1", .tag = 0xD0, .path = FLASH_CONC_PATH_NS,
		.core = FLASH_CONC_CORE_1, .addr = CONFIG_FLASH_CONC_SMP1_ADDR,
		.size = CONFIG_FLASH_CONC_SIZE, .interval_ms = CONFIG_FLASH_CONC_INTERVAL_MS,
	},
	[FLASH_CONC_IDX_PEER] = {
		.name = "fc_peer", .tag = 0xE0, .path = FLASH_CONC_PATH_NS,
	...
	if (os_strcmp(argv[1], "peer") == 0) {
		...
		if (os_strcmp(argv[2], "start") == 0) {
			...
			if (ret != BK_OK)
				...
				CLI_LOGD("CP peer started; also run on AP: flash_conc peer start\r\n");
		} else if (os_strcmp(argv[2], "stop") == 0) {
