...
static const flash_config_t flash_config[] = {
	...
	{0xCD6017,   FLASH_SIZE_8M,   2,             FLASH_LINE_MODE_FOUR,   14,       2,            0x1F,         0x1F,        0x00,         0x00E,                9,            1,           0xA0,                         }, //th_25q64ha
	{0xCD7017,   FLASH_SIZE_8M,   2,             FLASH_LINE_MODE_FOUR,   14,       2,            0x1F,         0x1F,        0x00,         0x00E,                9,            1,           0xA0,                         }, //TH25Q64UB
...
#if CONFIG_FLASH_CP_AP_DIRECT_ACCESS
	if (bk_flash_shared_lock_init(true) != BK_OK)
		return BK_FAIL;
#endif
...
#if (CONFIG_CPU_CNT > 1)
	/* Keep the server alive even in direct mode so a mismatched AP proxy image
	 * fails gracefully instead of waiting forever for a missing mailbox peer. */
...
#if CONFIG_FLASH_CP_AP_DIRECT_ACCESS
	bk_flash_shared_set_flash_init_done();
#endif
