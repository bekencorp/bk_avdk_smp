...
static void dma_id_enable_interrupt_common(dma_id_t id)
{
	...
	#elif CONFIG_SPE
        #if (CONFIG_SOC_SMP)
            sys_drv_core_intr_group1_enable(CPU0_CORE_ID, GDMA_INTERRUPT_CTRL_BIT);
            sys_drv_core_intr_group2_enable(CPU0_CORE_ID, DMA1_SEC_INTERRUPT_CTRL_BIT);
        #else
            sys_drv_int_enable(GDMA_INTERRUPT_CTRL_BIT);
...
bk_err_t bk_dma_driver_init(void)
{
	...
	{
		u32 start_id = CONFIG_DMA_LOGIC_CHAN_ID_MIN;
		u32 end_id = CONFIG_DMA_LOGIC_CHAN_ID_MIN + CONFIG_DMA_LOGIC_CHAN_CNT;
		u32 max_id = (end_id > DMA_ID_MAX) ? DMA_ID_MAX : end_id;
		for (u32 i = start_id; i < max_id; i++) {
...
#if CONFIG_GENERAL_DMA_TEST
    int bk_dma_register_cli_test_feature(void);
    bk_dma_register_cli_test_feature();
#endif
...
static void __BK_IRQ dma_isr(void)
{
	DMA_LOGV("dma_isr hw 0\r\n");
	dma_isr_common(0);
}

static void __BK_IRQ dma1_isr(void)
{
