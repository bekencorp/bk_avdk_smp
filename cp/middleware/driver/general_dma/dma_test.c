...
static void cli_dma_help(void)
{
    ...
    CLI_LOGD("dma_link_test {desc_cnt} {len1} [len2] [len3] ...\r\n");
    CLI_LOGD("  Example: dma_link_test 3 256 512 128  (3 descriptors with lengths 256, 512, 128)\r\n");
#endif
}
...
#if CONFIG_GENERAL_DMA_LINK_MODE
static void cli_dma_link_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    if (argc < 3) {
        cli_dma_help();
        return;
    ...
    uint32_t desc_cnt = os_strtoul(argv[1], NULL, 10);
    
    if (desc_cnt == 0 || desc_cnt > 16) {
        CLI_LOGE("Invalid descriptor count: %d (max 16)\r\n", desc_cnt);
        return;
    }

    // Allocate source and destination buffers (16-byte aligned)
    uint32_t total_len = 0;
    uint32_t lens[16] = {0};
    
    // Parse lengths (starting from argv[2])
    for (uint32_t i = 0; i < desc_cnt && (i + 2) < argc; i++) {
        lens[i] = os_strtoul(argv[i + 2], NULL, 10);
        total_len += lens[i];
    }
    
    if (total_len == 0) {
        CLI_LOGE("Total length is 0\r\n");
        return;
    }
    ...
    uint32_t src_addr = ((uint32_t)src_buf + 15) & ~15;
    uint32_t dst_addr = ((uint32_t)dst_buf + 15) & ~15;

    // Initialize source buffer with test pattern
    for (uint32_t i = 0; i < total_len; i++) {
        ((uint8_t *)src_addr)[i] = (uint8_t)(i & 0xFF);
    }
    
    // Clear destination buffer
    os_memset((void *)dst_addr, 0, total_len);

    CLI_LOGD("DMA Link Test: desc_cnt=%d, src=0x%x, dst=0x%x, total_len=%d\r\n", 
             desc_cnt, src_addr, dst_addr, total_len);

    // Initialize descriptor table
    void *desc_table = bk_dma_link_init(desc_cnt);
    if (desc_table == NULL) {
    ...
    dma_link_config_t configs[16] = {0};
    uint32_t src_offset = 0;
    uint32_t dst_offset = 0;

    for (uint32_t i = 0; i < desc_cnt; i++) {
        if (lens[i] == 0) {
            lens[i] = total_len / desc_cnt;  // Distribute evenly if not specified
        }
        
        configs[i].src_addr = src_addr + src_offset;
        configs[i].dst_addr = dst_addr + dst_offset;
        configs[i].ctrl.bits.length = lens[i];
        configs[i].ctrl.bits.int_finish_en = (i == desc_cnt - 1) ? 1 : 0;  // Enable interrupt on last descriptor
        configs[i].ctrl.bits.int_half_finish_en = 0;

        CLI_LOGD("Desc[%d]: src=0x%x, dst=0x%x, len=%d\r\n", 
                 i, configs[i].src_addr, configs[i].dst_addr, lens[i]);
    ...
    // Set all descriptors
    bk_err_t ret = bk_dma_link_set_descs(desc_table, configs, desc_cnt);
    if (ret != BK_OK) {
        CLI_LOGE("Failed to set descriptors: %d\r\n", ret);
        bk_dma_link_deinit(desc_table);
        os_free(src_buf);
        os_free(dst_buf);
        return;
    }
    ...
    dma_id_t dma_id = bk_dma_alloc(DMA_DEV_DTCM);
    if (dma_id >= SOC_DMA_CHAN_NUM_PER_UNIT * SOC_DMA_UNIT_NUM) {
    ...
    CLI_LOGD("Starting DMA link transfer with channel %d...\r\n", dma_id);
    
    // Start linked list transfer (channel ID passed by application layer)
    ret = bk_dma_link_transfer(dma_id, desc_table);
    if (ret != BK_OK) {
        CLI_LOGE("Failed to start link transfer: %d\r\n", ret);
        bk_dma_free(DMA_DEV_DTCM, dma_id);
        bk_dma_link_deinit(desc_table);
        os_free(src_buf);
        os_free(dst_buf);
        return;
	...
	// JUST FOR TEST, SHOULD BE REMOVED IN PRODUCTION CODE,USE ISR INSTEAD
    // Wait for transfer to complete by polling remain length
    // For simplicity, wait with timeout and check if data is transferred
    ...
    while (timeout > 0) {
        ...
        // Check if transfer might be complete (simple timeout-based check)
        // In real scenario, you would check remain_len or use ISR
        if (wait_count > 100) {  // Wait up to 1 second
            break;
        }
    }

    CLI_LOGD("DMA link transfer wait completed (waited %d ms)\r\n", wait_count * 10);

    // Verify data
    uint32_t error_count = 0;
    for (uint32_t i = 0; i < total_len; i++) {
        if (((uint8_t *)dst_addr)[i] != ((uint8_t *)src_addr)[i]) {
            if (error_count < 10) {  // Only print first 10 errors
                CLI_LOGE("Data mismatch at offset %d: src=0x%02x, dst=0x%02x\r\n", 
                         i, ((uint8_t *)src_addr)[i], ((uint8_t *)dst_addr)[i]);
            }
            error_count++;
        }
    }
    ...
    if (error_count == 0) {
        CLI_LOGD("DMA link test PASSED: All %d bytes transferred correctly\r\n", total_len);
    } else {
        CLI_LOGE("DMA link test FAILED: %d bytes mismatch\r\n", error_count);
    }

    // Cleanup
    bk_dma_free(DMA_DEV_DTCM, dma_id);  // Free DMA channel allocated by application
    bk_dma_link_deinit(desc_table);
    os_free(src_buf);
    os_free(dst_buf);
}
...
static void cli_dma_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	bk_dma_register_bus_err_isr(id, dma_bus_err_int_cb);
...
DRV_CLI_CMD_EXPORT static const struct cli_command s_dma_commands[] = {
...
#if CONFIG_GENERAL_DMA_LINK_MODE
    {"dma_link_test", "dma_link_test {desc_cnt} {len1} [len2] [len3] ...", cli_dma_link_test_cmd},
#endif
};

int bk_dma_register_cli_test_feature(void)
{
    BK_LOG_ON_ERR(bk_dma_driver_init());
    return cli_register_module_test_feature(s_dma_commands, DMA_CMD_CNT);
}
