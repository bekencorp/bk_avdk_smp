...
bk_err_t bk_scr_init(gpio_scr_map_group_t group)
{
	...
	sys_hal_set_7816_int_en(rtos_get_core_id(),1); //enable 7816 interrupt
	sys_hal_set_scr_clk(1); //enable 7816 clock
	bk_scr_gpio_init(group); //init 7816 gpio

	scr_hal_soft_reset();

#if CONFIG_CLI && CONFIG_SCR_TEST
	int bk_scr_register_cli_test_feature(void);
	bk_scr_register_cli_test_feature();
#endif
