...
DRV_CLI_CMD_EXPORT static const struct cli_command s_trng_commands[] = {
	{"trng_driver", "{init|deinit}", cli_trng_driver_cmd},
	{"trng", "trng {start|stop|get}", cli_trng_cmd}
};

int bk_trng_register_cli_test_feature(void)
{
	BK_LOG_ON_ERR(bk_trng_driver_init());
	return cli_register_module_test_feature(s_trng_commands, TRNG_CMD_CNT);
}
// eof
