...
默认的资源映射配置（可在 `hspl_res_lock.c` 中修改）：

#### 资源映射规则

资源映射采用**自动映射规则**，资源ID直接对应HSPL实例和通道：

- **资源 0-15**：使用 `BK_HSPL_ID_0` (M52) 的通道 0-15（**CP可用**）
- **资源 16-31**：使用 `BK_HSPL_ID_1` (M55) 的通道 0-15（**CP不可用**，仅AP SMP）

#### CP侧可用资源（0-15）

| 资源ID | 资源名称 | HSPL 实例 | 通道 |
|--------|---------|-----------|------|
| 0 | FLASH | BK_HSPL_ID_0 (M52) | 0 |
| 1 | CLOCK | BK_HSPL_ID_0 (M52) | 1 |
| 2 | POWER | BK_HSPL_ID_0 (M52) | 2 |
| 3 | SYS | BK_HSPL_ID_0 (M52) | 3 |
| 4 | RTC | BK_HSPL_ID_0 (M52) | 4 |
| 5 | ANA | BK_HSPL_ID_0 (M52) | 5 |
| 6 | FUSE | BK_HSPL_ID_0 (M52) | 6 |
| 7 | TRNG | BK_HSPL_ID_0 (M52) | 7 |
| 8 | WDT | BK_HSPL_ID_0 (M52) | 8 |
| 9 | SPI | BK_HSPL_ID_0 (M52) | 9 |
| 10 | GPIO | BK_HSPL_ID_0 (M52) | 10 |
| 11 | PWM | BK_HSPL_ID_0 (M52) | 11 |
| 12 | ADC | BK_HSPL_ID_0 (M52) | 12 |
| 13 | DAC | BK_HSPL_ID_0 (M52) | 13 |
| 14 | PMU | BK_HSPL_ID_0 (M52) | 14 |
| 15 | USER0 | BK_HSPL_ID_0 (M52) | 15 |

#### CP侧不可用资源（16-31）

资源16-31使用 `BK_HSPL_ID_1`，CP侧调用会返回 `BK_ERR_NOT_SUPPORT`。这些资源包括：
- OS, LVGL, AUDIO, VIDEO, GPU, NPU, DSP, ISP, VDEC, VENC, SDIO系列, USB, USER1-3

> **注意**：CP侧资源锁接口已限制使用 `BK_HSPL_ID_1`（资源16-31），调用会返回 `BK_ERR_NOT_SUPPORT`。
