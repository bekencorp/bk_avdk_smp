...
static void timer_clock_enable(timer_id_t id)
{
	...
	{
		case 0:
            sys_hal_clk_pwr_ctrl(CLK_PWR_ID_TIMER0, CLK_PWR_CTRL_PWR_UP);
			break;
		case 1:
...
static void timer_clock_disable(timer_id_t id)
{
	...
	{
		case 0:
            sys_hal_clk_pwr_ctrl(CLK_PWR_ID_TIMER0, CLK_PWR_CTRL_PWR_DOWN);
			break;
		case 1:
...
#if CONFIG_CLI && CONFIG_TIMER_TEST
    int bk_timer_register_cli_test_feature(void);
    bk_timer_register_cli_test_feature();
#endif
