...
{
    CLI_LOGD("[TIMER][ISR] chan:%d clk:%d\r\n", chan, bk_timer_get_clock_src(chan));
}
...
static void cli_timer_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    } else if (os_strcmp(argv[2], "clk_src") == 0) {
        uint32_t clk_src = os_strtoul(argv[3], NULL, 10);
        CLI_LOGD("[TIMER][SET_CLOCK_SRC] 0-32K 1-xtal, clk_src:%d\r\n", clk_src);
        bk_timer_clock_select(channel, clk_src);
...
{
    return cli_register_module_test_feature(s_timer_commands, TIMER_CMD_CNT);
}
