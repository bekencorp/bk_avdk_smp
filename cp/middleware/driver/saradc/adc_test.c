...
static float cli_adc_read_single_chan(UINT8 adc_chan, uint32_t clk)
{
    ...
    CLI_LOGD("adc:%d, volt value:%d mv\n", value, (uint32_t)(cali_value*1000));
    s_adc_reading_flag = 0;
...
static void cli_sadc_multi_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    if(4 != argc) {
        CLI_LOGD("sadc parameters count are exceptional\r\n");
        return ;
    }

    if(os_strcmp(argv[2], "chan") == 0) {
        channel_id = os_strtoul(argv[3], NULL, 10);
        CLI_LOGD("adc_channel = %d\r\n",channel_id);
    } else {
        CLI_LOGD("get sadc channel exceptionally\r\n");
        return;
    }
    ...
    if(os_strcmp(argv[1], "config") == 0) {
        ...
        if(BK_OK == ret) {
            CLI_LOGD("dump_hex rx_data[0x%x] cnt:%d\r\n", recv_data, got_cnt);
            bk_dump_hex(NULL, (uint8_t *)recv_data, got_cnt);
        } else {
            CLI_LOGD("get_raw failed ret:0x%x got_cnt:%d\r\n", ret, got_cnt);
        }
        if(recv_data) {
            os_free(recv_data);
            recv_data = NULL;
        ...
        buf_size = 32;
        CLI_LOGD("[2]rxing_data cnt expected_cnt:%d\r\n", buf_size/DEFAULT_ADC_SAMPLE_UNIT_BYTES);
        recv_data = (uint16_t *)os_zalloc(buf_size);
        if(NULL == recv_data) {
            CLI_LOGD("malloc failed buf_size:%d\r\n", buf_size);
            return;
        ...
        buf_size = 128;
        CLI_LOGD("[3]rxing_data cnt expected_cnt:%d\r\n", buf_size/DEFAULT_ADC_SAMPLE_UNIT_BYTES);
        recv_data = (uint16_t *)os_zalloc(buf_size);
        if(NULL == recv_data) {
            CLI_LOGD("malloc failed buf_size:%d\r\n", buf_size);
            return;
        ...
        CLI_LOGD("malloc recv_data:0x%x\r\n", recv_data);
        for(i = 0; i < (buf_size / DEFAULT_ADC_SAMPLE_UNIT_BYTES); i ++) {
            CLI_LOGD("[%d] recv_ptr:0x%x\r\n", i, &recv_data[i]);
            ret = bk_adc_channel_read(channel_id, &recv_data[i], BEKEN_WAIT_FOREVER);
        }
        ...
        CLI_LOGD("sadc parameters are exceptional\r\n");
        return ;
    }
...
static void cli_adc_rate_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    if(3 != argc) {
        CLI_LOGD("saradc_rate parameter exceptioanl\r\n");
        return;
    }
    ...
    CLI_LOGD("saradc_rate channel:%d clk:0x%x\r\n", adc_chan, clk);
...
static void cli_adc_cont_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    if(0 == param_in_cnt)
    {
        CLI_LOGD("input partam is exceptional\r\n");
        return;
    }

    if(s_adc_reading_flag == 1)
    {
        CLI_LOGD("adc_cont_read is running\r\n");
        return;
    }
    ...
    if(output_mode == SARADC_PSRAM_MODE) {
        ...
        if(NULL == s_sram_buf) {
            CLI_LOGD("malloc_buf failed\r\n");
            return;
        }
        s_rd_data_buf = s_sram_buf;
    } else {
        CLI_LOGD("Please configure mode, which decides the malloc space\r\n");
        return;
    }
...
{
    return cli_register_module_test_feature(s_adc_commands, ADC_CMD_CNT);
}
// eof
