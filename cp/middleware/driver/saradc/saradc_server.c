...
static void saradc_acquire_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_ACQUIRE, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
static void saradc_init_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_INIT, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
static void saradc_enable_bypass_calibration_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_ENABLE_BYPASS_CALIBRATION, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
static void saradc_start_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_START, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
static void saradc_read_raw_handler(u32 handle, saradc_cmd_t *cmd_buff, u8 connect_id)
{
	...
	{
		saradc_error_handler(handle, MB_SARADC_CMD_READ_RAW);
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_READ_RAW, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	...
	if((ret_val != sizeof(saradc_cmd_t)) || (user_cmd != MB_SARADC_CMD_READ_RAW_DONE))
	{
	...
	else
	{
		...
		mb_ipc_send(handle, MB_SARADC_CMD_READ_RAW_DONE, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	}
}
...
static void saradc_read_handler(u32 handle, saradc_cmd_t *cmd_buff, u8 connect_id)
{
	...
	if(cmd_buff->size > sizeof(saradc_buff))
	{
		saradc_error_handler(handle, MB_SARADC_CMD_READ);
		TRACE_I(TAG, "%s @%d, 0x%x buffer overflow!\r\n", __FUNCTION__, __LINE__, handle);
		return;
	}
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_READ, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	...
	if((ret_val != sizeof(saradc_cmd_t)) || (user_cmd != MB_SARADC_CMD_READ_DONE))
	{
	...
	else
	{
		memset(cmd_buff, 0, sizeof(saradc_cmd_t));
		cmd_buff->ret_status = BK_OK;
		mb_ipc_send(handle, MB_SARADC_CMD_READ_DONE, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	}
}
...
static void saradc_stop_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_STOP, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
static void saradc_deinit_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_DEINIT, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
static void saradc_release_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_RELEASE, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
static void saradc_set_config_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_SET_CONFIG, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
{
	cmd_buff->ret_status = BK_FAIL;//junpeng_bringup//bk_adc_en();
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_EN, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
static void saradc_data_calculate_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_DATA_CALCULATE, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
static void saradc_chan_init_gpio_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_INIT_GPIO, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
...
static void saradc_chan_deinit_gpio_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_DEINIT_GPIO, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
		TRACE_I(TAG, "0x%x, deinit adc chan gpio%d: %d, %d.\r\n", handle, cmd_buff->config.chan, cmd_buff->ret_status, ret_val);
}
...
static void saradc_set_chan_handler(u32 handle, saradc_cmd_t *cmd_buff)
{
	...
	int ret_val = mb_ipc_send(handle, MB_SARADC_CMD_SET_CHANNEL, (u8 *)cmd_buff, sizeof(saradc_cmd_t), SARADC_SVR_WAIT_TIME);
	if(ret_val != 0)
		TRACE_I(TAG, "0x%x, deinit adc chan gpio%d: %d, %d.\r\n", handle, cmd_buff->config.chan, cmd_buff->ret_status, ret_val);
}
