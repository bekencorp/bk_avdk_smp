...
extern void soc_mpu_cfg(void);

__attribute__((section(".iram"))) void mpu_register_regions(ARM_MPU_Region_t *regions, uint32_t region_cnt)
{
	BK_ASSERT(regions);
...
__attribute__((section(".iram"))) void mpu_register_attrs(uint8_t *attrs, uint32_t cnt)
{
	BK_ASSERT(attrs);
	BK_ASSERT(cnt);
...
__attribute__((section(".iram"))) void mpu_enable(void)
{
    uint32_t region_num;
    uint32_t attr_num;
