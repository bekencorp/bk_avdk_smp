...
bk_err_t irda_hal_init_tx(const irda_tx_init_config_t *tx_config)
{
	...
	/*When connecting an infrared transmitter, enable the carrier wave output*/
	irda_ll_set_reg0xa_carrier_enable(1);
...
bk_err_t irda_hal_set_tx_need_wr_int(bool enable)
{
	irda_ll_set_reg0x8_tx_need_wr_mask(enable);
	return BK_OK;
}
...
bk_err_t irda_hal_write_byte(uint32_t data)
{
	irda_ll_set_reg0x7_fifo_data_tx(data & IRDA_REG0X7_FIFO_DATA_TX_MASK);
	return BK_OK;
}

uint32_t irda_hal_read_byte(void)
{
	return irda_ll_get_reg0x7_fifo_data_rx();
}

uint32_t irda_hal_get_tx_fifo_count(void)
{
	return irda_ll_get_reg0x6_tx_fifo_count();
}

uint32_t irda_hal_get_rx_fifo_count(void)
{
	return irda_ll_get_reg0x6_rx_fifo_count();
}

bool irda_hal_is_rx_fifo_empty(void)
{
	return !!irda_ll_get_reg0x6_rx_fifo_empty();
}

bool irda_hal_is_rx_fifo_read_ready(void)
{
	return !!irda_ll_get_reg0x6_fifo_rd_ready();
}
...
bool irda_hal_is_tx_need_wr_int_triggered(uint32_t int_status)
{
	return !!(int_status & BIT(IRDA_REG0X9_TX_NEED_WR_STATE_POS));
}
