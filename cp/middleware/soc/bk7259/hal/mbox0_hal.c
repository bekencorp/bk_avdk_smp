...
static int mbox0_hal_chn3_cfg_fifo(mbox0_hal_t* hal, uint8_t start, uint8_t len)
{
	mbox0_hw_t * hw = hal->hw;
	...
	mbox0_ll_set_reg_0x40_fifo_start(hw, start);
	mbox0_ll_set_reg_0x41_fifo_length(hw, len);
	mbox0_ll_set_reg_0x41_chn_enable(hw, 1);
...
static int mbox0_hal_chn3_send(mbox0_hal_t* hal, mbox0_message_t * msg)
{
	mbox0_hw_t * hw = hal->hw;
	...
	mbox0_ll_set_reg_0x42_mail_tdata0(hw, msg->data[0]);
	mbox0_ll_set_reg_0x43_mail_tdata1(hw, msg->data[1]);
	mbox0_ll_set_reg_0x44_mail_tid(hw, msg->dest_cpu);
...
static int mbox0_hal_chn3_recv(mbox0_hal_t* hal, mbox0_message_t * msg)
{
	mbox0_hw_t * hw = hal->hw;
	...
	msg->src_cpu = mbox0_ll_get_reg_0x45_mail_sid(hw);
	msg->data[0] = mbox0_ll_get_reg_0x46_mail_rdata0(hw);
	msg->data[1] = mbox0_ll_get_reg_0x47_mail_rdata1(hw);
...
static int mbox0_hal_chn3_int_enable(mbox0_hal_t* hal, uint8_t enable)
{
	mbox0_hw_t * hw = hal->hw;
	...
	mbox0_ll_set_reg_0x40_int_wrerr_en(hw, 0);
	mbox0_ll_set_reg_0x40_int_rderr_en(hw, 0);
	mbox0_ll_set_reg_0x40_int_wrfull_en(hw, 0);

	mbox0_ll_set_reg_0x40_int_en(hw, enable);
...
static uint32_t mbox0_hal_chn3_get_int_status(mbox0_hal_t* hal)
{
	mbox0_hw_t * hw = hal->hw;
...
static uint32_t mbox0_hal_chn3_get_rx_fifo_stat(mbox0_hal_t* hal)
{
	mbox0_hw_t * hw = hal->hw;
	...
	if(mbox0_ll_get_reg_0x40_fifo_noempt(hw) != 0)
		rx_fifo_stat |= RX_FIFO_STAT_NOT_EMPTY;
	if(mbox0_ll_get_reg_0x48_fifo_empt(hw) != 0)
		rx_fifo_stat |= RX_FIFO_STAT_EMPTY;
	if(mbox0_ll_get_reg_0x48_fifo_full(hw) != 0)
		rx_fifo_stat |= RX_FIFO_STAT_FULL;
...
static uint32_t mbox0_hal_chn3_get_rx_fifo_count(mbox0_hal_t* hal)
{
	mbox0_hw_t * hw = hal->hw;
	...
	return mbox0_ll_get_reg_0x48_fifo_count(hw);
}
...
static uint32_t mbox0_hal_chn3_clear_tx_fifo_stat(mbox0_hal_t* hal)
{
	mbox0_hw_t * hw = hal->hw;
	...
	if(mbox0_ll_get_reg_0x40_wrerr_int_sta(hw) != 0)
	{
		tx_fifo_stat |= TX_FIFO_STAT_WR_ERR;
	}
	if(mbox0_ll_get_reg_0x40_rderr_int_sta(hw) != 0)
	{
		tx_fifo_stat |= TX_FIFO_STAT_RD_ERR;
	}
	if(mbox0_ll_get_reg_0x40_wrfull_int_sta(hw) != 0)
	{
		tx_fifo_stat |= TX_FIFO_STAT_WR_FULL;
	}

	mbox0_ll_set_reg_0x40_wrerr_int_sta(hw, 1);
	mbox0_ll_set_reg_0x40_rderr_int_sta(hw, 1);
	mbox0_ll_set_reg_0x40_wrfull_int_sta(hw, 1);
...
const hal_chn_drv_t  hal_chn3_drv =
{
	.chn_cfg_fifo = mbox0_hal_chn3_cfg_fifo,
	.chn_send = mbox0_hal_chn3_send,
	.chn_recv = mbox0_hal_chn3_recv,
	.chn_int_enable = mbox0_hal_chn3_int_enable,
	.chn_get_int_status = mbox0_hal_chn3_get_int_status,
	.chn_get_rx_fifo_stat = mbox0_hal_chn3_get_rx_fifo_stat,
	.chn_get_rx_fifo_count = mbox0_hal_chn3_get_rx_fifo_count,
	.chn_clear_tx_fifo_stat = mbox0_hal_chn3_clear_tx_fifo_stat,
} ;
