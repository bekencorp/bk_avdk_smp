...
static inline void dma_ll_init(dma_hw_t *hw)
{
	...
	hw->secure_attr.v = 0xFF;  // attr is 8-bit, so use 0xFF instead of 0xFFF
	hw->privileged_attr.v = 0xFF;  // attr is 8-bit, so use 0xFF instead of 0xFFF
#endif
	...
	for (dma_id = 0; dma_id < SOC_DMA_CHAN_NUM_PER_UNIT; dma_id++) {
		...
		hw->config_group[dma_id].remain_length.remain_len = 0;
		hw->config_group[dma_id].tran_length.transfer_len = 0;
		hw->config_group[dma_id].next_ll_addr = 0;
	}
}
...
static inline void dma_ll_init_without_channels(dma_hw_t *hw)
{
	if(0 == dma_ll_get_soft_reset_value(hw)) {
		...
		hw->prio_mode.v = 0;
		hw->prio_mode.soft_reset = 1;	//reset it before anyother operations
		hw->secure_attr.v = 0xFF;  
		hw->privileged_attr.v = 0xFF;  
#else
...
static inline uint32_t dma_ll_dev_to_req_mux(uint32 req_mux)
{
	switch (req_mux) {
		...
		CASE_DEV(SPDIF1);
		CASE_DEV(SADC_RX);
		CASE_DEV(AUD_SPK0_CALL);
		CASE_DEV(AUD_MIC1);
		CASE_DEV(AUD_SPK1_A2DP);
		CASE_DEV(AUD_SPK1_HINT);
		CASE_DEV(AUD_SPK1_CALL);
		CASE_DEV(GSPI3);
		CASE_DEV(GSPI3_RX);
		CASE_DEV(UART5);
		CASE_DEV(UART5_RX);
...
static inline void dma_ll_reset_config_to_default(dma_hw_t *hw, volatile dma_id_t id)
{
	...
	hw->config_group[id].remain_length.remain_len = 0;
	hw->config_group[id].tran_length.transfer_len = 0;
...
{
	hw->config_group[id].req_mux.dest_req_mux = (dma_ll_dev_to_req_mux(req_mux)) & 0x3f;
}

static inline void dma_ll_set_src_req_mux(dma_hw_t *hw, dma_id_t id, uint32_t req_mux)
{
	hw->config_group[id].req_mux.src_req_mux = (dma_ll_dev_to_req_mux(req_mux)) & 0x3f;
}
...
#define DMA_LL_TRANS_LEN_MAX (0x20000000)
static inline uint32_t dma_ll_get_transfer_len_max(dma_hw_t *hw)
{
	return (DMA_LL_TRANS_LEN_MAX - 1);
}

static inline void dma_ll_set_transfer_len(dma_hw_t *hw, dma_id_t id, uint32_t tran_len)
{
	// Note: BK7259 hardware uses direct value (0 means 0, 1 means 1), no need to subtract 1
	hw->config_group[id].tran_length.transfer_len = tran_len & 0x3fffffff;
}
...
static inline void dma_ll_set_secure_attr(dma_hw_t *hw, dma_id_t id, dma_sec_attr_t attr)
{
	if (attr == DMA_ATTR_NON_SEC) {
		hw->secure_attr.attr &= ~(BIT(id) & 0xFF);
...
{
	if (attr == DMA_ATTR_NON_SEC) {
		hw->privileged_attr.attr &= ~(BIT(id) & 0xFF);
	} else if (attr == DMA_ATTR_SEC) {
		hw->privileged_attr.attr |= (BIT(id) & 0xFF);
	}
}
 ...
 * Each channel occupies 2 bits in the register:
 *   - channel0: bits [1:0]
 *   - channel1: bits [3:2]
 *   - channel2: bits [5:4]
 *   - channel3: bits [7:6]
 ...
 *   - channel8: bits [17:16]
 * 
 * Note: Configuration must be done in secure world.
 */
static inline void dma_ll_set_int_allocate(dma_hw_t *hw, dma_id_t id, dma_int_id_t int_id)
{
	uint32_t mask = 0x3UL << (id * 2);  // 2-bit mask for this channel
	uint32_t value = ((uint32_t)int_id & 0x3UL) << (id * 2);
	
	// Clear the 2 bits for this channel and set new value
 ...
 */
static inline dma_int_id_t dma_ll_get_int_allocate(dma_hw_t *hw, dma_id_t id)
{
	uint32_t value = (hw->int_allocate.v >> (id * 2)) & 0x3UL;
	return (dma_int_id_t)value;
}
...
static inline void dma_ll_set_next_ll_addr(dma_hw_t *hw, dma_id_t id, uint32_t ll_addr)
{
	hw->config_group[id].next_ll_addr = ll_addr;
}

static inline uint32_t dma_ll_get_next_ll_addr(dma_hw_t *hw, dma_id_t id)
{
...
static inline void dma_ll_set_cachable(dma_hw_t *hw, dma_id_t id, uint32_t cachable)
{
	hw->config_group[id].req_mux.cachable = cachable & 0x1;
}

static inline uint32_t dma_ll_get_cachable(dma_hw_t *hw, dma_id_t id)
{
	return (hw->config_group[id].req_mux.cachable & 0x1);
}
