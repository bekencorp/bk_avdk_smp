...
#define CONFIG_SPI_MAX_BAUD_RATE                80000000     //80M
...
/* clk_div is 8-bit 2-divider coefficient
 * Actual SPI clock: spi_clk = src_clk / (2 * (clk_div + 1))
 * So: clk_div = (src_clk / (2 * spi_clk)) - 1
 * clk_div range: 0-255 (8-bit)
 */
bk_err_t spi_hal_set_baud_rate(spi_hal_t *hal, uint32_t baud_rate)
{
	...
	if (baud_rate <= CONFIG_XTAL_FREQ) {
		HAL_LOGD("spi select src_clk xtal (baud_rate=%d <= XTAL_FREQ=%d)\r\n", baud_rate, CONFIG_XTAL_FREQ);
		src_clk = CONFIG_XTAL_FREQ;
		sys_hal_spi_select_clock(hal->id, SPI_CLK_XTAL);
		clk_div = (src_clk / (2 * spi_clk)) - 1;
		if (clk_div > SPI_F_CLK_DIV_M) {
			clk_div = SPI_F_CLK_DIV_M;
			HAL_LOGW("clk_div overflow, clamped to 0xFF, actual baud_rate will be %d\r\n",
			         src_clk / (2 * (clk_div + 1)));
		}
		clk_div &= SPI_F_CLK_DIV_M;
	} else {
		...
		src_clk = 160000000UL;
		sys_hal_spi_select_clock(hal->id, SPI_CLK_160MHZ);
		clk_div = (src_clk / (2 * spi_clk)) - 1;
		if (clk_div > SPI_F_CLK_DIV_M) {
			clk_div = SPI_F_CLK_DIV_M;
			HAL_LOGW("clk_div overflow, clamped to 0xFF, actual baud_rate will be %d\r\n",
	...
	HAL_LOGD("baud_rate=%d, src_clk=%d, clk_div=0x%x, actual_clk=%d\r\n",
	         baud_rate, src_clk, clk_div, src_clk / (2 * (clk_div + 1)));
