...
#define TIMER_LL_REG_BASE(_group_id)                    \
	(((_group_id) == 0) ? SOC_TIMER0_REG_BASE :         \
	 ((_group_id) == 1) ? SOC_TIMER1_REG_BASE :         \
	 ((_group_id) == 2) ? SOC_TIMER2_REG_BASE :         \
	 SOC_TIMER3_REG_BASE)
...
static inline void timer_ll_init(timer_hw_t *hw, uint32_t chan)
{
	...
	timer_hw_t *unit = timer_ll_unit_hw(group);

	if (TIMER_GROUP_CHAN0 == group_chan) {
		unit->ctrl.timer0_en = 0;
		unit->ctrl.timer0_int_en = 0;
	} else if (TIMER_GROUP_CHAN1 == group_chan) {
		unit->ctrl.timer1_en = 0;
		unit->ctrl.timer1_int_en = 0;
	} else {
		unit->ctrl.timer2_en = 0;
		unit->ctrl.timer2_int_en = 0;
	}

	unit->read_ctrl.v = 0;
	unit->timer_read_value = 0;
...
static inline void timer_ll_set_enable(timer_hw_t *hw, uint32_t group, uint32_t chan, uint32_t value)
{
	...
	if (chan == TIMER_GROUP_CHAN0)
		unit->ctrl.timer0_en = value;
	else if (chan == TIMER_GROUP_CHAN1)
		unit->ctrl.timer1_en = value;
	else
		unit->ctrl.timer2_en = value;
}
...
static inline uint32_t timer_ll_get_enable_status(timer_hw_t *hw)
{
	...
	for (int group = 0; group < SOC_TIMER_GROUP_NUM; group++) {
		timer_hw_t *unit = timer_ll_unit_hw(group);
		status |= unit->ctrl.timer0_en << (bit_pos++);
		status |= unit->ctrl.timer1_en << (bit_pos++);
		status |= unit->ctrl.timer2_en << (bit_pos++);
	}
...
static inline void timer_ll_set_interrupt(timer_hw_t *hw, uint32_t group, uint32_t chan, uint32_t value)
{
	...
	if (chan == TIMER_GROUP_CHAN0)
		unit->ctrl.timer0_int_en = value;
	else if (chan == TIMER_GROUP_CHAN1)
		unit->ctrl.timer1_int_en = value;
	else
		unit->ctrl.timer2_int_en = value;
}
...
static inline uint32_t timer_ll_get_chan_interrupt_status(timer_hw_t *hw, uint32_t chan)
{
	...
	if (TIMER_GROUP_CHAN0 == group_chan)
		return unit->ctrl.timer0_int_en;
	else if (TIMER_GROUP_CHAN1 == group_chan)
		return unit->ctrl.timer1_int_en;
	else
		return unit->ctrl.timer2_int_en;
}
...
static inline uint32_t timer_ll_get_group_interrupt_status(timer_hw_t *hw, uint32_t group)
{
	...
	status |= unit->ctrl.timer0_int_en << 0;
	status |= unit->ctrl.timer1_int_en << 1;
	status |= unit->ctrl.timer2_int_en << 2;
...
static inline uint32_t timer_ll_get_interrupt_status(timer_hw_t *hw)
{
	...
	for (int group = 0; group < SOC_TIMER_GROUP_NUM; group++) {
		uint32_t group_status = timer_ll_get_group_interrupt_status(hw, group);
		status |= group_status << bit_pos;
		bit_pos += SOC_TIMER_CHAN_NUM_PER_GROUP;
	}
...
{
	timer_ll_unit_hw(TIMER_LL_GROUP(chan))->timer_cnt[TIMER_LL_CHAN(chan)] = timer_cnt;
}

static inline uint32_t timer_ll_get_end_count(timer_hw_t *hw, uint32_t chan)
{
	return timer_ll_unit_hw(TIMER_LL_GROUP(chan))->timer_cnt[TIMER_LL_CHAN(chan)];
}

static inline void timer_ll_set_clk_div(timer_hw_t *hw, uint32_t chan, uint32_t clk_div)
{
	timer_ll_unit_hw(TIMER_LL_GROUP(chan))->ctrl.clk_div = clk_div & TIMER_F_CLK_DIV_V;
}

static inline void timer_ll_set_cnt_read(timer_hw_t *hw, uint32_t chan)
{
	timer_ll_unit_hw(TIMER_LL_GROUP(chan))->read_ctrl.timer_cnt_read = 1;
}

static inline bool timer_ll_is_cnt_read_valid(timer_hw_t *hw, uint32_t chan)
{
	return !(timer_ll_unit_hw(TIMER_LL_GROUP(chan))->read_ctrl.timer_cnt_read & 0x1);
}

static inline void timer_ll_set_read_index(timer_hw_t *hw, uint32_t chan)
{
	timer_ll_unit_hw(TIMER_LL_GROUP(chan))->read_ctrl.timer_index = TIMER_LL_CHAN(chan);
}

static inline void timer_ll_clear_timer_count(timer_hw_t *hw, uint32_t chan)
{
	timer_ll_unit_hw(TIMER_LL_GROUP(chan))->timer_read_value = 0;
}

static inline uint32_t timer_ll_get_timer_count(timer_hw_t *hw, uint32_t chan)
{
	return timer_ll_unit_hw(TIMER_LL_GROUP(chan))->timer_read_value;
}
...
{
	timer_hw_t *unit = timer_ll_unit_hw(group_id);

	pm_backup[0]  = unit->global_ctrl.v;
	pm_backup[1]  = unit->timer_cnt[0];
	pm_backup[2]  = unit->timer_cnt[1];
	pm_backup[3]  = unit->timer_cnt[2];
	pm_backup[4]  = unit->ctrl.v & TIMER_CTRL_MASK;

	for (uint32_t i = 0; i < SOC_TIMER_CHAN_NUM_PER_GROUP; i++) {
...
static inline void timer_ll_restore(timer_hw_t *hw, uint32_t group_id, uint32_t *pm_backup)
{
	...
	unit->global_ctrl.v = pm_backup[0];
	unit->timer_cnt[0]  = pm_backup[1];
	unit->timer_cnt[1]  = pm_backup[2];
	unit->timer_cnt[2]  = pm_backup[3];
	unit->ctrl.v        = pm_backup[4];
}
#endif
