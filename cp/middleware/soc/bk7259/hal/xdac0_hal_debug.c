...
typedef void (*xdac0_dump_fn_t)(void);
typedef struct {
	uint32_t start;
	uint32_t end;
	xdac0_dump_fn_t fn;
...
static void xdac0_dump_reg0(void)
{
	SOC_LOGI("reg0: %8x\r\n", REG_READ(SOC_XDAC0_REG_BASE + (0x0 << 2)));
}

static void xdac0_dump_reg1(void)
{
	SOC_LOGI("reg1: %8x\r\n", REG_READ(SOC_XDAC0_REG_BASE + (0x1 << 2)));
}

static void xdac0_dump_reg2(void)
{
	xdac0_reg2_t *r = (xdac0_reg2_t *)(SOC_XDAC0_REG_BASE + (0x2 << 2));

	SOC_LOGI("reg2: %8x\r\n", REG_READ(SOC_XDAC0_REG_BASE + (0x2 << 2)));
	SOC_LOGI("	soft_reset: %8x\r\n", r->soft_reset);
	SOC_LOGI("	clkg_bypass: %8x\r\n", r->clkg_bypass);
	SOC_LOGI("	reserved_bit_2_31: %8x\r\n", r->reserved_bit_2_31);
}

static void xdac0_dump_reg3(void)
{
	SOC_LOGI("reg3: %8x\r\n", REG_READ(SOC_XDAC0_REG_BASE + (0x3 << 2)));
}

static void xdac0_dump_reg4(void)
{
	xdac0_reg4_t *r = (xdac0_reg4_t *)(SOC_XDAC0_REG_BASE + (0x4 << 2));

	SOC_LOGI("reg4: %8x\r\n", REG_READ(SOC_XDAC0_REG_BASE + (0x4 << 2)));
...
static void xdac0_dump_reg5(void)
{
	xdac0_reg5_t *r = (xdac0_reg5_t *)(SOC_XDAC0_REG_BASE + (0x5 << 2));

	SOC_LOGI("reg5: %8x\r\n", REG_READ(SOC_XDAC0_REG_BASE + (0x5 << 2)));
...
static void xdac0_dump_reg6(void)
{
	xdac0_reg6_t *r = (xdac0_reg6_t *)(SOC_XDAC0_REG_BASE + (0x6 << 2));

	SOC_LOGI("reg6: %8x\r\n", REG_READ(SOC_XDAC0_REG_BASE + (0x6 << 2)));
...
static void xdac0_dump_reg7(void)
{
	xdac0_reg7_t *r = (xdac0_reg7_t *)(SOC_XDAC0_REG_BASE + (0x7 << 2));

	SOC_LOGI("reg7: %8x\r\n", REG_READ(SOC_XDAC0_REG_BASE + (0x7 << 2)));
	SOC_LOGI("	dac_rthrd: %8x\r\n", r->dac_rthrd);
	SOC_LOGI("	dac_wthrd: %8x\r\n", r->dac_wthrd);
	SOC_LOGI("	reserved_10_31: %8x\r\n", r->reserved_10_31);
}

static void xdac0_dump_reg8(void)
{
	xdac0_reg8_t *r = (xdac0_reg8_t *)(SOC_XDAC0_REG_BASE + (0x8 << 2));

	SOC_LOGI("reg8: %8x\r\n", REG_READ(SOC_XDAC0_REG_BASE + (0x8 << 2)));
	SOC_LOGI("	tx_fifo_wr_data: %8x\r\n", r->tx_fifo_wr_data);
	SOC_LOGI("	reserved_12_31: %8x\r\n", r->reserved_12_31);
}

static xdac0_reg_fn_map_t s_fn[] =
{
	{0x0, 0x0, xdac0_dump_reg0},
	{0x1, 0x1, xdac0_dump_reg1},
	{0x2, 0x2, xdac0_dump_reg2},
	{0x3, 0x3, xdac0_dump_reg3},
	{0x4, 0x4, xdac0_dump_reg4},
	{0x5, 0x5, xdac0_dump_reg5},
	{0x6, 0x6, xdac0_dump_reg6},
	{0x7, 0x7, xdac0_dump_reg7},
	{0x8, 0x8, xdac0_dump_reg8},
	{-1, -1, 0}
};

void xdac0_struct_dump(uint32_t start, uint32_t end)
{
