...
static inline uint32_t psram_hal_get_data_base(psram_id_t psram_id)
{
...
void psram_hal_set_clk_with_id(psram_id_t psram_id, psram_clk_t clk)
{
	...
	{
		case PSRAM_240M:
			sys_drv_psram_clk_sel_with_id(psram_id, 1);	 // clk sel: 0-320 1-480
			sys_drv_psram_set_clkdiv_with_id(psram_id, 0); //frq:  F/(2 + (1+div))
			break;
		case PSRAM_160M:
			sys_drv_psram_clk_sel_with_id(psram_id, 0);	 // clk sel: 0-320 1-480
			sys_drv_psram_set_clkdiv_with_id(psram_id, 0); //frq:  F/(2 + (1+div))
			break;
		case PSRAM_120M:
			sys_drv_psram_clk_sel_with_id(psram_id, 1);	 // clk sel: 0-320 1-480
			sys_drv_psram_set_clkdiv_with_id(psram_id, 1); //frq:  F/(2 + (1+div))
			break;
		case PSRAM_80M:
			sys_drv_psram_clk_sel_with_id(psram_id, 0);	 // clk sel: 0-320 1-480
			sys_drv_psram_set_clkdiv_with_id(psram_id, 1); //frq:  F/(2 + (1+div))
...
{
    /*
     * Recommended default PSRAM clock: 240 MHz.
     */

	psram_hal_set_clk_with_id(psram_id, PSRAM_240M);
}
...
static inline bool is_psram_addr_out_of_range(psram_id_t psram_id, uint32_t addr)
{
	...
	uint32_t base = psram_hal_get_data_base(psram_id);
	return ((addr < base) || (addr > (base + SOC_PSRAM_DATA_SIZE)));
#else
	(void)psram_id;
	return false;
#endif
...
static int psram_hal_APS6408L_init_with_id(psram_id_t psram_id, uint32_t *id)
{
	...
	if (chip_id == CHIP_VERSION_C)
		psram_hal_set_mode_value_with_id(psram_id, PSRAM_MODE1);
	else
		psram_hal_set_mode_value_with_id(psram_id, PSRAM_MODE3);
#else
	psram_hal_set_mode_value_with_id(psram_id, PSRAM_MODE6);//PSRAM_MODE2
	//psram_hal_set_reg5_value_with_id(psram_id, 0x282);
	psram_hal_set_reg5_value_with_id(psram_id, 0x380);
#endif
	...
	val = psram_hal_get_regb_value_with_id(psram_id);
...
{
	uint32_t val = 0;
	psram_hal_set_mode_value_with_id(psram_id, PSRAM_MODE7);
#if (!CONFIG_SOC_BK7256XX)
...
uint32_t psram_hal_config_init_with_id(psram_id_t psram_id, uint32_t id)
{
	...
	/* set psram bypass */
	val = psram_hal_get_reg2_value_with_id(psram_id);
	val |= (0x1 << 1);
	psram_hal_set_reg2_value_with_id(psram_id, val);
...
void psram_hal_power_clk_enable(uint8_t enable)
{
	if (enable)
	{
		...
		bk_pm_clock_ctrl(CLK_PWR_ID_PSRAM0, CLK_PWR_CTRL_PWR_UP);//psram_clk_enable bit19=1
		bk_pm_clock_ctrl(CLK_PWR_ID_PSRAM1, CLK_PWR_CTRL_PWR_UP);//psram_clk_enable bit19=1
	}
	else
	{
		...
		bk_pm_clock_ctrl(CLK_PWR_ID_PSRAM0, CLK_PWR_CTRL_PWR_DOWN);//psram_clk_disable
		bk_pm_clock_ctrl(CLK_PWR_ID_PSRAM1, CLK_PWR_CTRL_PWR_DOWN);//psram_clk_disable
...
{
	psram_hal_reset_with_id(PSRAM_ID_0);
}
...
{
	psram_hal_config_with_id(PSRAM_ID_0);
}
