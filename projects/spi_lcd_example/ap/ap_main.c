...
void lcd_spi_display_fill_pure_color(frame_buffer_t *frame_buffer, uint16_t color)
{
    uint8_t data[2] = {0};
...
void cli_spi_lcd_display_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    if (os_strcmp(argv[1], "open") == 0) {
        ...
        AVDK_GOTO_VOID_ON_FALSE(ret == AVDK_ERR_OK, exit, TAG, "bk_display_spi_new failed!\n");
        ...
        uint32_t frame_len = spi_ctlr_config.lcd_device->width * spi_ctlr_config.lcd_device->height * 2;
        ...
        disp_frame->width = spi_ctlr_config.lcd_device->width;
        disp_frame->height = spi_ctlr_config.lcd_device->height;
...
#define CMDS_COUNT  (sizeof(s_spi_lcd_display_commands) / sizeof(struct cli_command))

static const struct cli_command s_spi_lcd_display_commands[] =
{
    {"spi_lcd_display", "open/close", cli_spi_lcd_display_cmd},
};

int cli_spi_lcd_display_init(void)
{
    return cli_register_commands(s_spi_lcd_display_commands, CMDS_COUNT);
}
