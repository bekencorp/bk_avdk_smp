...
`lwip_psram_example` 演示在 BK7258 SMP 平台上使用 PSRAM 作为 lwIP
动态内存。启用该功能后，lwIP 的 `mem_malloc()` 最终通过 `psram_malloc()`
分配内存，从而降低网络业务对 AP SRAM 的占用，并为大吞吐量、多连接等场景提供
更大的网络缓冲空间。
...
`CONFIG_CONTROLLER_AP_BUFFER_COPY` 是配套配置，缺少任意一项都会触发编译期配置
检查。

RAM 分区文件 `partitions/bk7258/ram_regions.csv` 中还必须提供大小非零的
`AP_PSRAM_HEAP`。本工程配置为：

```text
AP_PSRAM_HEAP,          PSRAM,           , 0x0a0000
```

即为 AP 提供 640 KB PSRAM heap。该 heap 由 lwIP 和其他调用
`psram_malloc()` 的 AP 组件共享，并非仅供 lwIP 独占。实际项目可根据并发连接
数、TCP 窗口、收发峰值以及其他 AP PSRAM 使用者调整该值。增大该区域时，需要
相应调整其他 PSRAM 分区，确保分区不重叠且总大小不超过物理 PSRAM 容量。修改
RAM 分区后建议先执行 `make clean`。
...
如果使用 CP UART0 控制台，需要通过 `ap_cmd` 将命令转发到 AP：
...
使用 CP UART0 控制台时，设备侧命令需要添加 `ap_cmd` 前缀，例如：
...
- `AP_PSRAM_HEAP` 过小可能导致内存分配失败，应按所有 AP PSRAM 使用者及网络
  业务峰值规划大小；
- PSRAM 的访问性能低于 SRAM，本配置用于降低 SRAM 压力，并不保证所有场景下
  吞吐量都会提升；
