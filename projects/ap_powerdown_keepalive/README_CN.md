...
**提示**：本工程示例中提供了一种将接收的保活相关报文直接交给 CP 侧协议栈处理的方法：CP 侧在建立保活 TCP 连接时可以将保活端口绑定到 CP 的端口范围，这样保活接收报文会直接送到 CP 协议栈处理。

## 2. 适用硬件与测试环境

* 推荐开发板：BK7259 系列（或兼容平台）
...
```
make bk7259 PROJECT=ap_powerdown_keepalive
```
