...
static void tflm_demo_task_entry(void *arg)
{
    ...
    LOGI("===========>start hand_gesture_detection demo\r\n");
    while(1) {
        bk_err_t ret = tflm_hand_gesture_detection_run_demo();
        if (ret != BK_OK) {
            LOGE("hand_gesture_detection demo failed, ret=%d\r\n", ret);
        } else {
            LOGI("hand_gesture_detection round done\r\n");
        }
