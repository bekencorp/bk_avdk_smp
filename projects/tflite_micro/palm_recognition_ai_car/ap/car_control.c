...
static bk_err_t car_control_send_internal(const uint8_t *data, uint32_t len)
{
    if (!s_uart_inited)
    {
...
{
    return car_control_send_internal(data, len);
}
...
bk_err_t car_control_init(const car_control_config_t *config)
{
    ...
    gpio_dev_map(GPIO_35, GPIO_DEV_UART1_TXD);
    gpio_dev_map(GPIO_34, GPIO_DEV_UART1_RXD);
    bk_gpio_pull_up(GPIO_35);
    bk_gpio_pull_up(GPIO_34);
...
{
    return car_control_send_internal(CAR_CMD_MOVE_LEFT, CAR_CMD_MOVE_LEFT_LEN);
}

bk_err_t car_control_move_right(void)
{
    return car_control_send_internal(CAR_CMD_MOVE_RIGHT, CAR_CMD_MOVE_RIGHT_LEN);
}

bk_err_t car_control_move_forward(void)
{
    return car_control_send_internal(CAR_CMD_MOVE_FORWARD, CAR_CMD_MOVE_FORWARD_LEN);
}

bk_err_t car_control_move_backward(void)
{
    return car_control_send_internal(CAR_CMD_MOVE_BACKWARD, CAR_CMD_MOVE_BACKWARD_LEN);
}

bk_err_t car_control_gimbal_up(void)
{
    return car_control_send_internal(CAR_CMD_GIMBAL_UP, CAR_CMD_GIMBAL_UP_LEN);
}

bk_err_t car_control_gimbal_down(void)
{
    return car_control_send_internal(CAR_CMD_GIMBAL_DOWN, CAR_CMD_GIMBAL_DOWN_LEN);
}
