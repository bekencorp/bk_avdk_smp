# LVGL Widgets V9 示例工程说明

* [English](./README.md)

## 1. 工程简介

`widgets_v9` 是 BK7259 SDK 中的 LVGL 官方 Widgets Demo 参考工程，使用 **LVGL V9.3.0**，用于演示按钮、滑条、列表、图表、图片、字体等常见 UI 控件。

简单理解：

1. **它是 LVGL V9 的上手工程**，适合验证 LVGL V9 在 BK7259 上的显示和触摸流程。
2. **它跑通了 MIPI DSI 屏、DPU、frame buffer、LVGL vendor 适配层和触摸输入**。
3. **它适合作为新 UI 项目的参考模板**：保留底层初始化流程，把官方 Demo 页面换成自己的页面即可。
...
> 如果你的屏幕型号、分辨率或 GPIO 与默认配置不同，需要先适配显示硬件，再调 LVGL UI。否则即使 LVGL 正常启动，也可能因为 LCD 配置不匹配而黑屏。
...
│   ├── lv_demo_widgets.c          # LVGL V9 Widgets Demo UI 代码
│   ├── lv_demo_widgets.h
...
2. `ap/ap_main.c`：看显示、触摸、LVGL 如何启动。
3. `ap/lv_demo_widgets.c`：看 LVGL V9 官方 Widgets Demo UI。
...
| 仪表/指针图片 | `widgets_v9` 额外编译了 `img_demo_widgets_needle.c` |
...
如果你使用项目内已有烧录脚本或自动化工具，请选择 `lvgl/widgets_v9` 这个工程。
...
7. 按 `output_compress` 计算 frame buffer 大小
8. 申请 LVGL frame buffer
9. 调用 `lv_vendor_init()` 初始化 LVGL 适配层
10. 如开启触摸，调用 `drv_tp_open()`
11. 加锁后调用 `lv_demo_widgets()` 创建 UI
12. 调用 `lv_vendor_start()` 启动 LVGL 线程
...
| `CONFIG_LVGL_USE_GPU_ROTATE=y` | 启用 LVGL GPU 旋转相关支持 |
...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `dpu_config.video.format` | `BK_PIXEL_FORMAT_ARGB8888` | DPU 视频层像素格式 |
| `dpu_config.video.decompress` | `true` | DPU 侧开启解压 |
| `lv_vnd_config.output_compress` | `true` | LVGL 输出使用压缩路径 |
| `draw_pixel_size` | `WIDTH * 64 * sizeof(bk_color_t)` | partial mode 下绘制 buffer 大小 |

这部分是 `widgets_v9` 和 `widgets_v8` 的主要差异之一。新手如果只是改 UI，通常不需要动这些参数；如果要改显示格式、压缩策略或性能调优，再重点看这一段代码。
...
`widgets_v9` 当前已编译这些图片资源：

| 资源文件 | 说明 |
| --- | --- |
| `assets/img_clothes.c` | 衣服图片资源 |
| `assets/img_demo_widgets_avatar.c` | 头像图片资源 |
| `assets/img_demo_widgets_needle.c` | 指针图片资源 |
| `assets/img_lvgl_logo.c` | LVGL logo |

## 8. 常见问题

### 8.1 编译失败，提示找不到 LVGL Demo、字体或图片
...
4. V8/V9 Demo 代码不要混用，`widgets_v9` 应使用 LVGL V9 对应的 API 和 Demo 文件。
...
### 8.5 从 LVGL V8 迁移到 V9 要注意什么

LVGL V9 相比 V8 有 API 和对象模型差异。迁移时建议：

1. 不要直接把 V8 的 UI 文件复制到 V9 工程后强行编译。
2. 先用 `widgets_v9` 跑通显示和触摸。
3. 按 LVGL V9 API 逐个页面迁移。
4. 每迁移一个页面就编译、烧录验证一次。
...
| DPU 解压 | 默认关闭 | 默认开启 |
| 额外资源 | 无 `img_demo_widgets_needle.c` | 包含 `img_demo_widgets_needle.c` |
| 适合场景 | 维护或验证基于 LVGL V8 的 UI | 新项目优先参考 LVGL V9 |

如果是新项目且没有历史包袱，建议优先参考本工程；如果已有 UI 基于 LVGL V8 开发，则参考 `widgets_v8` 更合适。
