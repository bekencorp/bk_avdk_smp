...
`widgets_v8` is an LVGL official Widgets Demo project for BK7259. It uses **LVGL V8.3.9** and demonstrates common UI widgets such as buttons, sliders, lists, charts, images, and fonts.

In short:

1. It is not a complete product application. It is a reference demo for LVGL widgets.
2. It verifies the full path of LVGL + MIPI LCD + touch + display flush on BK7259.
3. It can be used as a starting point for a new UI project. Bring up the LCD and touch first, then replace the demo UI with your own pages.
...
If your board uses a different LCD, resolution, or GPIO assignment, adapt the display hardware first. Otherwise LVGL may start correctly but the panel can still stay black or show abnormal colors.
...
│   ├── lv_demo_widgets.c          # LVGL V8 Widgets Demo UI
│   ├── lv_demo_widgets.h
...
2. `ap/ap_main.c`: understand display, LVGL, and touch initialization.
3. `ap/lv_demo_widgets.c`: inspect the official Widgets Demo UI.
...
> Note: the current `widgets_v8/CMakeLists.txt` uses `project(widgets_v9)`, so some build logs or output directories may contain `widgets_v9`. The actual project entry is still `PROJECT=lvgl/widgets_v8`.
...
7. Allocate LVGL frame buffers.
8. Initialize the LVGL vendor layer with `lv_vendor_init()`.
9. Open touch with `drv_tp_open()` if `CONFIG_TP` is enabled.
10. Call `lv_demo_widgets()` inside `lv_vendor_disp_lock()` and `lv_vendor_disp_unlock()`.
11. Start the LVGL task with `lv_vendor_start()`.
...
| `CONFIG_LVGL_V8=y` | Select LVGL V8 |
...
For new projects without legacy LVGL V8 code, prefer `widgets_v9`. For existing V8 UI code, use this project as the reference.
