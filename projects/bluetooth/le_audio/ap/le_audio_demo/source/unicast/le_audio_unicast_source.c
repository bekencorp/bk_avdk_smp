#include "le_audio_protocol_demo.h"
#include "le_audio_user_config.h"

#include <components/bluetooth/bk_dm_bap.h>
#include <components/log.h>
#include <modules/lc3_codec.h>
#include <os/mem.h>
#include <os/os.h>

#define TAG "lea_unicast"
#define LE_AUDIO_UNICAST_FEED_PRIORITY 5
#define LE_AUDIO_UNICAST_ENCODE_PRIORITY 4
#define LE_AUDIO_UNICAST_FEED_STACK 6144
#define LE_AUDIO_UNICAST_ENCODE_STACK 6144
#define LE_AUDIO_UNICAST_FRAME_BYTES 100
#define LE_AUDIO_UNICAST_FRAME_QUEUE_COUNT 3
#define LE_AUDIO_UNICAST_SEND_FAIL_LIMIT 10

extern int lc3_frame_samples(int dt_us, int sr_hz);

static lc3_enc_info_t s_enc_info;
static uint8_t *s_enc_mem;
static int16_t *s_pcm_in;
static beken_queue_t s_frame_queue;
static beken_thread_t s_encode_thread;
static beken_thread_t s_send_thread;
static beken_semaphore_t s_encode_exit_sem;
static beken_semaphore_t s_send_exit_sem;
static volatile uint8_t s_feed_run;
static uint16_t s_feed_handle;
static uint32_t s_tone_phase;

typedef struct
{
	uint16_t seq;
	uint16_t len;
	uint8_t data[LE_AUDIO_UNICAST_FRAME_BYTES];
} le_audio_unicast_frame_t;

static void le_audio_unicast_fill_fake_pcm(int16_t *pcm, int samples)
{
	const uint32_t period = 100;

	for (int i = 0; i < samples; i++)
	{
		uint32_t ph = s_tone_phase % period;
		int32_t v;

		if (ph < period / 2)
		{
			v = (int32_t)ph - (int32_t)(period / 4);
		}
		else
		{
			v = (int32_t)(period - ph) - (int32_t)(period / 4);
		}

		pcm[i] = (int16_t)(v * 400);
		s_tone_phase++;
	}
}

static void le_audio_unicast_tone_deinit(void)
{
	if (s_enc_mem)
	{
		os_free(s_enc_mem);
		s_enc_mem = NULL;
	}

	if (s_pcm_in)
	{
		os_free(s_pcm_in);
		s_pcm_in = NULL;
	}

	if (s_frame_queue)
	{
		rtos_deinit_queue(&s_frame_queue);
		s_frame_queue = NULL;
	}

	if (s_encode_exit_sem)
	{
		rtos_deinit_semaphore(&s_encode_exit_sem);
		s_encode_exit_sem = NULL;
	}

	if (s_send_exit_sem)
	{
		rtos_deinit_semaphore(&s_send_exit_sem);
		s_send_exit_sem = NULL;
	}

	os_memset(&s_enc_info, 0, sizeof(s_enc_info));
}

static void le_audio_unicast_wait_thread_exit(beken_thread_t *thread, beken_semaphore_t *sem, uint32_t timeout_ms)
{
	if (!thread || !(*thread))
	{
		return;
	}

	if (sem && *sem)
	{
		rtos_get_semaphore(sem, timeout_ms);
	}
	else
	{
		while (*thread)
		{
			rtos_delay_milliseconds(10);
		}
	}
}

static int le_audio_unicast_tone_init(void)
{
	int dt = CONFIG_LE_AUDIO_FRAME_US;
	int sr = CONFIG_LE_AUDIO_SAMPLE_RATE;
	int frame_samples = lc3_frame_samples(dt, sr);
	unsigned enc_size;

	if (frame_samples <= 0)
	{
		BK_LOGE(TAG, "bad lc3 params dt=%d sr=%d\n", dt, sr);
		return -1;
	}

	os_memset(&s_enc_info, 0, sizeof(s_enc_info));
	s_enc_info.channel = 1;
	s_enc_info.frame_us = dt;
	s_enc_info.enc_srate_hz = sr;
	s_enc_info.pcm_sbytes = sizeof(int16_t);
	s_enc_info.bitrate = LE_AUDIO_UNICAST_FRAME_BYTES * 8 * (1000000 / dt);

	enc_size = (unsigned)frame_samples * sizeof(float) * 4 + 1024;
	s_enc_mem = (uint8_t *)os_malloc(enc_size);
	s_pcm_in = (int16_t *)os_malloc(frame_samples * sizeof(int16_t));

	if (!s_enc_mem || !s_pcm_in)
	{
		BK_LOGE(TAG, "encoder alloc failed mem=%p pcm=%p\n", s_enc_mem, s_pcm_in);
		le_audio_unicast_tone_deinit();
		return -1;
	}

	if (rtos_init_queue(&s_frame_queue,
	                    "lea_cis_frame_q",
	                    sizeof(le_audio_unicast_frame_t),
	                    LE_AUDIO_UNICAST_FRAME_QUEUE_COUNT) != BK_OK)
	{
		BK_LOGE(TAG, "frame queue init failed\n");
		le_audio_unicast_tone_deinit();
		return -1;
	}

	if (rtos_init_semaphore_ex(&s_encode_exit_sem, 1, 0) != BK_OK)
	{
		BK_LOGE(TAG, "encode exit sem init failed\n");
		le_audio_unicast_tone_deinit();
		return -1;
	}

	if (rtos_init_semaphore_ex(&s_send_exit_sem, 1, 0) != BK_OK)
	{
		BK_LOGE(TAG, "send exit sem init failed\n");
		le_audio_unicast_tone_deinit();
		return -1;
	}

	lc3_encoder_init(&s_enc_info, s_enc_mem);
	BK_LOGI(TAG, "lc3 encoder ready dt=%dus sr=%dHz samples=%d frame_bytes=%d queue=%d\n",
	        dt, sr, s_enc_info.frame_samples, s_enc_info.frame_bytes,
	        LE_AUDIO_UNICAST_FRAME_QUEUE_COUNT);
	return 0;
}

static void le_audio_unicast_encode_thread(void *arg)
{
	uint16_t seq = 0;

	(void)arg;

	while (s_feed_run)
	{
		le_audio_unicast_frame_t frame;

		os_memset(&frame, 0, sizeof(frame));
		le_audio_unicast_fill_fake_pcm(s_pcm_in, s_enc_info.frame_samples);
		lc3_encoder_proc(&s_enc_info, (uint8_t *)s_pcm_in, frame.data);

		frame.seq = seq;
		frame.len = (uint16_t)s_enc_info.frame_bytes;
		if (rtos_push_to_queue(&s_frame_queue, &frame, 20) == BK_OK)
		{
			seq++;
		}
	}

	BK_LOGI(TAG, "unicast encode exit seq=%u\n", seq);
	if (s_encode_exit_sem)
	{
		rtos_set_semaphore(&s_encode_exit_sem);
	}
	s_encode_thread = NULL;
	rtos_delete_thread(NULL);
}

static void le_audio_unicast_send_thread(void *arg)
{
	uint32_t delay_ms = CONFIG_LE_AUDIO_FRAME_US / 1000;
	uint32_t next_send_ms;
	uint32_t send_fail_count = 0;
	uint32_t underrun_count = 0;
	uint32_t late_count = 0;

	(void)arg;

	if (delay_ms == 0)
	{
		delay_ms = 10;
	}

	BK_LOGI(TAG, "unicast tone start handle=0x%04x\n", s_feed_handle);
	rtos_delay_milliseconds(delay_ms * (LE_AUDIO_UNICAST_FRAME_QUEUE_COUNT - 1));
	next_send_ms = rtos_get_time();

	while (s_feed_run)
	{
		bk_err_t ret;
		le_audio_unicast_frame_t frame;

		if (rtos_pop_from_queue(&s_frame_queue, &frame, delay_ms) != BK_OK)
		{
			underrun_count++;
			if ((underrun_count == 1U) || ((underrun_count % 100U) == 0U))
			{
				BK_LOGW(TAG, "unicast frame queue underrun #%lu\n", underrun_count);
			}
			continue;
		}

		if (!s_feed_run)
		{
			break;
		}

		ret = bk_dm_bap_unicast_send(s_feed_handle, 0, 0, frame.seq, frame.data, frame.len);
		if (ret != BK_OK)
		{
			send_fail_count++;
			if ((send_fail_count == 1U) || (send_fail_count >= LE_AUDIO_UNICAST_SEND_FAIL_LIMIT))
			{
				BK_LOGW(TAG, "unicast send failed ret=0x%x count=%lu handle=0x%04x seq=%u\n",
				        ret, send_fail_count, s_feed_handle, frame.seq);
			}

			if (send_fail_count >= LE_AUDIO_UNICAST_SEND_FAIL_LIMIT)
			{
				BK_LOGW(TAG, "stop unicast tone after consecutive send failures\n");
				s_feed_run = 0;
				break;
			}
		}
		else
		{
			send_fail_count = 0;
		}

		next_send_ms += delay_ms;
		{
			uint32_t now_ms = rtos_get_time();

			if ((int32_t)(next_send_ms - now_ms) > 0)
			{
				rtos_delay_milliseconds(next_send_ms - now_ms);
			}
			else
			{
				late_count++;
				if ((late_count == 1U) || ((late_count % 100U) == 0U))
				{
					BK_LOGW(TAG, "unicast send late #%lu lag=%ldms\n",
					        late_count, (int32_t)(now_ms - next_send_ms));
				}
				next_send_ms = now_ms;
			}
		}
	}

	le_audio_unicast_wait_thread_exit(&s_encode_thread, &s_encode_exit_sem, 1000);

	BK_LOGI(TAG, "unicast tone exit\n");
	if (s_send_exit_sem)
	{
		rtos_set_semaphore(&s_send_exit_sem);
	}
	s_send_thread = NULL;
	rtos_delete_thread(NULL);
}

int le_audio_unicast_tone_start(uint16_t connection_handle)
{
	if (s_feed_run)
	{
		return 0;
	}

	le_audio_unicast_wait_thread_exit(&s_send_thread, &s_send_exit_sem, 2000);
	le_audio_unicast_wait_thread_exit(&s_encode_thread, &s_encode_exit_sem, 1000);

	if (s_enc_mem || s_pcm_in || s_frame_queue || s_encode_exit_sem || s_send_exit_sem)
	{
		le_audio_unicast_tone_deinit();
	}

	if (le_audio_unicast_tone_init() != 0)
	{
		return -1;
	}

	s_feed_handle = connection_handle;
	s_feed_run = 1;

	if (rtos_create_thread(&s_encode_thread,
	                       LE_AUDIO_UNICAST_ENCODE_PRIORITY,
	                       "lea_cis_enc",
	                       le_audio_unicast_encode_thread,
	                       LE_AUDIO_UNICAST_ENCODE_STACK,
	                       NULL) != 0)
	{
		BK_LOGE(TAG, "create unicast encode thread failed\n");
		s_feed_run = 0;
		le_audio_unicast_tone_deinit();
		return -1;
	}

	if (rtos_create_thread(&s_send_thread,
	                       LE_AUDIO_UNICAST_FEED_PRIORITY,
	                       "lea_cis_send",
	                       le_audio_unicast_send_thread,
	                       LE_AUDIO_UNICAST_FEED_STACK,
	                       NULL) != 0)
	{
		BK_LOGE(TAG, "create unicast send thread failed\n");
		s_feed_run = 0;
		le_audio_unicast_wait_thread_exit(&s_encode_thread, &s_encode_exit_sem, 1000);
		le_audio_unicast_tone_deinit();
		return -1;
	}

	return 0;
}

int le_audio_unicast_tone_stop(void)
{
	if (!s_feed_run && !s_encode_thread && !s_send_thread &&
	    !s_enc_mem && !s_pcm_in && !s_frame_queue && !s_encode_exit_sem && !s_send_exit_sem)
	{
		return 0;
	}

	s_feed_run = 0;
	le_audio_unicast_wait_thread_exit(&s_send_thread, &s_send_exit_sem, 2000);
	le_audio_unicast_wait_thread_exit(&s_encode_thread, &s_encode_exit_sem, 1000);
	le_audio_unicast_tone_deinit();

	return 0;
}
