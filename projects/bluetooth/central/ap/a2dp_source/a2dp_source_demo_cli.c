...
#define CONFIG_A2DP_SOURCE_DEMO_AVRCP 1
...
static void cmd_avrcp(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    else if (os_strcmp(argv[1], "abs_vol") == 0)
    {
        uint32_t vol = 0;

        if (argc >= 3)
        {
            ret = sscanf(argv[2], "%u", &vol);
            ...
            ret = bt_avrcp_demo_set_remote_abs_vol(vol);
...
static void cmd_a2dp_player_demo(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    else if (os_strcmp(argv[1], "discover") == 0)
    {
        ...
        if (argc >= 3)
        {
            ...
            if (argc >= 4)
            {
                ret = sscanf(argv[3], "%u", &num_report);
...
#endif
    else if (os_strcmp(argv[1], "test_performance") == 0)
    {
        uint32_t bytes = 1024 * 10;
        uint32_t cpu_fre = 0;
        uint32_t loop = 100;
        uint32_t cpu_id = 0;
        ...
        bt_a2dp_source_demo_test_performance(cpu_fre, bytes, loop, cpu_id);
    }
    else
    {
        goto __usage;
...
#if CONFIG_A2DP_SOURCE_DEMO_AVRCP
    //{"a2dp_player_avrcp", "see -h", cmd_avrcp},
#endif
