...
static void avrcp_ct_recv_noti_cb(uint8_t event_id, bk_avrcp_rn_param_t *event_parameter)
{
    ...
    bk_bt_avrcp_ct_send_register_notification_cmd(s_avrcp_env.peer_addr, event_id, 0);
}
...
static void avrcp_ct_cb(bk_avrcp_ct_cb_event_t event, bk_avrcp_ct_cb_param_t *param)
{
    ...
    case BK_AVRCP_CT_PASSTHROUGH_RSP_EVT:
    {
        ...
        avrcp_logi("AVRCP psth rsp 0x%x op 0x%x release %d tl %d %02x:%02x:%02x:%02x:%02x:%02x",
                   pm->rsp_code,
                   pm->key_code,
                   pm->key_state,
                   pm->tl,
    ...
    {
        avrcp_logi("AVRCP peer supported notification events 0x%x %02x:%02x:%02x:%02x:%02x:%02x", avrcp->get_rn_caps_rsp.evt_set.bits,
        ...
        {
            bk_bt_avrcp_ct_send_register_notification_cmd(s_avrcp_env.peer_addr, BK_AVRCP_RN_VOLUME_CHANGE, 0);
        }

        if (s_avrcp_env.ct_peer_cap & (0x01 << BK_AVRCP_RN_BATTERY_STATUS_CHANGE))
        {
            bk_bt_avrcp_ct_send_register_notification_cmd(s_avrcp_env.peer_addr, BK_AVRCP_RN_BATTERY_STATUS_CHANGE, 0);
        }
    }
    break;
    ...
    {
        avrcp_logi("AVRCP event notification: 0x%x %02x:%02x:%02x:%02x:%02x:%02x", avrcp->change_ntf.event_id,
    ...
    {
        if (avrcp->set_volume_rsp.status == 0)
        {
            avrcp_logi("set abs vol success %d %02x:%02x:%02x:%02x:%02x:%02x", avrcp->set_volume_rsp.volume,
                       avrcp->set_volume_rsp.remote_bda[5],
                       avrcp->set_volume_rsp.remote_bda[4],
                       avrcp->set_volume_rsp.remote_bda[3],
                       avrcp->set_volume_rsp.remote_bda[2],
                       avrcp->set_volume_rsp.remote_bda[1],
                       avrcp->set_volume_rsp.remote_bda[0]);

        }
        else
        {
            avrcp_loge("set abs vol fail, maybe remote unsupport");
        }
    }
    break;
    ...
    case BK_AVRCP_CT_SET_PLAYER_APP_SETTING_RSP_EVT:
        if (avrcp->set_player_app_setting_rsp.status == 0)
        {
            avrcp_logi("set player app settings success");
        }
        else
        {
            avrcp_loge("set player app settings fail, maybe remote unsupport");
        }

        break;

    case BK_AVRCP_CT_GET_ELEM_ATTR_RSP_EVT:
        if (avrcp->elem_attr_rsp.status == 0)
        {
            avrcp_logi("get elem attr success count %d", avrcp->elem_attr_rsp.attr_count);

            for (int i = 0; i < avrcp->elem_attr_rsp.attr_count; ++i)
            {
                avrcp_logi("attr id 0x%x attr charset 0x%x attr len %d",
                           avrcp->elem_attr_rsp.attr_array[i].attr_id,
                           avrcp->elem_attr_rsp.attr_array[i].attr_text_charset,
                           avrcp->elem_attr_rsp.attr_array[i].attr_length);
                //avrcp_logi("%s", avrcp->elem_attr_rsp.attr_array[i].attr_text);
            }
        }
        else
        {
            avrcp_loge("get elem attr fail, maybe remote unsupport");
        }
...
static void avrcp_tg_cb(bk_avrcp_tg_cb_event_t event, bk_avrcp_tg_cb_param_t *param)
{
    ...
    {
        avrcp_logd("recv pt key 0x%02x press status %d %02x:%02x:%02x:%02x:%02x:%02x", param->psth_cmd.key_code, param->psth_cmd.key_code,
                   param->psth_cmd.remote_bda[5],
                   param->psth_cmd.remote_bda[4],
                   param->psth_cmd.remote_bda[3],
                   param->psth_cmd.remote_bda[2],
                   param->psth_cmd.remote_bda[1],
                   param->psth_cmd.remote_bda[0]);
    ...
    //    {
    //        avrcp_logw("recv abs vol %d, notice if local is central/master/a2dp source will not recv this msg !!!", param->set_abs_vol.volume);
    //    }
    //    break;

    case BK_AVRCP_TG_REGISTER_NOTIFICATION_EVT:
    {
        avrcp_logi("recv reg evt 0x%x param %d %02x:%02x:%02x:%02x:%02x:%02x\n", param->reg_ntf.event_id, param->reg_ntf.event_parameter,
        ...
        case BK_AVRCP_RN_PLAY_POS_CHANGED:
            ...
            avrcp_pos_report_start_timer(avrcp_pos_report_timer_hdl, param->reg_ntf.event_parameter * 1000, NULL);
...
int bt_avrcp_demo_set_remote_abs_vol(uint32_t vol)
{
    ...
    return bk_bt_avrcp_ct_send_absolute_volume_cmd(s_avrcp_env.peer_addr, (uint8_t)vol);
}
...
{
    int ret = 0;
    bk_avrcp_rn_evt_cap_mask_t support_cap = {0};
    bk_avrcp_rn_evt_cap_mask_t final_cap = {0};
    const uint8_t try_set_cap[] =
    {
    ...
    bk_avrcp_psth_bit_mask_t support_ps_filter;
    bk_avrcp_psth_bit_mask_t final_ps_filter;
    const uint8_t try_set_ps_filter[] =
    {
    ...
    os_memset(&support_cap, 0, sizeof(support_cap));
    os_memset(&final_cap, 0, sizeof(final_cap));
    os_memset(&support_ps_filter, 0, sizeof(support_ps_filter));
    os_memset(&final_ps_filter, 0, sizeof(final_ps_filter));

    bk_bt_avrcp_tg_init();
    bk_bt_avrcp_ct_init();

    bk_bt_avrcp_tg_get_rn_evt_cap(BK_AVRCP_RN_CAP_API_METHOD_ALLOWED, &support_cap);

    for (int i = 0; i < sizeof(try_set_cap) / sizeof(try_set_cap[0]); ++i)
    {
        if (bk_bt_avrcp_rn_evt_bit_mask_operation(BK_AVRCP_BIT_MASK_OP_TEST, &support_cap, try_set_cap[i]))
        {
            bk_bt_avrcp_rn_evt_bit_mask_operation(BK_AVRCP_BIT_MASK_OP_SET, &final_cap, try_set_cap[i]);
        }
    }
    ...
    bk_bt_avrcp_tg_get_psth_cmd_filter(BK_AVRCP_PSTH_FILTER_METHOD_ALLOWED, &support_ps_filter);

    for (int i = 0; i < sizeof(try_set_ps_filter) / sizeof(try_set_ps_filter[0]); ++i)
    {
        if (bk_bt_avrcp_psth_bit_mask_operation(BK_AVRCP_BIT_MASK_OP_TEST, &support_ps_filter, try_set_ps_filter[i]))
        {
            bk_bt_avrcp_psth_bit_mask_operation(BK_AVRCP_BIT_MASK_OP_SET, &final_ps_filter, try_set_ps_filter[i]);
        }
    }

    ret = bk_bt_avrcp_tg_set_psth_cmd_filter(BK_AVRCP_PSTH_FILTER_METHOD_CURRENT_ENABLE, &final_ps_filter);

    if (ret)
    {
        avrcp_loge("set psth filter err %d", ret);
        return -1;
    }
