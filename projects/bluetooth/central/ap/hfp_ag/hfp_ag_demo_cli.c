 ...
 *
 * CLI commands for the HFP AG (Audio Gateway) demo. The demo logic lives in
 * hfp_ag_demo.c; this file only parses the `hfp_ag` console command and
 * dispatches to the demo helpers.
 */
...
static void hfp_ag_usage(void)
{
    CLI_LOGI("HFP AG demo:\n"
            ...
            //  "  hfp_ag disconnect\n"
             ...
             "  hfp_ag vgs <0-15> | vgm <0-15>   (set speaker/mic volume)\n"
             "  hfp_ag cmd <at-result-code>\n"
             //"  hfp_ag role master|slave   (switch BT role on the current link)\n");
             );
}
...
static void cmd_hfp_ag(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    // else if (os_strcmp(argv[1], "role") == 0)
    // {
    //     if (argc < 3)
    //     {
    //         goto __usage;
    ...
    //     hfp_ag_demo_switch_role(os_strcmp(argv[2], "master") == 0 ? 1 : 0);
    // }
    else
    {
        goto __usage;
