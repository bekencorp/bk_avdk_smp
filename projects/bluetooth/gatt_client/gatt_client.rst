...
--------------------------
    ...
    This project demonstrates Bluetooth GATT client operation on BK7259 SMP:

    -  Configure BLE scan, start/stop scan, and connect to an advertising peripheral.
    -  Automatic GATT service discovery after connection.
    -  ATT read/write, notify (CCC), manual discovery, bonding, and white list via ``ap_cmd ble_gattc``.
...
----------------------------
    ...
    Flash build/bk7259/gatt_client/package/all-app.bin to the board by the tool BKFIL.
...
----------------------------------
    ...
    On BK7259 SMP, enter commands on UART0 with the ``ap_cmd`` prefix. The registered
    command name is ``ble_gattc``; without ``ap_cmd`` the shell returns
    ``cmd NOT found: ble_gattc``.

    | ``ap_cmd ble_gattc help`` : list all commands.
    | ``ap_cmd ble_gattc scan 1`` : start scan (results in ``BLE_5_REPORT_ADV`` logs).
    | ``ap_cmd ble_gattc scan 0`` : stop scan.
    | ``ap_cmd ble_gattc conn xx:xx:xx:xx:xx:xx [addr_type]`` : connect (``addr_type`` 0=public, 1=random, default 0).
    | ``ap_cmd ble_gattc disconn`` : disconnect.
    | ``ap_cmd ble_gattc read [val_handle]`` : ATT read (handle hex, no ``0x`` prefix).
    | ``ap_cmd ble_gattc write [val_handle] [data]`` : ATT write (ASCII string).
    | ``ap_cmd ble_gattc notifyindcate_en 1 [desc_handle]`` : enable notify CCC (``1`` = notify).
    | ``ap_cmd ble_gattc notifyindcate_en 0 [desc_handle]`` : disable notify CCC.
    | ``ap_cmd ble_gattc read_ext [handle] [offset]`` : GATT read with offset.
    | ``ap_cmd ble_gattc read_by_uuid [sh] [eh] [uuid16]`` : read by 16-bit UUID in handle range.
    | ``ap_cmd ble_gattc write_ext [handle] [len] [is_cmd]`` : write bytes ``0..len-1`` (``is_cmd`` 1 = write command).
    ...
    | ``ap_cmd ble_gattc bond`` : start bonding on current connection.
    | ``ap_cmd ble_gattc add_whl [addr] [type]`` / ``rmv_whl`` / ``clear_whl`` : white list.
    | ``ap_cmd ble_gattc scan_filter 0|1`` : recreate scan (0=all devices, 1=whitelist only).

8.0 Test with gatt_server (BK7259)
***********************************

    - Server board: ``projects/bluetooth/gatt_server``, CLI ``ap_cmd ble_gatts`` (also requires ``ap_cmd``).
    - Server BLE MAC = flash base MAC with **last byte + 1**; use ``adv_addr`` from client scan for ``conn``.
    - After connect, wait for ``APPC_SERVICE_CONNECTED`` and handle list (service ``0xFA00``, chars ``0xEA01``–``0xEA07``).
    - Typical handles (match your log if different): CCC ``0x13``, notify value ``0x12``, RW ``0x17``/``0x19``, device name ``0x03``.
    - Notify test: client ``ap_cmd ble_gattc notifyindcate_en 1 13``, then server ``ap_cmd ble_gatts notify``.
    - After disconnect, server: ``ap_cmd ble_gatts adv_en 1`` before reconnecting.
...
- ap_cmd ble_gattc scan 1
    Scan parameters are created in ``gatt_client_demo_init``; results appear in ``BLE_5_REPORT_ADV``.

::

    [11:02:21.684]OUT→◇ap_cmd ble_gattc scan 1
    [11:02:21.689]IN←◆ap_cmd ble_gattc scan 1
...
- ap_cmd ble_gattc conn xx:xx:xx:xx:xx:xx
    Use another board running ``gatt_server`` (BK7259). Connect using the peer ``adv_addr`` from scan.
...
::
    ...
    [11:07:38.977]OUT→◇ap_cmd ble_gattc conn c8:47:8c:5f:cc:7f
    [11:07:38.983]IN←◆ap_cmd ble_gattc conn c8:47:8c:5f:cc:7f
    ...
    BLE-GATT:I(1599398):==>Get GATT Characteristic UUID:0xEA07, cha_handle:0x1A, val_handle:0x1B, property:0x0a
...
::
    ...
    [11:20:15.902]OUT→◇ap_cmd ble_gattc disconn
    [11:20:15.907]IN←◆ap_cmd ble_gattc disconn
...
- ap_cmd ble_gattc notifyindcate_en 1 [desc_handle]
...
::
    ...
    [11:23:49.409]OUT→◇ap_cmd ble_gattc notifyindcate_en 1 13
    [11:23:49.416]IN←◆ap_cmd ble_gattc notifyindcate_en 1 13
...
- ap_cmd ble_gattc notifyindcate_en 0 [desc_handle]

::

    [11:30:35.366]OUT→◇ap_cmd ble_gattc notifyindcate_en 0 13
    [11:30:35.372]IN←◆ap_cmd ble_gattc notifyindcate_en 0 13
...
- ap_cmd ble_gattc write [val_handle] [data]
...
::
    ...
    [11:31:39.560]OUT→◇ap_cmd ble_gattc write 17 ssid_ab
    [11:31:39.565]IN←◆ap_cmd ble_gattc write 17 ssid_ab
...
- ap_cmd ble_gattc read [val_handle]:

::

    [11:35:48.307]OUT→◇ap_cmd ble_gattc read 17
    [11:35:48.312]IN←◆ap_cmd ble_gattc read 17
...
::
    ...
    [11:38:26.991]OUT→◇ap_cmd ble_gatts notify
    [11:38:26.997]IN←◆ap_cmd ble_gatts notify
    os:I(1850370):create shell_handle, tcb=28066d28, stack=[28072980-28073d80:5120], prio=5

    BLE GATTS RSP:OK

    $
    [11:38:51.815]OUT→◇ap_cmd ble_gatts notify
    [11:38:51.819]IN←◆ap_cmd ble_gatts notify
    os:I(1875194):create shell_handle, tcb=28066d28, stack=[28072980-28073d80:5120], prio=5

    BLE GATTS RSP:OK

    $
    [11:38:52.331]OUT→◇ap_cmd ble_gatts notify
    [11:38:52.336]IN←◆ap_cmd ble_gatts notify
...
----------------------------------------
    The source code is in ``projects/bluetooth/gatt_client/ap/gatt_client_demo.c`` (AP main).
