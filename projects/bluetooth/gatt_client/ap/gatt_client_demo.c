...
{
    BLEGATTC_LOGI("\r\n");
    BLEGATTC_LOGI("On BK7259 SMP UART CLI, prefix each line with: ap_cmd\r\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc help\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc scan [1|0]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc conn [addr] [addr_type]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc disconn\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc notifyindcate_en [0|1] [desc_handle_hex]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc read [val_handle_hex]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc write [val_handle_hex] [data]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc read_ext [handle_hex] [offset]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc read_by_uuid [sh] [eh] [uuid16_hex]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc write_ext [handle_hex] [len] [is_cmd]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc discover_service [sh] [eh] [uuid] [uuid_len]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc discover_char [sh] [eh] [uuid] [uuid_len]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc discover_desc [sh] [eh]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc bond\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc add_whl [addr] [addr_type]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc rmv_whl [addr] [addr_type]\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc clear_whl\n");
    BLEGATTC_LOGI("ap_cmd ble_gattc scan_filter [0|1]\n");
    BLEGATTC_LOGI("\r\n");
}
...
void gatt_client_command(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    {
        if (argc < 3) {
            BLEGATTC_LOGE("usage: ap_cmd ble_gattc scan [1|0]\r\n");
            goto error;
        }
    ...
    else if(strcmp(argv [ 1 ], "notifyindcate_en") == 0)
    {
        ...
        if (argc < 4) {
            BLEGATTC_LOGE("usage: ap_cmd ble_gattc notifyindcate_en [0|1] [desc_handle_hex]\r\n");
            goto error;
        }
    ...
    else if(strallcmp(argv [ 1 ], "read") == 0)
    {
        ...
        if (argc < 3) {
            BLEGATTC_LOGE("usage: ap_cmd ble_gattc read [val_handle_hex]\r\n");
            goto error;
        }
        uint16_t char_handle = 0xFF;
    ...
    else if(strallcmp(argv [ 1 ], "write") == 0)
    {
        ...
        if (argc < 4) {
            BLEGATTC_LOGE("usage: ap_cmd ble_gattc write [val_handle_hex] [data]\r\n");
            goto error;
        }
        uint16_t char_handle = 0xFF;
        char_handle = os_strtoul(argv[2], NULL, 16);
        ...
        if(ret != BK_OK)
        {
            BLEGATTC_LOGE("ble write att fail :%d\n", ret);
            goto error;
        }
    }
    ...
    }else if(strcmp(argv [ 1 ], "rmv_whl") == 0)
    {
        ...
        if(!get_addr_from_param(&addr, argv[2]))
        {
            ...
            BLEGATTC_LOGI("rmv whl: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x, addr_type:%d\n",
                          addr.addr[0], addr.addr[1], addr.addr[2], addr.addr[3], addr.addr[4], addr.addr[5],
                          addr_type);
    ...
    {
        if (argc < 3) {
            BLEGATTC_LOGE("usage: ap_cmd ble_gattc scan_filter [0|1]\r\n");
            goto error;
        }
