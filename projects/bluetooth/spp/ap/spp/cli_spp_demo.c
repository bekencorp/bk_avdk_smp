...
static void cli_spp_help(void)
{
    ...
    CLI_LOGI("spp disconn [remote_addr] [spp_handle optional]\n");
            ...
            if(get_spp_dev_by_handle(&dev, param->data_ind.handle))
            {
                ...
                }else
                {
                    if(dev->pending&CLI_SPP_W4_RATE_RX)
                    {
                        ...
                        CLI_LOGI("\r\n%s, spp data ind, handle:0x%02x, len:%d speed: %.3fKB/s \r\n", __func__, param->data_ind.handle, param->data_ind.len, speed);
...
void spp_command(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    else if (!strcmp(argv [ 1 ], "conn"))
    {
        ...
        if (argc < 3)
        {
            CLI_LOGE("Usage: spp conn [remote_addr]\r\n");
            goto error;
        }
    ...
    else if(!strcmp(argv [ 1 ], "disconn"))
    {
        ...
        if (argc < 3)
        {
            CLI_LOGE("Usage: spp disconn [remote_addr] [spp_handle optional]\r\n");
            goto error;
        }
        ...
        else if(!get_spp_dev_by_addr(&dev, addr))
        {
            CLI_LOGI("spp disconn, not find dev \r\n");
            goto error;
        }
    ...
    else if(!strcmp(argv [ 1 ], "stop_server"))
    {
        if (argc < 3)
        {
            CLI_LOGE("Usage: spp stop_server [local_channel]\r\n");
            goto error;
        }
    ...
    else if(!strcmp(argv [ 1 ], "write"))
    {
        if (argc < 4)
        {
            CLI_LOGE("Usage: spp write [handle][data]\r\n");
            goto error;
        }
    ...
    else if(!strcmp(argv [ 1 ], "rate"))
    {
        if (argc < 4)
        {
            CLI_LOGE("Usage: spp rate [handle][length]\r\n");
            goto error;
        }
    ...
    else if(!strcmp(argv [ 1 ], "status"))
    {
        if (argc < 3)
        {
            CLI_LOGE("Usage: spp status [handle]\r\n");
            goto error;
        }
    ...
    else if(!strcmp(argv [ 1 ], "conn1"))
    {
        ...
        if (argc < 4)
        {
            CLI_LOGE("Usage: spp conn1 [remote_addr] [remote_channel]\r\n");
            goto error;
        }
