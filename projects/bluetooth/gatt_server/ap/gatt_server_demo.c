...
static ble_attm_desc_t gatts_service_db[GATTS_IDX_NB] = {
    ...
    [GATTS_IDX_CHAR_N4_VALUE]   = {{GATTS_CHARA_N4_UUID & 0xFF, GATTS_CHARA_N4_UUID >> 8, 0}, BK_BLE_PERM_SET(WRITE_REQ, ENABLE) | BK_BLE_PERM_SET(RD, ENABLE), BK_BLE_PERM_SET(RI, ENABLE) | BK_BLE_PERM_SET(UUID_LEN, UUID_16), 128},
};
            ...
            case GATTS_IDX_CHAR_N4_VALUE:
				{
					...
					n4_len = w_req->len;
					n4_v = os_malloc(n4_len + 1);

					os_memset((uint8_t *)n4_v, 0, n4_len + 1);
	                os_memcpy((uint8_t *)n4_v, w_req->value, n4_len);
        ...
        if (r_req->prf_id == PRF_TASK_ID_GATTS) {
            switch(r_req->att_idx)
            {
                ...
                case GATTS_IDX_CHAR_N4_DECL:
                    break;
                case GATTS_IDX_CHAR_N4_VALUE:
                    bk_ble_read_response_value(r_req->conn_idx, n4_len, (uint8_t*)n4_v, r_req->prf_id, r_req->att_idx);
					BLEGATTS_LOGI("read N4: %s, length: %d\n", n4_v, n4_len);
                    break;
...
{
    BLEGATTS_LOGI("\r\n");
    BLEGATTS_LOGI("ap_cmd ble_gatts help\n");
    BLEGATTS_LOGI("ap_cmd ble_gatts notify\n");
    BLEGATTS_LOGI("ap_cmd ble_gatts adv_en [1|0]\n");
    BLEGATTS_LOGI("ap_cmd ble_gatts bond\n");
    BLEGATTS_LOGI("\r\n");
}
