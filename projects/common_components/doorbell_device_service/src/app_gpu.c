...
#if (CONFIG_PSRAM_WRITE_THROUGH)
static uint32_t s_psram_cover_area;   //todo: doorbell_frame_malloc with params handler
#endif
...
avdk_err_t app_gpu_turn_on(gpu_board_config_t *config)
{
    ...
    gpu_config.malloc = doorbell_frame_malloc;
    gpu_config.free = doorbell_frame_free;
    gpu_config.frame_display = doorbell_frame_complete;
