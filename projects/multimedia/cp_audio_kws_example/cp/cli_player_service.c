...
void cli_player_service_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    else if (os_strcmp(argv[1], "prompt_tone") == 0)
    {
        ...
        if (os_strcmp(argv[2], "start") == 0)
        {
            ...
            {
                LOGE("%s, %d, start voice_service before prompt_tone\n", __func__, __LINE__);
                goto exit;
            }

            tone_id = os_strtoul(argv[3], NULL, 10);
            if (((tone_id == 1) && gl_player1_handle) || ((tone_id != 1) && gl_player_handle))
            {
                LOGE("%s, %d, prompt_tone %u already exists, stop it first\n", __func__, __LINE__, tone_id);
                goto exit;
            }
        ...
        else if (os_strcmp(argv[2], "stop") == 0)
        {
            ...
            {
                LOGE("%s, %d, prompt_tone %u is not running\n", __func__, __LINE__, tone_id);
                goto exit;
            }
