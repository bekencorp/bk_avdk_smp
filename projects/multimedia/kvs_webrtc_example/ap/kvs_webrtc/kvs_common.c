...
STATUS handleAnswer(PSampleConfiguration pSampleConfiguration, PSampleStreamingSession pSampleStreamingSession, PSignalingMessage pSignalingMessage)
{
    ...
    if (pSampleConfiguration->receiveAudioVideoSource != NULL) {
        ThreadParams rxMediaThreadParams;
        rxMediaThreadParams.version = THREAD_PARAMS_CURRENT_VERSION;
        rxMediaThreadParams.stackSize = KVS_DEFAULT_STACK_SIZE;
        rxMediaThreadParams.schedPriority = KVS_THREAD_PRIO_MEDIA;
        THREAD_CREATE_WITH_PARAMS(&pSampleStreamingSession->receiveAudioVideoSenderTid, &rxMediaThreadParams,
                                  pSampleConfiguration->receiveAudioVideoSource, (PVOID) pSampleStreamingSession);
    }
CleanUp:
...
PVOID mediaSenderRoutine(PVOID customData)
{
    ...
    {
        ThreadParams mediaThreadParams;
        mediaThreadParams.version = THREAD_PARAMS_CURRENT_VERSION;
        mediaThreadParams.stackSize = KVS_DEFAULT_STACK_SIZE;
        mediaThreadParams.schedPriority = KVS_THREAD_PRIO_MEDIA;
        ...
        if (pSampleConfiguration->videoSource != NULL &&
            ...
            THREAD_CREATE_WITH_PARAMS(&pSampleConfiguration->videoSenderTid, &mediaThreadParams, pSampleConfiguration->videoSource,
        ...
        if (pSampleConfiguration->audioSource != NULL && pSampleConfiguration->mediaType != SAMPLE_STREAMING_VIDEO_ONLY) {
            ...
            THREAD_CREATE_WITH_PARAMS(&pSampleConfiguration->audioSenderTid, &mediaThreadParams, pSampleConfiguration->audioSource,
                                      (PVOID) pSampleConfiguration);
        }
    }
...
STATUS handleOffer(PSampleConfiguration pSampleConfiguration, PSampleStreamingSession pSampleStreamingSession, PSignalingMessage pSignalingMessage)
{
    ...
    if (!mediaThreadStarted) {
        // mediaSenderRoutine just spawns the real video/audio senders and
        // THREAD_JOIN()s them, so it can stay at generic worker priority.
...
STATUS respondWithAnswer(PSampleStreamingSession pSampleStreamingSession)
{
...
#else
    CHK(NULL != (pMessage = (PSignalingMessage) MEMCALLOC(1, SIZEOF(SignalingMessage))), STATUS_NOT_ENOUGH_MEMORY);
#endif
#if 0
    ...
    #else
    SNPRINTF(pMessage->correlationId, MAX_CORRELATION_ID_LEN, "%" PRIu64 "_%" PRIu64, GETTIME(),
             ATOMIC_INCREMENT(&pSampleStreamingSession->correlationIdPostFix));
    #endif
    #if 0
    ...
    #else
    DLOGD("Responding With Answer With correlationId: %s", pMessage->correlationId);
...
VOID onIceCandidateHandler(UINT64 customData, PCHAR candidateJson)
{
    ...
    if (candidateJson == NULL) {
        ...
        SNPRINTF(pMessage->correlationId, MAX_CORRELATION_ID_LEN, "ice_%" PRIu64 "_%" PRIu64, GETTIME(),
                 ATOMIC_INCREMENT(&pSampleStreamingSession->correlationIdPostFix));
...
STATUS initializePeerConnection(PSampleConfiguration pSampleConfiguration, PRtcPeerConnection* ppRtcPeerConnection)
{
    ...
    #else
    CHK(NULL != (pConfiguration = (PRtcConfiguration) MEMCALLOC(1, SIZEOF(RtcConfiguration))), STATUS_NOT_ENOUGH_MEMORY);
    ...
    #else
    pConfiguration->kvsRtcConfiguration.enableIceStats = pSampleConfiguration->enableIceStats;
    #endif
#endif
    ...
    if (pSampleConfiguration->useTurn) {
        ...
        for (uriCount = 0, i = 0; i < maxTurnServer; i++) {
            ...
            for (j = 0; j < pIceConfigInfo->uriCount; j++) {
                 ...
                 * if pConfiguration->iceServers[uriCount + 1].urls is "turn:ip:port" then ICE will try both TURN over UDP and TCP/TLS
                 *
                 * It's recommended to not pass too many TURN iceServers to pConfiguration because it will slow down ice gathering in non-trickle mode.
                 */
    ...
    if (retStatus == STATUS_NOT_FOUND) {
        ...
        pRtcCertificate = (PRtcCertificate)(uintptr_t) data;
        #if 1
                                    ...
                                    PSampleStreamingSession* ppSampleStreamingSession)
{
    ...
    if (isMaster) {
        ...
        #else
        /* Viewer sends Offer/ICE to Master; must match SAMPLE_MASTER_CLIENT_ID used in kvsWebRTCClientViewer (not local viewer name). */
        STRCPY(pSampleStreamingSession->peerId, SAMPLE_MASTER_CLIENT_ID);
        #endif
    }
                                            ...
                                            UINT64 duration)
{
    ...
    SampleStreamingSession* pSampleStreamingSession = (SampleStreamingSession*)(uintptr_t) customData;
...
STATUS traverseDirectoryPEMFileScan(UINT64 customData, DIR_ENTRY_TYPES entryType, PCHAR fullPath, PCHAR fileName)
{
    ...
    PCHAR certName = (PCHAR)(uintptr_t) customData;
...
STATUS lookForSslCert(PSampleConfiguration* ppSampleConfiguration)
{
    ...
    #else
    CHK(NULL != (certName = (PCHAR) MEMCALLOC(1, MAX_PATH_LEN * SIZEOF(CHAR))), STATUS_NOT_ENOUGH_MEMORY);
    MEMSET(certName, 0x0, MAX_PATH_LEN * SIZEOF(CHAR));
    #endif
                                 ...
                                 PSampleConfiguration* ppSampleConfiguration)
{
    ...
    #else
    pSampleConfiguration->clientInfo.cacheFilePath = (PCHAR) SAMPLE_SIGNALING_CACHE_FILE_PATH; /* SD card */
    #endif
...
STATUS freeSampleConfiguration(PSampleConfiguration* ppSampleConfiguration)
{
    ...
    if (pSampleConfiguration->pPendingSignalingMessageForRemoteClient != NULL) {
        ...
        while (IS_VALID_ITERATOR(iterator)) {
            ...
            freeMessageQueue((PPendingMessageQueue)(uintptr_t) data);
        }
    ...
    if (pSampleConfiguration->pregeneratedCertificates != NULL) {
        ...
        while (IS_VALID_ITERATOR(iterator)) {
            stackQueueIteratorGetItem(iterator, &data);
            stackQueueIteratorNext(&iterator);
            freeRtcCertificate((PRtcCertificate)(uintptr_t) data);
        }
    ...
    while (!ATOMIC_LOAD_BOOL(&pSampleConfiguration->interrupted)) {
        ...
        if (ATOMIC_LOAD_BOOL(&pSampleConfiguration->recreateSignalingClient)) {
            ...
            if (STATUS_SUCCEEDED(retStatus)) {
                ...
                os_printf("[KVS Common] recreating Signaling Client\n");
...
STATUS submitPendingIceCandidate(PPendingMessageQueue pPendingMessageQueue, PSampleStreamingSession pSampleStreamingSession)
{
    ...
    do {
        ...
        if (!noPendingSignalingMessageForClient) {
            ...
            pReceivedSignalingMessage = (PReceivedSignalingMessage)(uintptr_t) hashValue;
...
STATUS signalingMessageReceived(UINT64 customData, PReceivedSignalingMessage pReceivedSignalingMessage)
{
    ...
    if (peerConnectionFound) {
        ...
        pSampleStreamingSession = (PSampleStreamingSession)(uintptr_t) hashValue;
    }
...
STATUS getPendingMessageQueueForHash(PStackQueue pPendingQueue, UINT64 clientHash, BOOL remove, PPendingMessageQueue* ppPendingMessageQueue)
{
    ...
    while (iterate && IS_VALID_ITERATOR(iterator)) {
        ...
        pPendingMessageQueue = (PPendingMessageQueue)(uintptr_t) data;
