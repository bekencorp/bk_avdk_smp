...
#if CONFIG_LPVAD
	os_printf("aud_lp_vad_test {init|reg|prep|flow|deinit|status} \r\n");
#endif
}
...
static int xdac_sample_rate = 16000;
...
static void cli_aud_generate_pcm_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    if (os_strcmp(argv[1], "gen") == 0)
    {
        ...
        frequency = (int)strtoul(argv[2], NULL, 10);
        amp_db = (int)strtoul(argv[3], NULL, 10);
        ...
        if(4 < argc)
        {
            sample_rate = (int)strtoul(argv[4], NULL, 10);
        }
        if(5 < argc)
        {
        ...
        {
            duration_ms = sample_rate/frequency;
        }
        if(6 < argc)
        {
        ...
        LOGI("malloc %d bytes\n", mem_size);
        signal = (uint32_t *)psram_malloc(mem_size);
        if (signal == NULL) {
            LOGE("Failed to allocate memory for PCM signal (size: %d bytes)\n", mem_size);
            return;
        }
        xdac_sample_rate = sample_rate;
...
extern const uint32_t XDAC_12bit_16000[16];
...
static void cli_xdac_dma_ringbuf_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    if (os_strcmp(argv[1], "start") == 0)
    {
        ...
        ret = bk_xdac_set_sample_rate(xdac_ch, xdac_sample_rate);

        // Ring buffer size: entire audio array size (aud_len * 4 bytes) * 2 + safe interval
        ring_buff_size = data_len * 2 * 2 + DAC_DMA_RING_BUFF_SAFE_INTERVAL;

        // Allocate ring buffer for DMA
        xdac_ring_buff = (int32_t *)psram_malloc(ring_buff_size);
...
#if CONFIG_LPVAD
static volatile uint32_t s_lp_vad_irq_cnt = 0;

static void cli_lp_vad_isr(void)
{
	s_lp_vad_irq_cnt++;
	LOGI("lp_vad irq, cnt=%lu\r\n", s_lp_vad_irq_cnt);
}

static void cli_aud_lp_vad_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	bk_err_t ret = BK_OK;

	__maybe_unused_var(pcWriteBuffer);
	__maybe_unused_var(xWriteBufferLen);
	...
	if (os_strcmp(argv[1], "init") == 0) {
		ret = bk_lp_vad_init();
		LOGI("bk_lp_vad_init ret=%d\r\n", ret);
	} else if (os_strcmp(argv[1], "reg") == 0) {
		ret = bk_lp_vad_register_isr(cli_lp_vad_isr);
		LOGI("bk_lp_vad_register_isr ret=%d\r\n", ret);
	} else if (os_strcmp(argv[1], "prep") == 0) {
		ret = bk_lp_vad_deepsleep_enter();
		LOGI("bk_lp_vad_deepsleep_enter ret=%d\r\n", ret);
	} else if (os_strcmp(argv[1], "flow") == 0) {
		ret = bk_lp_vad_init();
		LOGI("flow: bk_lp_vad_init ret=%d\r\n", ret);
		if (ret != BK_OK) {
			return;
		}

		ret = bk_lp_vad_register_isr(cli_lp_vad_isr);
		LOGI("flow: bk_lp_vad_register_isr ret=%d\r\n", ret);
		if (ret != BK_OK) {
			return;
		}

		ret = bk_lp_vad_deepsleep_enter();
		LOGI("flow: bk_lp_vad_deepsleep_enter ret=%d\r\n", ret);
	} else if (os_strcmp(argv[1], "deinit") == 0) {
		ret = bk_lp_vad_deinit();
		LOGI("bk_lp_vad_deinit ret=%d\r\n", ret);
...
#if CONFIG_LPVAD
	{"aud_lp_vad_test", "aud_lp_vad_test {init|reg|prep|flow|deinit|status}", cli_aud_lp_vad_test_cmd},
#endif

	{"aud_generate_pcm_test", "aud_generate_pcm_test {gen|dump}  {frequency} {amp_db} {sample_rate} {duration_seconds}", cli_aud_generate_pcm_test_cmd},
};
