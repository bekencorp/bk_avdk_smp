...
def check_is_ab_project():
    if os.path.exists('%s/partitions/%s/ab_position_independent.csv'%(project_dir, chip)):
        ab_pos_independent_file = "%s/partitions/%s/ab_position_independent.csv"%(project_dir, chip)
...
def get_bootloader_archieve_dir():
    archieve_dir = f"{armino_path}/components/bk_libs/{chip}/bootloader"
...
def copy_bootloader_to_relevant_dir():
    ...
    if check_is_ab_project() == 'True':
        ...
        dest_boot_dir = f'{armino_path}/components/bk_libs/{chip}/bootloader/ab_bootloader'
    else:
        bl_project_path = non_ab_project_bl_path
        dest_boot_dir = f'{armino_path}/components/bk_libs/{chip}/bootloader/normal_bootloader'
