...
def bl2_sign(action_type, key_type, privkey_pem_file, pubkey_pem_file, signature, bin_file, partition_size, version, security_counter, sign_outfile, hash_outfile, pad=True):
    ...
    # trailer at the slot end. With DIRECT_XIP_REVERT off and boot_param A/B,
    # the trailer is unused; pass pad=False to keep images content-sized.
    # --slot-size is kept either way for the "image too big for slot" check.
