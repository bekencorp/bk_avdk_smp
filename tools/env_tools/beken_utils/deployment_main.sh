...
#   bl2.bin cp_app.bin ap_app.bin
#   partitions.csv security.csv ota.csv pack.json
#   (the exact code bins follow the partition names in partitions.csv)
#
...
# "key,value" layouts are supported). The flash AES engine uses AES-128-XTS,
# so the key must be 64 hex chars (32 bytes of XTS key material).
inject_flash_aes_key() {
    ...
    if [ "$key_len" -ne 64 ]; then
        echo "Error: invalid flash AES key length: $key_len (expected 64 hex chars)"
        return 1
    fi
...
echo "========================================"
echo "Secure firmware deployment"
echo "========================================"
