...
def infer_aes_key_bits_from_hex(key_hex: str) -> str:
    key_len = len(key_hex)
    if key_len == 64:
        return '128'
    if key_len == 128:
        return '256'
    raise ValueError(f"Unsupported AES key length: {key_len} hex chars")


def is_plain_hex_key(keywords: str) -> bool:
    if len(keywords) not in (64, 128):
        return False

    try:
        bytes.fromhex(keywords)
    except ValueError:
        return False
...
def normalize_key_hex_for_aes_mode(key_hex: str, aes: str) -> str:
    if aes == '128' and len(key_hex) == 128:
        print("use last 64 hex chars for AES-128 test")
        return key_hex[64:]
...
def xts_aes_process(data: bytes, key: bytes, start_address: int, mode: str) -> bytes:
    ...
    #print(f" total_units {total_units}, unit_num {unit_num}")
...
def process(mode, infile, startaddress, aes, keywords, keyfile, outfile):
    ...
    key_hex = normalize_key_hex_for_aes_mode(key_hex, aes)
    key_bytes = str2key(key_hex)
    print(f"key_bytes {key_bytes}")
    expected_aes = infer_aes_key_bits_from_hex(key_hex)
    if aes != expected_aes:
        raise ValueError(f"AES mode {aes} does not match key length, expected {expected_aes}.")
