...
def packager(args):
    ...
    if os.path.exists('%s/partitions/ota_rbl.config'%(project_dir)) and os.path.exists('%s/partitions/%s/configurationab.json'%(project_dir,armino_soc)):
        b_new_chip = 1
        config = '%s/partitions/ota_rbl.config'%(project_dir)
        print('=========================>>>> b_new_chip = 1')
    #print("============>>>>",config)
    ...
    with open(args.outfile,"wb") as f:
        if b_new_chip == 1:
            ...
            config = '%s/partitions/%s/configurationab.json'%(project_dir, armino_soc)
