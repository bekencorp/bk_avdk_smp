...
function(armino_build_executable bin)
    ...
    # Secure firmware post-build: direct CP builds pack here. Top-level SMP
    # builds defer packing until both AP and CP have completed, otherwise the
    # CP target can race the AP target before ap_app.bin exists.
    set(armino_secure_pack)
    if(CONFIG_SECURITY_FIRMWARE AND NOT "$ENV{ARMINO_DEFER_SECURE_PACK}" STREQUAL "1")
...
function(armino_build_parse_config_file_list)
    ...
    if(SDKCONFIG)
        ...
        if (BUILD_PROPERTIES_LIB)
            ...
            if(DEFINED ENV{CONFIG_SUBTITUTE_FILE})
                set(config_name "$ENV{CONFIG_SUBTITUTE_FILE}")
            endif()
            set(sdkconfig "${CMAKE_BINARY_DIR}/config/${config_name}")
            # set(sdkconfig "${CMAKE_SOURCE_DIR}/${sub_sys}/config/${ARMINO_SOC}/${config_name}")
            get_filename_component(sdkconfig_parent_dir "${sdkconfig}" DIRECTORY)
            file(MAKE_DIRECTORY "${sdkconfig_parent_dir}")
        endif()
    endif()
...
function(armino_build_parse_toolchain_dir)
    ...
    if(toolchain_dir)
        ...
        armino_build_set_property(TOOLCHAIN_DIR "")
        set(ENV{TOOLCHAIN_DIR} "")
    endif()
endfunction()
