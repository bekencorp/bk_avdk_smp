...
from kconfiglib.core import STR_TO_TRI
...
[Q] Quit (prompts for save) [D] Save Project minimal default config
"""[1:-1].split("\n")
    ...
    while True:
        ...
        if c == curses.KEY_RESIZE:
            ...
            default_filename = os.environ.get(
                "ARMINO_PROJECT_DEFCONFIG",
                os.path.join(os.path.dirname(_conf_filename), "defconfig")
            )
            filename = _save_dialog(
                _save_project_diff, default_filename, "project default configuration"
            )
            if filename:
...
def _save_dialog(save_fn, default_filename, description):
    ...
    if msg:
        _msg("Success", msg)
        # After successfully saving the full configuration, update the
        # project-specific minimal configuration (defconfig) if applicable.
...
def _update_project_config():
    # Generate a minimal project-specific config (defconfig) next to the
    # main configuration file, if the environment provides enough information.
    # First, ensure the config file is up-to-date (write in-memory changes to disk)
    try:
        _kconf.write_config(_conf_filename)
    except Exception:
        # If write fails, still try to generate defconfig (based on old config)
        pass
    
    full_cfg = os.environ.get("KCONFIG_CONFIG", _conf_filename)
    if not full_cfg:
        return

    armino_path = os.environ.get("ARMINO_PATH")
    armino_soc = os.environ.get("ARMINO_SOC")
    if not armino_path or not armino_soc:
        return

    # Board-level defconfig: ${ARMINO_PATH}/middleware/soc/${ARMINO_SOC}/${ARMINO_SOC}.defconfig
    defconfig = os.path.join(
        armino_path,
        "middleware",
        "soc",
        armino_soc,
        f"{armino_soc}.defconfig",
    )
    if not os.path.exists(defconfig):
        return

    project_cfg = os.environ.get(
        "ARMINO_PROJECT_DEFCONFIG",
        os.path.join(os.path.dirname(full_cfg), "defconfig")
    )
    os.makedirs(os.path.dirname(project_cfg), exist_ok=True)
    diff_script = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "kconf_diff_project.py"
    )
    if not os.path.exists(diff_script):
        return

    try:
        # Run diff generation quietly to avoid disturbing the curses UI.
        subprocess.run(
            [
                sys.executable,
                diff_script,
                "--kconfig",
                os.path.join(armino_path, "Kconfig"),
                "--defconfig",
                defconfig,
                "--full-config",
                full_cfg,
        ...
        # Don't break menuconfig if diff generation fails
        pass


def _save_project_diff(filename):
    # Generate defconfig (diff relative to board-level defconfig)
    # filename parameter comes from save dialog, but actual output path is
    # determined by _update_project_config
    _update_project_config()
    return "Project default configuration saved: " + filename
