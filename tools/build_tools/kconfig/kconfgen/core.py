...
import kconfdoc.gen_kconfig_doc as gen_kconfig_doc
import kconfiglib.core as kconfiglib
from kconfdoc import __version__
...
def write_config(
    ...
    # BEKEN IoT Development Framework (BEKEN-ARMINO) {armino_version} Project Configuration
    #
    """
    )
...
def write_min_config(_, config: kconfiglib.Kconfig, filename: str) -> None:
    ...
    CONFIG_HEADING = textwrap.dedent(
        f"""\
    # This file was generated using armino save-defconfig. It can be edited manually.
    # BEKEN IoT Development Framework (BEKEN-ARMINO) {armino_version} Project Minimal Configuration
    #
    {target_symbol.config_string if write_target else ""}\
    """
    )

    if os.environ.get("BEKEN_ARMINO_KCONFIG_MIN_LABELS", False) == "1":
...
def write_header(
    ...
    CONFIG_HEADING = f"""/*
 * Automatically generated file. DO NOT EDIT.
 * BEKEN IoT Development Framework (BEKEN-ARMINO) {armino_version} Configuration Header
 */
#pragma once
"""
...
def main():
    ...
    if args.config and os.path.exists(args.config):
        ...
        with tempfile.NamedTemporaryFile(prefix="kconfgen_tmp", delete=False) as f:
            temp_file = f.name
        try:
            deprecated_options.replace(
                sdkconfig_in=args.config, sdkconfig_out=temp_file
            )
