...
#if CONFIG_USB_RISCV_BRIDGE
/* ===================================================================
 * RISC-V CP USB-device bridge for CherryUSB v1.6
 *
 * Mirror of the host bridge (usb_hc_musb.c): the low-level MUSB *device*
 * engine (EP0 state machine, FIFO read/write, reset/suspend) runs on the
 * RISC-V CP firmware. The AP keeps running the v1.6 device stack and only:
 *   - publishes g_musb_udc + usb_ep0_state, which the CP reads/writes IN PLACE
 *     (g_musb_udc is byte-compatible with the CP's musb_udc_t -- this requires
 *     CONFIG_USB_MUSB_EP_NUM == 16, see usb_config.h)
 *   - fills g_musb_udc.in_ep/out_ep on usbd_ep_start_write/read (unchanged v1.6
 *     code paths) so the CP can run the transfer
 *   - receives CP completions over IPI and replays the cherryusb upcalls.
 *
 * The shared IPI_DOMAIN_USB callback lives in usb_hc_musb.c and dispatches here
 * by probe->role == RISCV_USB_ROLE_DEVICE.
 * =================================================================== */
...
/* Registered in usb_hc_musb.c; shared IPI_DOMAIN_USB callback enable. */
extern bk_err_t bk_v16_bridge_ipi_enable(void);

static uint32_t s_dev_last_irq_seq;

static void bk_v16_device_probe_init(void)
{
...
/* AP-side consumer of the device drain protocol. Called from the shared IPI
 * callback when probe->role is DEVICE. The CP firmware owns all MUSB
 * register/FIFO access and updates g_musb_udc in place; here we only replay the
 * cherryusb upcalls, mirroring the M55-resident USBD_IRQHandler order
...
/* Returns true when the RISC-V CP firmware took ownership of the USBD interrupt
 * and the AP must NOT register its own USBD_IRQHandler. Returns false to fall
 * back to the M55-resident device path. */
bool bk_v16_device_bridge_bringup(void)
{
    bk_v16_device_probe_init();

    if (usb_dc_riscv_device_prepare() != 0) {
        USB_LOG_WRN("[bk_v1_6] riscv device_prepare failed, fall back to M55 USBD\r\n");
        return false;
    }

#if CONFIG_IPI
    if (bk_v16_bridge_ipi_enable() != BK_OK) {
        USB_LOG_ERR("[bk_v1_6] riscv device IPI enable failed\r\n");
    }
#endif
    ...
    USB_LOG_INFO("[bk_v1_6] usb device use riscv CP bridge path\r\n");
    return true;
}
