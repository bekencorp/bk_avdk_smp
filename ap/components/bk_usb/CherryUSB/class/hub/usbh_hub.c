                ...
                if (!(portstatus & HUB_PORT_STATUS_RESET) && (portstatus & HUB_PORT_STATUS_ENABLE)) {
                    ...
                    */
                    ...
                    if (usbh_enumerate(child) < 0) {
                        ...
                        /* BK7259: HS storage devices frequently fall back to FS
                         * on the first chirp and then fail. Power-cycle the PHY
                         * and re-arm the controller so the device re-attaches
                         * and gets another chirp, until HS is negotiated. */
                        if (hub->is_roothub) {
                            extern void bk_cherryusb_v16_host_recover(struct usbh_bus *bus);
                            bk_cherryusb_v16_host_recover(hub->bus);
                        }
#endif
                    }
                } else {
