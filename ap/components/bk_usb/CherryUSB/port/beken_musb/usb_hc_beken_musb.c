...
#include "riscv_bridge/riscv_usb_cache.h"
#endif
...
static void usb_hc_riscv_probe_init(uint32_t role)
{
    ...
    riscv_usb_probe_cache_clean();
    riscv_usb_cache_clean(&g_musb_hcd, sizeof(g_musb_hcd));
    riscv_usb_cache_clean((void *)&usb_ep0_state, sizeof(usb_ep0_state));
}
...
void usb_hc_riscv_poll_events(void)
{
    ...
    if (ctx->irq_seq == s_riscv_probe_last_irq_seq) {
        USB_LOG_WRN("irq_seq %d == last_irq_seq %d, return\r\n", ctx->irq_seq, s_riscv_probe_last_irq_seq);
        return;
    }
    ...
    switch (event) {
        ...
        case RISCV_USB_PROBE_EVT_DISCONNECT:
            USB_LOG_VBS("%s DISCONNECT from riscv\r\n", __func__);
...
#define SYS_ANA_SPI_TRIG_REG (0x44010000 + 0x40 * 4)
    ...
    USB_LOG_VBS("usb trigger spitrig\r\n");
    REG_WRITE(SYS_ANA_SPI_TRIG_REG, value & ~(0x01<<19));
}
...
__WEAK void usb_hc_low_level_init(void)
{
    ...
    bk_int_isr_register(INT_SRC_USB_HS, USBH_IRQHandler, NULL);
...
int usbh_submit_urb(struct usbh_urb *urb)
{
    ...
    switch (pipe->ep_type) {
        case USB_ENDPOINT_TYPE_CONTROL:
            ...
            riscv_usb_urb_clean_for_riscv(urb);
            riscv_usb_cache_clean(&g_musb_hcd, sizeof(g_musb_hcd));
            riscv_usb_cache_clean((void *)&usb_ep0_state, sizeof(usb_ep0_state));
#endif
...
#if CONFIG_USB_RISCV_BRIDGE && CONFIG_SRAM_DIRECT_ADDR
    riscv_usb_urb_invalidate_from_riscv(urb);
#endif
