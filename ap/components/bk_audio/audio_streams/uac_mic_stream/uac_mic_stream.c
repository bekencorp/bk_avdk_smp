...
/* dump uac_mic stream output pcm data by uart */
...
#define UAC_MIC_DATA_DUMP_BY_UART_OPEN()                    uart_util_create(&gl_ob_mic_uart_util, UAC_MIC_DATA_DUMP_UART_ID, UAC_MIC_DATA_DUMP_UART_BAUD_RATE)
#define UAC_MIC_DATA_DUMP_BY_UART_CLOSE()                   uart_util_destroy(&gl_ob_mic_uart_util)
#define UAC_MIC_DATA_DUMP_BY_UART_DATA(data_buf, len)       uart_util_tx_data(&gl_ob_mic_uart_util, data_buf, len)

#else

#define UAC_MIC_DATA_DUMP_BY_UART_OPEN()
#define UAC_MIC_DATA_DUMP_BY_UART_CLOSE()
#define UAC_MIC_DATA_DUMP_BY_UART_DATA(data_buf, len)
...
typedef struct uac_mic_stream
{
    ...
    uint32_t                      urb_err_cnt;           /**< consecutive URB completion error count, re-trigger EP when >= UAC_MIC_URB_RE_TRIGGER_ERR_THRESHOLD */
...
static void usb_hub_uac_mic_port_dev_complete_callback(void *pCompleteParam, int nbytes)
{
    ...
    {
        BK_LOGD(TAG, "%s, %d, nbytes:%d \n", __func__, __LINE__, nbytes);
        uac_mic->urb_err_cnt++;
        if (uac_mic->urb_err_cnt >= UAC_MIC_URB_RE_TRIGGER_ERR_THRESHOLD)
        {
            ...
            {
                uac_mic->urb_err_cnt = 0;
                BK_LOGV(TAG, "%s, %d, re-trigger ep after %d consecutive errors\n", __func__, __LINE__, UAC_MIC_URB_RE_TRIGGER_ERR_THRESHOLD);
            }
            else
            {
                BK_LOGE(TAG, "%s, %d, re-trigger ep fail, ret:%d \n", __func__, __LINE__, ret);
            }
        }
    }
...
static bk_err_t uac_mic_drv_init(uac_mic_stream_t *uac_mic)
{
    ...
    UAC_MIC_DATA_DUMP_BY_UART_OPEN();
...
static int _uac_mic_process(audio_element_handle_t self, char *in_buffer, int in_len)
{
    ...
    if (r_size > 0)
    {
        ...
        w_size = audio_element_output(self, in_buffer, r_size);
        UAC_MIC_OUTPUT_END();
        UAC_MIC_DATA_DUMP_BY_UART_DATA(in_buffer, r_size);
...
static int _uac_mic_destroy(audio_element_handle_t self)
{
    ...
    UAC_MIC_DATA_DUMP_BY_UART_CLOSE();
