...
static onboard_dual_dmic_mic_stream_t *gl_onboard_dmic_mic = NULL;
...
static void aud_adc_dma_finish_isr(void)
{
    ...
    if(gl_onboard_dmic_mic->dual_dmic && gl_onboard_dmic_mic->ref_mode)
    {
        rtos_set_event_flags(&gl_onboard_dmic_mic->mic_evt, EVENT_MIC_DMA_DONE_BIT);
    }
...
{
    rtos_set_event_flags(&gl_onboard_dmic_mic->mic_evt,EVENT_DUAL_DMIC_DMA_DONE_BIT);
...
static int _onboard_dual_dmic_mic_read_hw_ref(audio_element_handle_t self, char *buffer, int len)
{
    ...
    if (len)
    {
        ...
        if (fill_size >= len)
        {
            read_size = ring_buffer_read(&onboard_mic->mic_rb, (uint8_t *)p_mic_data_temp_save, len/2);
            if (read_size == onboard_mic->frame_size)
            {
                for (uint16_t i = 0; i < onboard_mic->frame_size; i++)
                {
                    addr[i] = p_mic_data_temp_save[i];//get hw ref data,only half data needed
                }
                ret = read_size;
...
static bk_err_t _onboard_dual_dmic_mic_close(audio_element_handle_t self)
{
    ...
    ret = bk_aud_adc_stop(AUD_ADC_CHL_0);
    if (ret != BK_OK)
    {
...
static bk_err_t _onboard_dual_dmic_mic_destroy(audio_element_handle_t self)
{
    ...
    if(gl_onboard_dmic_mic->hw_ref_buf)
    {
        audio_free(gl_onboard_dmic_mic->hw_ref_buf);
        gl_onboard_dmic_mic->hw_ref_buf = NULL;
    }
...
audio_element_handle_t onboard_dual_dmic_mic_stream_init(onboard_dual_dmic_mic_stream_cfg_t *config)
{
    ...
    bk_err_t ret = BK_OK;
    gl_onboard_dmic_mic = audio_calloc(1, sizeof(onboard_dual_dmic_mic_stream_t));
    AUDIO_MEM_CHECK(TAG, gl_onboard_dmic_mic, return NULL);
    ...
    gl_onboard_dmic_mic->ref_mode = config->ref_mode;
    gl_onboard_dmic_mic->dual_dmic = config->dual_dmic;
    gl_onboard_dmic_mic->dual_dmic_sgl_out = config->dual_dmic_sgl_out;
    if(gl_onboard_dmic_mic->dual_dmic)
    {
        if(gl_onboard_dmic_mic->ref_mode)
        {
            p_mic_data_temp_save = audio_calloc(1, config->frame_size);
            AUDIO_MEM_CHECK(TAG, p_mic_data_temp_save, return NULL);

            gl_onboard_dmic_mic->hw_ref_buf = audio_calloc(1, config->frame_size);
            AUDIO_MEM_CHECK(TAG, gl_onboard_dmic_mic->hw_ref_buf, return NULL);
    ...
    cfg.tag = "onboard_mic";
    gl_onboard_dmic_mic->frame_size = config->frame_size;
    os_memcpy(&gl_onboard_dmic_mic->adc_cfg, &config->adc_cfg, sizeof(aud_adc_config_t));
    gl_onboard_dmic_mic->out_block_size = config->out_block_size;
    gl_onboard_dmic_mic->out_block_num = config->out_block_num;
    gl_onboard_dmic_mic->dmic_data_shift = config->dmic_data_shift;
    ...
    aud_adc_cfg.sample_rate = config->adc_cfg.sample_rate;
    ...
    ret = bk_aud_adc_init(&gl_onboard_dmic_mic->adc_cfg);
    if (ret != BK_OK)
    {
    ...
    ret = aud_adc_dma_config(gl_onboard_dmic_mic);
    if (ret != BK_OK)
    {
    ...
    if(gl_onboard_dmic_mic->dual_dmic)
    {
        ...
        BK_LOGD(TAG, "dmic_cfg samp_rate: %d, clk_src: %s\n",dmic_config.samp_rate, dmic_config.clk_src == 1 ? "APLL" : "XTAL");
        ...
        ret = aud_dmic_dma_config(gl_onboard_dmic_mic);
        if (ret != BK_OK)
        {
            BK_LOGE(TAG, "%s, %d, adc_dma_init fail\n", __func__, __LINE__);
            goto _onboard_mic_init_exit;
    ...
    rtos_init_event_flags(&gl_onboard_dmic_mic->mic_evt);
    if (ret != BK_OK)
    {
    ...
    audio_element_setdata(el, gl_onboard_dmic_mic);
...
_onboard_mic_init_exit:
    /* deinit dma */
    aud_adc_dma_deconfig(gl_onboard_dmic_mic);
    /* deinit adc */
    bk_aud_adc_deinit();
    bk_aud_driver_deinit();

    if(gl_onboard_dmic_mic->mic_evt)
    {
        rtos_deinit_event_flags(&gl_onboard_dmic_mic->mic_evt);
    }

    audio_free(gl_onboard_dmic_mic);
    gl_onboard_dmic_mic = NULL;
    return NULL;
}
