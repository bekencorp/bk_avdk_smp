...
volatile ap_secure_fault_context_t
    g_ap_secure_fault_context[AP_SEC_DUMP_CONTEXT_COUNT]
    __attribute__((aligned(32)));
const uintptr_t g_ap_secure_fault_context_address =
    (uintptr_t)g_ap_secure_fault_context;
...
void bk_coredump_secure_registers(const ap_secure_fault_context_t *context)
{
    ...
    bool source_secure = (context->flags & AP_SEC_DUMP_FLAG_SOURCE_SECURE) != 0U;
    bool source_psp = (context->flags & AP_SEC_DUMP_FLAG_SOURCE_PSP) != 0U;
...
{
    ap_secure_fault_context_t *context =
        (ap_secure_fault_context_t *)context_arg;
    bool valid_context = false;

    for (uint32_t i = 0; i < AP_SEC_DUMP_CONTEXT_COUNT; i++) {
        if (context == (ap_secure_fault_context_t *)&g_ap_secure_fault_context[i]) {
            valid_context = true;
            break;
        }
    }
    ...
    if (valid_context &&
        context->magic == AP_SEC_DUMP_CONTEXT_MAGIC &&
        context->version == AP_SEC_DUMP_ABI_VERSION &&
        context->size == sizeof(*context) &&
        context->frame_valid != 0U) {
        bk_exception_handler_from_secure(context);
        /*
         * IPC_AP_TRAP_HANDLE_END only posts the dump request to CP. Match the
         * normal NS exception wrapper: keep the faulting AP core stopped while
         * CP pulls AP memory and performs the final watchdog reboot.
         */
        while (1) {
        }
    }

    /* Invalid handoff context: CP cannot safely pull memory, reboot locally. */
