...
/* P0-3: aligned with the CP coredump UART writer. On a per-byte write timeout
 * the stuck UART is snapshotted, soft-reset/recovered and the current 4KB block
 * is retransmitted; after COREDUMP_UART_MAX_TIMEOUTS the AP resets the board so
 * a wedged UART never turns into a silent no-dump hang. */
