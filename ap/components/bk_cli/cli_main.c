...
void cli_sort_command(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
        /* the space of commands cannot at the flash space*/
#if CONFIG_CLI_CMD_SORT
...
static void _help_cmd_handler(void **argtable)
{
	const char *sub_cmd;
	int i, n, is_show, is_cmd_cmp = 0;
	struct arg_lit *help = (struct arg_lit *)argtable[0];
	struct arg_str *arg_str = (struct arg_str *)argtable[1];
...
#define cmd_ind_printf		os_printf
	...
	cmd_ind_printf("====CLI Commands is the following: ====\r\n");
	for (i = 0, n = 0; i < pCli->num_commands; i++) {
                is_show = 0;

                if(is_cmd_cmp){
                        if(NULL != strstr(pCli->commands[i]->name, sub_cmd)){
                                is_show = 1;
                        }
                }else if(pCli->commands[i]->name){
                        is_show = 1;
                }
...
void help_command(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	struct arg_str *arg_str = arg_strn(NULL, NULL, "sub_cmd", 0, 1, "find_cmd_str");
	struct arg_end *end = arg_end(10);

	void* argtable[] = { help, arg_str, end };
	int argtable_size = sizeof(argtable) / sizeof(argtable[0]);

	common_cmd_handler(argc, argv, argtable, argtable_size, _help_cmd_handler);
#endif
}
...
bk_err_t cli_commands_init(struct cli_st *cli_ptr)
{
        int num_commands;
        struct cli_command *cmd_table_start_ptr = (struct cli_command *)&_cli_cmds_start;
        struct cli_command *cmd_table_end_ptr = (struct cli_command *)&_cli_cmds_end;

        num_commands = ((uint32_t)cmd_table_end_ptr - (uint32_t)cmd_table_start_ptr) /
                                sizeof(cmd_table_end_ptr[0]);

        return cli_register_commands(cmd_table_start_ptr, num_commands);
}
