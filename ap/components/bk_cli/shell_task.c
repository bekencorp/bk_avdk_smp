...
// #if (CONFIG_SUPPORT_CACHEABLE_SRAM) && (CONFIG_LV_USE_DEMO_METER)
...
static int shell_assert_out_va(bool bContinue, const char * format, va_list arg_list)
{
	...
	if (arch_is_enter_exception()) {
		...
		bk_coredump_write_prompt_data((uint8_t *)pbuf, data_len);
...
int shell_assert_raw(bool bContinue, char * data_buff, u16 data_len)
{
	...
	if (arch_is_enter_exception()) {
		void bk_coredump_write_prompt_data(uint8_t *data, uint32_t size);
		bk_coredump_write_prompt_data((uint8_t *)data_buff, data_len);
		return 1;
	}
