...
static void cli_interrupt_help(void)
{
	...
	CLI_LOGD("m55sub_int register {int_number} - register m55sub interrupt\n");
	CLI_LOGD("m55sub_int unregister {int_number} - unregister m55sub interrupt\n");
}
...
static void m55sub_interrupt_handler(void) {
	BK_LOGI("INT", "m55sub interrupt handler\r\n");
}

static void cli_m55sub_interrupt_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	if (argc < 3) {
		cli_interrupt_help();
		return;
	}
	uint32_t int_number = os_strtoul(argv[2], NULL, 10);
	if (os_strcmp(argv[1], "register") == 0) {
		bk_interrupt_register_m55sub_int(int_number, m55sub_interrupt_handler);
	} else if (os_strcmp(argv[1], "unregister") == 0) {
		bk_interrupt_unregister_m55sub_int(int_number);
	}
	else {
		cli_interrupt_help();
	}
...
static const struct cli_command s_interrupt_commands[] = {
	...
	{"m55sub_int", "m55sub_int {register|unregister} {int_number}", cli_m55sub_interrupt_cmd},
};
