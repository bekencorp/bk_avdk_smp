...
#if defined(CONFIG_SRAM2_TEST_ADDR) && defined(CONFIG_SRAM2_TEST_SIZE)
    { CONFIG_SRAM2_TEST_ADDR, CONFIG_SRAM2_TEST_SIZE, "SRAM2_TEST" },
#endif
...
static void cli_cpu_perf_test(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    if (target_core >= 0)
    {
        ...
        CLI_LOGI("cpu_perf_test start, mask=0x%X, target_core=%d (CPU%d), current_core=%u (CPU%u)\r\n",
                 (unsigned)mask, target_core, target_core + 2, (unsigned)cur, (unsigned)cur + 2);
