...
/* Poll until cpu3 online-state == @want (offline teardown is asynchronous). */
static uint32_t cli_cpu_hp_vote_wait_online(uint32_t want, uint32_t timeout_ms)
{
	for (uint32_t waited = 0; waited <= timeout_ms; waited += CPU_HP_VOTE_TEST_POLL_MS) {
		if (bk_cpu_hp_is_online(CPU3_CORE_ID) == want) {
			return 1;
		}
		rtos_delay_milliseconds(CPU_HP_VOTE_TEST_POLL_MS);
	}

	return (bk_cpu_hp_is_online(CPU3_CORE_ID) == want);
}

/*
 * Guard for the self-test: it needs a clean slate (no voters registered
 * and no tasks pinned to cpu3), otherwise cpu3 would legitimately stay online
...
static uint32_t cli_cpu_hp_vote_precheck(const char *what)
{
	...
	if (xTaskHasTasksPinnedToCore(SMP_CORE1_ID) == pdTRUE) {
		CLI_LOGE("cpu vote %s: skip, tasks pinned to cpu3 block offline\r\n", what);
		return 0;
	}
...
{
	if (was_online == bk_cpu_hp_is_online(CPU3_CORE_ID)) {
		return;
	}

	if (was_online) {
		bk_cpu_hp_online_direct(CPU3_CORE_ID);
	} else {
		bk_cpu_hp_offline_direct(CPU3_CORE_ID);
	}
	CLI_LOGI("cpu vote %s: restored cpu3 %s\r\n", what, was_online ? "online" : "offline");
}

/*
 * Self-test for the voting layer: verifies OR power-up (any online vote onlines
 * cpu3), AND power-down (cpu3 offlines only when all voters release), vote
 * idempotency, unregister-releases-vote, and parameter validation.
 *
 * Preconditions: no voters registered yet and no tasks pinned to cpu3 (either
 * would legitimately keep cpu3 online and make the AND-offline steps fail).
 */
...
#define VOTE_CHECK(cond, desc) do {                       \
	...
	was_online = bk_cpu_hp_is_online(CPU3_CORE_ID);

	CLI_LOGI("cpu vote test: start\r\n");

	if (bk_cpu_hp_is_online(CPU3_CORE_ID)) {
		if ((bk_cpu_hp_offline_direct(CPU3_CORE_ID) != BK_OK) ||
	...
	/* AND: releasing one voter while another holds keeps cpu3 online. */
	ret = bk_cpu_hp_vote_offline(a);
	VOTE_CHECK((ret == BK_OK) && bk_cpu_hp_is_online(CPU3_CORE_ID), "A offline -> cpu3 still online");
	VOTE_CHECK(bk_cpu_hp_vote_get_online_count() == 1, "online_count == 1");

	/* AND: last release powers cpu3 down. */
	ret = bk_cpu_hp_vote_offline(b);
	VOTE_CHECK((ret == BK_OK) && cli_cpu_hp_vote_wait_online(0, CPU_HP_VOTE_TEST_TIMEOUT_MS),
		"B offline -> cpu3 offline (all released)");
	VOTE_CHECK(bk_cpu_hp_vote_get_online_count() == 0, "online_count == 0");

	/* Unregistering the last online voter must release its vote and offline cpu3. */
	ret = bk_cpu_hp_vote_online(a);
	VOTE_CHECK((ret == BK_OK) && cli_cpu_hp_vote_wait_online(1, CPU_HP_VOTE_TEST_TIMEOUT_MS),
		"A online again -> cpu3 online");
	ret = bk_cpu_hp_vote_unregister(a);
	a = NULL;
	VOTE_CHECK((ret == BK_OK) && cli_cpu_hp_vote_wait_online(0, CPU_HP_VOTE_TEST_TIMEOUT_MS),
		"unregister online voter -> cpu3 offline");
...
#define VOTE_BATCH_CAP   (64)   /* try more than any expected pool size */
		...
		VOTE_CHECK((bk_cpu_hp_vote_get_online_count() == 0) &&
			!bk_cpu_hp_is_online(CPU3_CORE_ID),
			"batch voters registered, cpu3 stays offline");
		...
		if (n > 0) {
			...
			VOTE_CHECK((ret == BK_OK) &&
				...
				"last batch voter online -> cpu3 online");
			ret = bk_cpu_hp_vote_offline(many[n - 1]);
			VOTE_CHECK((ret == BK_OK) &&
				cli_cpu_hp_vote_wait_online(0, CPU_HP_VOTE_TEST_TIMEOUT_MS),
				"last batch voter offline -> cpu3 offline");
		}
...
static void cli_cpu_hotplug_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "busy-test") == 0) {
		...
		if (ret != expected_ret) {
			recover_ret = bk_cpu_hp_online_direct(CPU3_CORE_ID);
...
static const struct cli_command s_cpu_hotplug_commands[] = {
	{"cpu", "cpu {list|state|offline 3|online 3|irq-affinity|task-affinity|stress 3 <loops>|busy-test|gov on|off|status|gov stress <cycles>|vote reg|unreg|on|off <name>|vote status|test}", cli_cpu_hotplug_cmd},
#if CONFIG_DBG_PROBE
