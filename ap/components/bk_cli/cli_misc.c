...
void cli_misc_cache_help(void)
{
	...
	CLI_RAW_LOGI("  -flush<str><optional>: Flush cache. If addr and size are not provided, flush all cache.\n");
	CLI_RAW_LOGI("  -addr<hex><optional>: Address to flush (e.g., 0x12345678).\n");
	CLI_RAW_LOGI("  -size<hex><optional>: Size to flush (e.g., 0x1000).\n");
	CLI_RAW_LOGI("  example1: cache\n");
	CLI_RAW_LOGI("  example2: cache 0\n");
	CLI_RAW_LOGI("  example3: cache 1\n");
	CLI_RAW_LOGI("  example4: cache flush\n");
	CLI_RAW_LOGI("  example5: cache flush 0x12345678 0x1000\n");
}
...
#if CONFIG_SOC_SMP
	CLI_LOGD("system: ap smp\n");
#else
	CLI_LOGD("system: ap single core\n");
#endif
}
