...
avdk_err_t file_list_check_file_valid(const char *file_path)
{
    ...
    if (fd < 0)
    {
        LOGE("%s: File cannot open for read: %s, open ret=%d\n",
             __func__, file_path, fd);
        return AVDK_ERR_NODEV;
    }
    close(fd);
...
#elif CONFIG_FATFS
    FILINFO f;
    FRESULT res = f_stat(file_path, &f);
    if (res != FR_OK)
    {
        LOGE("%s: File does not exist or cannot access: %s, f_stat res=%d\n",
             __func__, file_path, res);
        return AVDK_ERR_NODEV;
    }

    LOGD("%s: File exists: %s, size=%lld bytes\n", __func__, file_path, (long long)f.fsize);
    ...
    if (res != FR_OK)
    {
        LOGE("%s: File cannot open for read: %s, f_open res=%d\n",
             __func__, file_path, res);
        return AVDK_ERR_NODEV;
    }
