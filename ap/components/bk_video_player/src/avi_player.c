...
#else
    ret = jpeg_decoder_decode(avi_player->jpeg_decoder_handle, (uint8_t*)avi_player->video_frame, avi_player->video_len, avi_player->framebuffer, avi_player->frame_size);
    if(ret < 0) {
        LOGE("%s %d jpeg_decoder_decode failed, ret: %d\r\n", __func__, __LINE__, ret);
        return ret;
    }
#endif
...
#else
    jpeg_decoder_init(&avi_player->jpeg_decoder_handle, VCDEC_FLEXA_MODE_NONE, NULL, NULL, NULL);
#endif
...
#else
        jpeg_decoder_deinit(avi_player->jpeg_decoder_handle);
#endif
        goto out;
    }
