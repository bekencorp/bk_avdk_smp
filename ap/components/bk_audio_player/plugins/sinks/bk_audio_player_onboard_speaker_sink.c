...
/* Main DAC source used by the audio player onboard speaker output.
 * v2 onboard speaker config keeps sample_rate/frame_size as per-dac-source
 * arrays and routes element->in from main_dac_source, so the player must keep
 * the bitmap / array index / runtime set_param all referring to the same
 * source. Manage it here in one place: change this to re-route the player. */
#define PLAYER_SPK_DAC_SOURCE       AUD_DAC_SOURCE_A2DP
#define PLAYER_SPK_DAC_SOURCE_BIT   (1 << PLAYER_SPK_DAC_SOURCE)
...
static bk_err_t play_pipeline_open(uint32_t samp_rate, uint8_t bits, uint8_t chl_num, int gain_db)
{
    ...
    onboard_spk_cfg.dac_source_bitmap = PLAYER_SPK_DAC_SOURCE_BIT;
    onboard_spk_cfg.main_dac_source   = PLAYER_SPK_DAC_SOURCE;
...
static bk_err_t play_pipeline_change_frame_info(int rate, int ch, int bits)
{
    ...
    if (BK_OK != onboard_speaker_stream_set_param(onboard_spk, rate, bits, ch, PLAYER_SPK_DAC_SOURCE))
    {
...
static int device_sink_open(audio_sink_type_t sink_type, void *param, bk_audio_player_sink_t **sink_pp)
{
    ...
    *sink_pp = sink;
    ...
    BK_LOGD(AUDIO_PLAYER_TAG, "device_sink_open complete \n");
