...
#include "cv2008_1080p_calib.h"
//#include <driver/isp_hardware.h>

#define LOGTAG "CV2008"
...
#define CV2008_GAIN_NE          0x3109

// #define CV2008_AGAIN         0x3118
// #define CV2008_DGAIN_L       0x311C
// #define CV2008_DGAINE_H      0x311D
#define CV2008_DGAIN_1          0x311C
#define CV2008_DGAIN_2          0x311D
#define CV2008_AGAIN_1          0x3118
//#define CV2008_AGAIN_2            0xB4
// #define CV2008_AGAIN_3           0xB8
// #define CV2008_AGAIN_4           0xB9

/* 0x3028: bit[0]=H_MIRROR, bit[1]=V_FLIP */
#define CV2008_REG_MIRROR_FLIP    0x3028
#define CV2008_MIRROR_BIT         (1U << 0)
#define CV2008_VFLIP_BIT          (1U << 1)

//to do 
// const uint8_t cv2008_regValTable[29][4] = {

// };

enum CV2008_REG_INDEX {
    REG_EXPTIME_H       = 0,
    REG_EXPTIME_L       = 1,
    REG_DGAIN_1         = 2,
    REG_DGAIN_2         = 3,
...
#define CV2008_1080P_30FPS_LINEAR_MODE (0)

#define CV2008_VMAX_1080P30_LINEAR (1125)

typedef struct vsiCV2008_DEVICE_S {
    vsi_u8_t i2cBus;
...
static CV2008_DEVICE_S *CV2008Dev[ISP_DEV_CNT][ISP_PORT_CNT] = {0};

static ISP_CALIB_DATA_S * CV2008_1080P_CalibParam_dynamic = NULL;
...
static const cv2008_gain_lut_t s_cv2008_gain_lut[] = {
...
static CV2008_DEVICE_S *CV2008_GetSensorDev(ISP_PORT IspPort)
{
    if (CV2008Dev[IspPort.devId][IspPort.portId] == NULL)
    {
        CV2008Dev[IspPort.devId][IspPort.portId] = os_malloc(sizeof(CV2008_DEVICE_S));
        if (CV2008Dev[IspPort.devId][IspPort.portId] == NULL)
        {
            LOGE("%s %d CV2008Dev[%d][%d] malloc failed \r\n", __func__, __LINE__, IspPort.devId, IspPort.portId);
            return NULL;
        }
        os_memset(CV2008Dev[IspPort.devId][IspPort.portId], 0 , sizeof(CV2008_DEVICE_S));
    }

    return CV2008Dev[IspPort.devId][IspPort.portId];
}

static int CV2008_SetStream(ISP_PORT IspPort, vsi_bool_t stream);

static int CV2008_InitRegInfo(ISP_PORT IspPort)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    if (pCV2008Dev == NULL)
    {
        LOGE("%s %d failed\n", __func__, __LINE__);
        return BK_FAIL;
    ...
    ISP_SNS_REGS_INFO_S *pSnsRegsInfo = &pCV2008Dev->snsRegsInfo;
    pSnsRegsInfo->snsDev = pCV2008Dev->i2cBus;

    pSnsRegsInfo->addrByteNum = pCV2008Dev->i2cAttr.reg_bytes;
    pSnsRegsInfo->dataByteNum = pCV2008Dev->i2cAttr.data_bytes;
    pSnsRegsInfo->slaveAddr   = pCV2008Dev->i2cAttr.slave_addr;
    pSnsRegsInfo->regCnt = 5;
    pSnsRegsInfo->delayMax = 2;

    pSnsRegsInfo->snsData[REG_EXPTIME_H].delayFrameNum = 2;
    pSnsRegsInfo->snsData[REG_EXPTIME_H].regAddr = CV2008_EXPTIME_H;
    pSnsRegsInfo->snsData[REG_EXPTIME_L].delayFrameNum = 2;
    pSnsRegsInfo->snsData[REG_EXPTIME_L].regAddr = CV2008_EXPTIME_L;

    pSnsRegsInfo->snsData[REG_DGAIN_1].delayFrameNum = 2;
    pSnsRegsInfo->snsData[REG_DGAIN_1].regAddr = CV2008_DGAIN_1;
    pSnsRegsInfo->snsData[REG_DGAIN_2].delayFrameNum = 2;
    pSnsRegsInfo->snsData[REG_DGAIN_2].regAddr = CV2008_DGAIN_2;

    pSnsRegsInfo->snsData[REG_AGAIN_1].delayFrameNum = 2;
    pSnsRegsInfo->snsData[REG_AGAIN_1].regAddr = CV2008_AGAIN_1;
    //pSnsRegsInfo->snsData[REG_AGAIN_2].delayFrameNum = 2;
    //pSnsRegsInfo->snsData[REG_AGAIN_2].regAddr = CV2008_AGAIN_2;
    // pSnsRegsInfo->snsData[REG_AGAIN_3].delayFrameNum = 2;
    // pSnsRegsInfo->snsData[REG_AGAIN_3].regAddr = CV2008_AGAIN_3;
    // pSnsRegsInfo->snsData[REG_AGAIN_4].delayFrameNum = 2;
    // pSnsRegsInfo->snsData[REG_AGAIN_4].regAddr = CV2008_AGAIN_4;

    return BK_OK;
}

static int CV2008_Init(ISP_PORT IspPort, vsi_u8_t snsDev)
{
    if (CV2008_1080P_CalibParam_dynamic == NULL)
    {
        ...
        os_memcpy(CV2008_1080P_CalibParam_dynamic, &CV2008_1080P_CalibParam, sizeof(CV2008_1080P_CalibParam));
    }
    ...
    CV2008_SetStream(IspPort, 0);

    return  BK_OK;
}

static int CV2008_Exit(ISP_PORT IspPort)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    if (pCV2008Dev == NULL)
    {
...
static int CV2008_WriteReg(ISP_PORT IspPort, vsi_u32_t addr, vsi_u32_t data)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    if (pCV2008Dev == NULL)
    {
    ...
    vsi_u8_t i2cBus = pCV2008Dev->i2cBus;
    vsios_i2c_attr_t *pI2cAttr = &pCV2008Dev->i2cAttr;
...
static int CV2008_ReadReg(ISP_PORT IspPort, vsi_u32_t addr, vsi_u32_t *pData)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    if (pCV2008Dev == NULL)
    {
...
static int CV2008_InitAeDefault(ISP_PORT IspPort)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    if (pCV2008Dev == NULL)
    {
    ...
    AE_SNS_DEFAULT_S *pAeSnsDft = &pCV2008Dev->aeDefault;

    switch (pCV2008Dev->snsModeId) {
        case CV2008_1080P_30FPS_LINEAR_MODE:
            pAeSnsDft->fullLinesMax = 0xFFFF;
            pAeSnsDft->fullLinesStd = CV2008_VMAX_1080P30_LINEAR;
            ...
            pAeSnsDft->maxIntLine  = (pAeSnsDft->fullLines - 10) / 2;
...
static int CV2008_SetMode(ISP_PORT IspPort, ISP_SNS_MODE_S *pSnsMode)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    if (pCV2008Dev == NULL)
    {
    ...
    if ((pSnsMode->width == pCV2008Dev->snsMode.width) &&
        (pSnsMode->height == pCV2008Dev->snsMode.height) &&
        (pSnsMode->hdrMode == pCV2008Dev->snsMode.hdrMode) &&
        (pSnsMode->stichMode == pCV2008Dev->snsMode.stichMode)) {
        return BK_OK;
    }
    ...
    if ((pSnsMode->width  == 1920) &&
        ...
        // CV2008_Linear1920x1080Init(pCV2008Dev->i2cBus, &pCV2008Dev->i2cAttr);
        LOGI("cv2008 1080p set mode end \r\n");
        os_memcpy(&pCV2008Dev->snsMode, pSnsMode, sizeof(*pSnsMode));
        pCV2008Dev->snsModeId = CV2008_1080P_30FPS_LINEAR_MODE;
        CV2008_InitAeDefault(IspPort);
    }
    /*else if ((pSnsMode->width  == 640) &&
        (pSnsMode->height == 480) &&
        (pSnsMode->hdrMode == HDR_MODE_LINEAR)) {
        // CV2008_Linear1920x1080Init(pCV2008Dev->i2cBus, &pCV2008Dev->i2cAttr);
        os_printf("cv2008 480p set mode end \r\n");
        os_memcpy(&pCV2008Dev->snsMode, pSnsMode, sizeof(*pSnsMode));
        pCV2008Dev->snsModeId = CV2008_1080P_30FPS_LINEAR_MODE;
        CV2008_InitAeDefault(IspPort);
    } */else {
        LOGI("cv2008 custom set mode end ~~~\r\n");
        os_memcpy(&pCV2008Dev->snsMode, pSnsMode, sizeof(*pSnsMode));
        pCV2008Dev->snsModeId = CV2008_1080P_30FPS_LINEAR_MODE;
        CV2008_InitAeDefault(IspPort);
    }
...
void csi_read_cv2008(unsigned int addr)
{
    vsi_u32_t new_value = 0x55;
    CV2008_ReadReg(g_port, addr, &new_value);
    LOGI("0x%x is 0x%x\n", addr, new_value);
}

void csi_write_cv2008(unsigned int addr, unsigned int value)
{
    CV2008_WriteReg(g_port, addr, value);
    LOGI("0x%x to 0x%x\n", addr, value);
}
...
static int CV2008_SetStream(ISP_PORT IspPort, vsi_bool_t stream)
{
    ...
    if (stream) {
        CV2008_WriteReg(IspPort, 0x3000, 0x00);
        // CV2008_WriteReg(IspPort, 0x3012, 1);
    } else {
        CV2008_WriteReg(IspPort, 0x3000, 0x01);
    }

    pCV2008Dev->stream = stream;
...
static int CV2008_SetIspDefault(ISP_PORT IspPort)
{
    VSI_MPI_ISP_SetCalib(IspPort, CV2008_1080P_CalibParam_dynamic);

    return  BK_OK;
}

static int CV2008_InitIspSnsFunc(ISP_SNS_FUNC_S *pIspSnsFunc)
{
    pIspSnsFunc->pfnSensorInit    = CV2008_Init;
    pIspSnsFunc->pfnSensorExit    = CV2008_Exit;
    pIspSnsFunc->pfnWriteReg      = CV2008_WriteReg;
    pIspSnsFunc->pfnReadReg       = CV2008_ReadReg;
    pIspSnsFunc->pfnSetMode       = CV2008_SetMode;
    pIspSnsFunc->pfnSetStream     = CV2008_SetStream;
    pIspSnsFunc->pfnSetIspDefault = CV2008_SetIspDefault;

    return BK_OK;
}

static int CV2008_GetAeDefault(ISP_PORT IspPort, AE_SNS_DEFAULT_S *pAeSnsDft)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    if (pCV2008Dev == NULL)
    {
    ...
    os_memcpy(pAeSnsDft, &pCV2008Dev->aeDefault, sizeof(*pAeSnsDft));

    return BK_OK;
}

#if 0
static int CV2008_SetFps(ISP_PORT IspPort, vsi_u32_t fps)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    AE_SNS_DEFAULT_S *pAeSnsDft = &pCV2008Dev->aeDefault;
    ISP_SNS_REGS_INFO_S *pSnsRegsInfo = &pCV2008Dev->snsRegsInfo;
    ...
    switch(pCV2008Dev->snsModeId) {
        case CV2008_1080P_30FPS_LINEAR_MODE:
            if ((fps <= 30 * ISP_SNS_FPS_ACCU) && (fps >= 0.5 * ISP_SNS_FPS_ACCU)) {
                vts = CV2008_VMAX_1080P30_LINEAR * 30 * ISP_SNS_FPS_ACCU / fps;
            } else {
                return BK_FAIL;
            }
...
static int CV2008_SlowFrameRate(ISP_PORT IspPort, vsi_u32_t fullLines)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    AE_SNS_DEFAULT_S *pAeSnsDft = &pCV2008Dev->aeDefault;
    ISP_SNS_REGS_INFO_S *pSnsRegsInfo = &pCV2008Dev->snsRegsInfo;
...
static void CV2008_CalcGain(vsi_u32_t *pGain, vsi_u8_t *pAgainReg, vsi_u16_t *pDGainReg, vsi_u8_t *pConvReg)
{
    const cv2008_gain_lut_t *selected = &s_cv2008_gain_lut[0];
    const vsi_u32_t target_gain = *pGain;

    (void)pConvReg;

    for (vsi_u32_t i = 1; i < ARRAY_SIZE(s_cv2008_gain_lut); i++) {
        if (target_gain < s_cv2008_gain_lut[i].gain) {
            break;
        }
        selected = &s_cv2008_gain_lut[i];
    }
...
static int CV2008_GainUpdate(ISP_PORT IspPort, vsi_u32_t *pAgain, vsi_u32_t *pDgain)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    ISP_SNS_REGS_INFO_S *pSnsRegsInfo = &pCV2008Dev->snsRegsInfo;
    vsi_u32_t gain;
    ...
    switch(pCV2008Dev->snsModeId) {
        case CV2008_1080P_30FPS_LINEAR_MODE:
            gain = (*pAgain);
            CV2008_CalcGain(&gain, &againReg, &dGainReg, &convReg);
            *pAgain = gain;
            *pDgain = 1024;
...
static int CV2008_SetExpRatio(ISP_PORT IspPort, ISP_EXP_RATIO_S *pExpRatio)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    AE_SNS_DEFAULT_S *pAeSnsDft = &pCV2008Dev->aeDefault;
...
static int CV2008_IntTimeUpdate(ISP_PORT IspPort, vsi_u32_t *pIntLine)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    if (pCV2008Dev == NULL)
    {
    ...
    if((pAeSnsDft->fullLines / 2 - 5) > (*pIntLine))
    {
        shutter = pAeSnsDft->fullLines / 2 - (*pIntLine);
    } else {
    	shutter = 5;
    }
    if(shutter < 5)
        shutter = 5;
    if(shutter > pAeSnsDft->fullLines / 2 - 1)
        shutter = pAeSnsDft->fullLines / 2 - 1;
...
static int CV2008_GetSnsRegInfo(ISP_PORT IspPort, ISP_SNS_REGS_INFO_S *pSnsRegsInfo)
{
    CV2008_DEVICE_S *pCV2008Dev = CV2008_GetSensorDev(IspPort);
    if (pCV2008Dev == NULL)
    {
    ...
    os_memcpy(pSnsRegsInfo, &pCV2008Dev->snsRegsInfo, sizeof(*pSnsRegsInfo));

    return BK_OK;
}

static int CV2008_InitAeSnsFunc(AE_SNS_FUNC_S *pAeSnsFunc)
{
    pAeSnsFunc->pfnGetAeDefault  = CV2008_GetAeDefault;
    pAeSnsFunc->pfnSetFps        = NULL;
    pAeSnsFunc->pfnSlowFrameRate = NULL;
    pAeSnsFunc->pfnIntTimeUpdate = CV2008_IntTimeUpdate;
    pAeSnsFunc->pfnGainUpdate    = CV2008_GainUpdate;
    pAeSnsFunc->pfnSetExpRatio   = NULL;
    pAeSnsFunc->pfnGetSnsRegInfo = CV2008_GetSnsRegInfo;

    return BK_OK;
}

ISP_SNS_OBJ_S snsCV2008Obj = {
    .pfnInitIspSnsFunc = CV2008_InitIspSnsFunc,
    .pfnInitAeSnsFunc  = CV2008_InitAeSnsFunc,
};
...
#define CHIP_ID_VAL_LB (0x08)       // 芯片ID低字节值 ('S' 的 ASCII)
...
#define CV2008_PCLK (DEFAULT_FRAME_LEN * DEFAULT_LINE_LEN * 30)
...
#define CV2008_TABLE_SIZE(table) (sizeof(table) / 4)

avdk_err_t cv2008_detect(bk_camera_sensor_handle_t *handle, bk_camera_sensor_config_t *config);

bool cv2008_read_flag = false;
void mipi_phy_term_set(uint32_t v1, uint32_t v2);

const uint16_t sensor_cv2008_init_table[][2] = 
{
//information:24.0,66,2,792.0,39.6,1100,1200,33.333333333333336,29.999999999999996
//PLL freq: 792.0Mhz
//PCLK: 39.6Mhz
    {0x3808, 0x4B},
    {0x3809, 0x00},
...
int cv2008_init(bk_camera_sensor_ctlr_t *controller)
{
    ...
    uint32_t size = CV2008_TABLE_SIZE(sensor_cv2008_init_table);

    for (int i = 0; i < size; i++)
    {
        bus->write16(bus, sensor_cv2008_init_table[i][0], sensor_cv2008_init_table[i][1]);
    }
...
int cv2008_set_fps_test(bk_camera_sensor_ctlr_t *controller, uint16_t hts, uint16_t vts)
{
    bk_camera_csi_sensor_t *csi_sensor = __containerof(controller, bk_camera_csi_sensor_t, ops);
    AVDK_RETURN_ON_FALSE(csi_sensor, AVDK_ERR_INVAL, TAG, "csi sensor is NULL");
    bk_camera_bus_t *bus = csi_sensor->config.bus;
...
static uint16_t s_cv2008_out_width;
static uint16_t s_cv2008_out_height;
static bool s_cv2008_hmirror;
static bool s_cv2008_vflip;

static void cv2008_write_crop_regs(bk_camera_bus_t *bus, uint16_t width, uint16_t height)
{
...
avdk_err_t cv2008_set_ppi(bk_camera_sensor_ctlr_t *controller, uint16_t width, uint16_t height)
{
    bk_camera_csi_sensor_t *csi_sensor = __containerof(controller, bk_camera_csi_sensor_t, ops);
    AVDK_RETURN_ON_FALSE(csi_sensor, AVDK_ERR_INVAL, TAG, "csi sensor is NULL");
    bk_camera_bus_t *bus = csi_sensor->config.bus;
    ...
    s_cv2008_out_width = width;
    s_cv2008_out_height = height;
    bk_mipi_csi_controller_init(width, height, 0x2b);
    cv2008_write_crop_regs(bus, width, height);

    return AVDK_ERR_OK;
}

avdk_err_t cv2008_set_fps(bk_camera_sensor_ctlr_t *controller, uint16_t fps)
{
    bk_camera_csi_sensor_t *csi_sensor = __containerof(controller, bk_camera_csi_sensor_t, ops);
    AVDK_RETURN_ON_FALSE(csi_sensor, AVDK_ERR_INVAL, TAG, "csi sensor is NULL");
    bk_camera_bus_t *bus = csi_sensor->config.bus;
    ...
    {
        // uint16_t vts = CV2008_PCLK / DEFAULT_LINE_LEN / fps;
        // bus->write16(bus, 0x301D, UINT16_HB(vts));
        // bus->write16(bus, 0x301C, UINT16_LB(vts));
        uint16_t line_len = CV2008_PCLK / DEFAULT_FRAME_LEN / fps;
...
avdk_err_t cv2008_ctrl(bk_camera_sensor_ctlr_t *controller, uint8_t cmd, uint16_t addr, uint8_t val)
{
    bk_camera_csi_sensor_t *csi_sensor = __containerof(controller, bk_camera_csi_sensor_t, ops);
    AVDK_RETURN_ON_FALSE(csi_sensor, AVDK_ERR_INVAL, TAG, "csi sensor is NULL");
    bk_camera_bus_t *bus = csi_sensor->config.bus;
    ...
    if (cmd == 1) // sensor reg write
    {
        uint8_t dump_val;
        bus->read16(bus, addr, &dump_val);
        os_printf("cv2008 {%04x, %02x} -> {%04x, %02x} \r\n", addr, dump_val, addr, val);
...
static void cv2008_apply_mirror_reg(bk_camera_bus_t *bus)
{
    uint8_t val;
    ...
    bus->read16(bus, CV2008_REG_MIRROR_FLIP, &val);
    val &= (uint8_t)~(CV2008_MIRROR_BIT | CV2008_VFLIP_BIT);
    if (s_cv2008_hmirror)
    {
        val |= CV2008_MIRROR_BIT;
    }
    if (s_cv2008_vflip)
    {
        val |= CV2008_VFLIP_BIT;
    }
    bus->write16(bus, CV2008_REG_MIRROR_FLIP, val);

    if (s_cv2008_out_width > 0 && s_cv2008_out_height > 0)
    {
        cv2008_write_crop_regs(bus, s_cv2008_out_width, s_cv2008_out_height);
    }
}

static avdk_err_t cv2008_set_hmirror(bk_camera_sensor_ctlr_t *controller, bool enable)
{
    bk_camera_csi_sensor_t *csi_sensor = __containerof(controller, bk_camera_csi_sensor_t, ops);
    AVDK_RETURN_ON_FALSE(csi_sensor, AVDK_ERR_INVAL, TAG, "csi sensor is NULL");

    s_cv2008_hmirror = enable;
    cv2008_apply_mirror_reg(csi_sensor->config.bus);
    return AVDK_ERR_OK;
}

static avdk_err_t cv2008_set_vflip(bk_camera_sensor_ctlr_t *controller, bool enable)
{
    bk_camera_csi_sensor_t *csi_sensor = __containerof(controller, bk_camera_csi_sensor_t, ops);
    AVDK_RETURN_ON_FALSE(csi_sensor, AVDK_ERR_INVAL, TAG, "csi sensor is NULL");

    s_cv2008_vflip = enable;
    cv2008_apply_mirror_reg(csi_sensor->config.bus);
    return AVDK_ERR_OK;
}

avdk_err_t cv2008_set_format(bk_camera_sensor_ctlr_t *controller, bk_camera_sensor_format_t *format)
{
    bk_camera_csi_sensor_t *csi_sensor = __containerof(controller, bk_camera_csi_sensor_t, ops);
    AVDK_RETURN_ON_FALSE(csi_sensor, AVDK_ERR_INVAL, TAG, "csi sensor is NULL");
    ...
    cv2008_apply_mirror_reg(csi_sensor->config.bus);
    cv2008_set_ppi(controller, format->width, format->height);
    cv2008_set_fps(controller, format->fps);
...
const csi_sensor_config_t csi_sensor_cv2008 = { //???
    .name = "cv2008",
    .clk = MCLK_24M,
    .mipi_data_type = 0x2b,
    ...
    .default_fps = 30,
    .id = ID_CV2008,
    .address = (CV2008_WRITE_ADDRESS >> 1),
    .init = cv2008_init,
    .detect = cv2008_detect,
    .set_ppi = cv2008_set_ppi,
    .set_fps = cv2008_set_fps,
    .reg_ctrl = cv2008_ctrl,
};


const ISP_PUB_ATTR_S cv2008_mipi_linear_attr = {    //???
    .pSnsObj      = &snsCV2008Obj,
...
void *cv2008_get_sensor_object(bk_camera_sensor_ctlr_t *controller)
{
    bk_camera_csi_sensor_t *csi_sensor = __containerof(controller, bk_camera_csi_sensor_t, ops);
    AVDK_RETURN_ON_FALSE(csi_sensor, NULL, TAG, "csi sensor is NULL");
    return (void*)&snsCV2008Obj;
}

void *cv2008_get_sensor_cfg(bk_camera_sensor_ctlr_t *controller)
{
    bk_camera_csi_sensor_t *csi_sensor = __containerof(controller, bk_camera_csi_sensor_t, ops);
    AVDK_RETURN_ON_FALSE(csi_sensor, NULL, TAG, "csi sensor is NULL");
...
static const bk_camera_sensor_format_t cv2008_format_array[] = {
    {
        .width = 1280,
        .height = 720,
        .fps = 30,
...
static avdk_err_t cv2008_query_support_formats(bk_camera_sensor_ctlr_t *controller, bk_camera_sensor_format_array_t *format_array)
{
    AVDK_RETURN_ON_FALSE(format_array, AVDK_ERR_INVAL, TAG, "format array is NULL");
    format_array->format_array = &cv2008_format_array[0];
    format_array->size = ARRAY_SIZE(cv2008_format_array);
    return AVDK_ERR_OK;
}
...
avdk_err_t cv2008_detect(bk_camera_sensor_handle_t *handle, bk_camera_sensor_config_t *config)
{
    ...
    config->bus->write_address = CV2008_WRITE_ADDRESS;

	s_cv2008_hmirror = true;
	s_cv2008_vflip = false;
    ...
    csi_sensor->ops.init = cv2008_init;
    csi_sensor->ops.set_format = cv2008_set_format;
    csi_sensor->ops.reg_ctrl = cv2008_ctrl;
    csi_sensor->ops.set_hmirror = cv2008_set_hmirror;
    csi_sensor->ops.set_vflip = cv2008_set_vflip;
    csi_sensor->ops.get_sensor_object = cv2008_get_sensor_object;
    csi_sensor->ops.get_sensor_cfg = cv2008_get_sensor_cfg;
    csi_sensor->ops.query_support_formats = cv2008_query_support_formats;

    csi_sensor->isp_pub_attr = &cv2008_mipi_linear_attr;
    csi_sensor->sensor_config = &csi_sensor_cv2008;
...
BK_CAMERA_SENSOR_DETECT_SECTION(cv2008_detect, CSI_CAMERA_PORT);
