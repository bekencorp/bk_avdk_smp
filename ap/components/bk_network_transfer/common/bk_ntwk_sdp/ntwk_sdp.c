...
int ntwk_sdp_generate(const char *name, uint32_t cmd_port, uint32_t img_port, uint32_t aud_port, const char *ble_mac)
{
    uint32 adv_len = ADV_ALLOC_LEN;
    const char *adv_temp =                                                                      \
            "{\"service\":\"%s\", \"cmd\":\"%u\",\"img\":\"%u\", \"aud\":\"%u\", \"ble_mac\":\"%s\"}";
...
int ntwk_sdp_start(const char *name, uint32_t cmd_port, uint32_t img_port, uint32_t aud_port)
{
    ...
    ntwk_sdp_generate(name, cmd_port, img_port, aud_port, ble_mac_str);
