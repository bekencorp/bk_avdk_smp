...
void stack_thread_show(uint32_t *total_cnt, uint32_t *used_cnt)
{
	...
	uint8_t core_name[8] = "CoreX\0";
#endif

	BK_DUMP_OUT("%-22s   %-10s   %-22s   %-10s   %-13s   %-11s\r\n", "task", "stack_size", "address", "peak_used", "current_used", "water");
...
#if (CONFIG_FREERTOS_SMP)
		if (pxTaskItem->xCoreID == SMP_CORE0_ID) {
			core_name[4] = '0';
		} else if (pxTaskItem->xCoreID == SMP_CORE1_ID) {
			core_name[4] = '1';
		}
		BK_DUMP_OUT("%s:%-16s   %-10d   0x%-08x- 0x%-08x   %-10d   %-13d   %-11d\r\n",
					core_name,
