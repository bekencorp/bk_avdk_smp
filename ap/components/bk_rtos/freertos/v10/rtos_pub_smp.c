...
BaseType_t xTaskCreateInHsram( TaskFunction_t pxTaskCode,
                        const char * const pcName, /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
                        const configSTACK_DEPTH_TYPE usStackDepth,
                        void * const pvParameters,
                        UBaseType_t uxPriority,
...
extern void platform_setup_systick(uint32_t cpu_clock_hz);
...
{
	bk_err_t ret;
	ret = xTaskCreateInHsram( (native_thread_t)function,
                                name,
                                (unsigned short) (stack_size/sizeof( portSTACK_TYPE )),
                                (void *)arg,
                                BK_PRIORITY_TO_NATIVE_PRIORITY(priority),
...
bk_err_t rtos_create_hsram_thread( beken_thread_t* thread, uint8_t priority, const char* name, 
                        beken_thread_function_t function, uint32_t stack_size, beken_thread_arg_t arg )
{
    /* Limit priority to default lib priority */
    if ( priority > RTOS_HIGHEST_PRIORITY )
    ...
    if( pdPASS == xTaskCreateInHsram( (native_thread_t)function, 
                                      name, 
                                     (unsigned short) (stack_size/sizeof( portSTACK_TYPE )), 
                                     (void *)arg, 
                                     BK_PRIORITY_TO_NATIVE_PRIORITY(priority), 
...
{
	return xPortGetHsramTotalHeapSize();
}

size_t rtos_get_hsram_free_heap_size(void)
{
	return xPortGetHsramFreeHeapSize();
}
...
{
	platform_setup_systick(cpu_clock_hz);
}
