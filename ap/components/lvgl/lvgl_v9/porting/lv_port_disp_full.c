...
static bk_err_t lv_full_frame_buffer_free_cb(void *frame)
{
...
void lv_disp_flush_for_full_mode(lv_display_t * disp_drv, const lv_area_t * area, uint8_t * px_map)
{
    ...
    vnd_data->config.flush_cb(vnd_data->config.args, px_map, lv_full_frame_buffer_free_cb);
