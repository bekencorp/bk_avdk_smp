...
#if LV_GIF_USE_PSRAM
static void * gif_psram_buf_malloc(size_t size, lv_color_format_t color_format);
static void gif_psram_buf_free(void * buf);
static lv_draw_buf_handlers_t * gif_get_psram_draw_buf_handlers(void);
#endif
...
#if LV_GIF_USE_PSRAM
static lv_draw_buf_handlers_t gif_psram_draw_buf_handlers;
static bool gif_psram_draw_buf_handlers_inited;
#endif
...
#if LV_GIF_USE_PSRAM
    gifobj->draw_buf = lv_draw_buf_create_ex(gif_get_psram_draw_buf_handlers(), width, height, gifobj->color_format,
                                             LV_STRIDE_AUTO);
#else
...
static lv_draw_buf_handlers_t * gif_get_psram_draw_buf_handlers(void)
{
    if(!gif_psram_draw_buf_handlers_inited) {
        lv_draw_buf_init_with_default_handlers(&gif_psram_draw_buf_handlers);
        gif_psram_draw_buf_handlers.buf_malloc_cb = gif_psram_buf_malloc;
        gif_psram_draw_buf_handlers.buf_free_cb = gif_psram_buf_free;
        gif_psram_draw_buf_handlers_inited = true;
    }

    return &gif_psram_draw_buf_handlers;
}
#endif
