...
#define TAG "bk_flexa_mjpegd_gpu_bond"
...
static private_jpeg_decode_flexa_ctlr_t *
jpeg_flexa_ctrl_from_handle(bk_jpeg_decode_ctlr_handle_t handle)
{
	if (handle == NULL) {
		return NULL;
	}
...
static void gpu_bond_mjpegd_decode_error(uint32_t reason, void *args)
{
	...
	(void)reason;
	if (in_stream == NULL || in_stream->bond_config == NULL || in_stream->handle == NULL) {
		return;
	}
	...
	jpeg = (bk_jpeg_decode_ctlr_handle_t)in_stream->handle;
					  ...
					  bk_gpu_ctlr_handle_t gpu)
{
	...
	bond_new->in_stream_type = BK_FLEXA_TYPE_MJPEGD;
	...
	ret = bk_jpeg_decode_ioctl((bk_jpeg_decode_ctlr_handle_t)in_stream->handle,
				   BK_JPEG_DECODE_IOCTL_REGISTER_BOND, in_stream);
	if (ret != AVDK_ERR_OK) {
		LOGE("%s REGISTER_BOND JPEG failed: %d\r\n", __func__, ret);
		goto error;
	}
	jpeg_registered = 1;
	...
	if (ret != AVDK_ERR_OK) {
		LOGE("%s REGISTER_BOND GPU failed: %d\r\n", __func__, ret);
		goto error;
	}
	gpu_registered = 1;
...
void bk_flexa_mjpegd_gpu_bond_stop(void *bond)
{
	...
	if (in_stream != NULL && in_stream->handle != NULL && out_stream != NULL) {
		...
		(void)bk_jpeg_decode_ioctl((bk_jpeg_decode_ctlr_handle_t)in_stream->handle,
					   BK_JPEG_DECODE_IOCTL_PORT_SET_RD_PTR, &rd_cmd);
		(void)bk_jpeg_decode_ioctl((bk_jpeg_decode_ctlr_handle_t)in_stream->handle,
