...
#define ETHOSU_LOG_SEVERITY ETHOSU_LOG_DEBUG//ETHOSU_LOG_WARN
#endif
...
// Define log tag for Ethos-U driver
#define ETHOSU_LOG_TAG "ETHOSU"

// Map LOG_COMMON to project's log system
// LOG_COMMON receives stream parameter (s) and format string
// For stderr, use BK_LOGE; for stdout, check format string prefix to determine log level
// Format string prefix check: "W: " -> BK_LOGW, "I: " -> BK_LOGI, "D: " -> BK_LOGD, others -> BK_LOGI
#if ETHOSU_LOG_ENABLE
#define LOG_COMMON(s, f, ...) \
    ((s) == stderr ? BK_LOGE(ETHOSU_LOG_TAG, f, ##__VA_ARGS__) : \
     (f[0] == 'W' && f[1] == ':' && f[2] == ' ' ? BK_LOGW(ETHOSU_LOG_TAG, f, ##__VA_ARGS__) : \
      (f[0] == 'I' && f[1] == ':' && f[2] == ' ' ? BK_LOGI(ETHOSU_LOG_TAG, f, ##__VA_ARGS__) : \
       (f[0] == 'D' && f[1] == ':' && f[2] == ' ' ? BK_LOGD(ETHOSU_LOG_TAG, f, ##__VA_ARGS__) : \
        BK_LOGI(ETHOSU_LOG_TAG, f, ##__VA_ARGS__)))))
#else
#define LOG_COMMON(s, f, ...)
#endif
