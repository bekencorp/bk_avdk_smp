...
static MbedTLSSession *sl_ssl_handle_create(void)
{
	MbedTLSSession *tls_session = os_malloc(sizeof(MbedTLSSession));

	if (!tls_session) {
		bk_printf("no memory for tls_session struct\n");
		return NULL;
	}
	os_memset(tls_session, 0, sizeof(MbedTLSSession));

	return tls_session;
}

static int sl_ssl_connect_create(MbedTLSSession *session)
{
	char *pers = "beken";
	int ret = 0;

	if ((ret = mbedtls_client_init(session, (void *)pers, os_strlen(pers))) != 0) {
...
int ssl_txdat_sender(MbedTLSSession *tls_session, int len, char *data)
{
	...
	if (!data || !len || !tls_session)
		return -1;

#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
	max_len = mbedtls_ssl_get_output_max_frag_len(&tls_session->ssl);
	if (max_len > 1024)
		max_len = 1024;
#endif
	sd_len = len;
...
int ssl_read_data(MbedTLSSession *session, unsigned char *msg, unsigned int mlen,
		  unsigned int timeout_ms)
{
	...
	do {
		if (mbedtls_ssl_get_bytes_avail(&session->ssl) == 0) {
			...
			if (!FD_ISSET(session->server_fd.fd, &readfds))
				return 0;
		}
		if (mlen == 0)
			return 0;
	...
	len = ret;
	os_memcpy(msg, session->buffer, len);
