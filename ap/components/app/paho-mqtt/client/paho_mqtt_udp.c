...
_mqtt_start:
    ...
    while (1)
    {
        ...
        if (((tick_now - c->tick_ping) / bk_get_ticks_per_second()) > (c->keepAliveInterval - 5))
        {
            timeout.tv_sec = 1;
        ...
        {
            timeout.tv_sec = c->keepAliveInterval - 10 - (tick_now - c->tick_ping) / bk_get_ticks_per_second();
