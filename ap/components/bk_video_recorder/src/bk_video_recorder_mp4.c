...
/* Default to the original recording behavior: rate limiting keeps H.264
 * IDR/I frames but can skip non-key frames. Set to 1 to keep those H.264
 * non-key frames too when rate limiting would otherwise drop them. */
#ifndef VIDEO_RECORDER_MP4_H264_KEEP_ALL_FRAMES
#define VIDEO_RECORDER_MP4_H264_KEEP_ALL_FRAMES 1
#endif
    ...
    else
    {
        ...
        else
        {
            if (controller->config.record_format == VIDEO_RECORDER_FORMAT_H264)
            {
                ...
                {
#if VIDEO_RECORDER_MP4_H264_KEEP_ALL_FRAMES
                    LOGV("%s: Saving H264 P/B-frame despite short interval (interval=%u ms, min=%u ms)\n",
                         __func__, time_since_last_saved, min_interval_ms);
                    should_save_frame = true;
#else
