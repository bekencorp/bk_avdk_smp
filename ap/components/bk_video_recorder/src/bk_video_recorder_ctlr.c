...
static avdk_err_t video_recorder_ctlr_start(bk_video_recorder_ctlr_handle_t handler, char *file_path, uint32_t record_type)
{
    ...
    // MP4 + fixed-frame audio (PCM/G.711/G.722): enforce fixed 20ms packetization at recording layer.
    // This keeps mp4lib write-mode (single STTS entry) consistent and avoids variable-size packets
    // (e.g. a first 320-byte packet followed by 160-byte packets) breaking the audio timeline, which
    // shows up as repeated "MP4_write_audio: inferred duration ... differs from fixed ... forcing fixed".
    controller->mp4_audio_packetize_enable = false;
    controller->mp4_audio_packetize_frame_bytes = 0;
    controller->mp4_audio_packetize_partial_used = 0;

    if (controller->config.record_type == VIDEO_RECORDER_TYPE_MP4 &&
        (controller->config.audio_format == VIDEO_RECORDER_AUDIO_FORMAT_PCM ||
        ...
        controller->config.audio_channels > 0)
    {
        ...
        else if (controller->config.audio_format == VIDEO_RECORDER_AUDIO_FORMAT_PCM)
        {
            // Linear PCM: bytes_per_20ms = (sample_rate / 50) * channels * (bits / 8).
            uint32_t sr = controller->config.audio_rate;
            uint32_t bits = controller->config.audio_bits;
            if (bits == 0)
            {
                bits = 16; // sensible default for onboard mic PCM capture
            }
            if (sr == 0 || (sr % 50u) != 0u || (bits % 8u) != 0u)
            {
                LOGE("%s: unsupported PCM params for 20ms framing, sample_rate=%u, bits=%u\n",
                     __func__, (unsigned)sr, (unsigned)bits);
                return AVDK_ERR_INVAL;
            }
            frame_bytes = (sr / 50u) * ch * (bits / 8u);
        }
        else
        {
