                       ...
                       const unsigned char *key, size_t keybits )
{
    ...
    if ( (ctx->cipher_info == NULL) || (keybits != mbedtls_cipher_info_get_key_bitlen(ctx->cipher_info)) ) {
        return ( MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA );
    }
                                        ...
                                        unsigned int ek1bits )
{
    ...
    if ( (ctx->cipher_info == NULL) || (ek1bits != mbedtls_cipher_info_get_key_bitlen(ctx->cipher_info)) ) {
        return ( MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA );
    }
