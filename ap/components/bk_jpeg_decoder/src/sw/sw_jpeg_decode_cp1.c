...
static void software_decode_main_cp1(beken_thread_arg_t data)
{
	int ret = BK_OK;
		...
		if (ret == BK_OK)
		{
			switch (msg.event)
			{
				...
				case SOFTWARE_DECODE_SET_BYTE_ORDER:
				{
					...
					sw_dec_config->byte_order = byte_order;
...
static void create_thread_on_cp1(beken_thread_arg_t data)
{
	...
	sw_dec_config->sw_dec_thread = xTaskCreateStaticPinnedToCore( (TaskFunction_t)software_decode_main_cp1,
										"sw_dec_task",
										512,
										( void * ) NULL,
...
bk_err_t software_decode_task_close_cp1()
{
	...
	if (sw_dec_config->sw_dec_thread)
	{
		...
		software_decode_task_send_msg_cp1(SOFTWARE_DECODE_EXIT, 0);
