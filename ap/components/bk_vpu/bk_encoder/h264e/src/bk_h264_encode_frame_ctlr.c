 ...
 * enc_param.frame_done_cb. Used to live behind h264e_driver's
...
static void h264e_end_cb(void *buffer, uint32_t size, uint32_t type, uint32_t result, uint32_t param)
{
    ...
    /* Debug stats accumulation; skip when periodic debug is not enabled. */
...
static avdk_err_t h264_encode_msg_callback(void *param)
{
    ...
    ENCODE_FRAME_START;
    /*
     * Push the per-frame staging fields into the vcenc param block right
...
static avdk_err_t h264_encode_ctlr_open(bk_h264_encode_ctlr_handle_t handle)
{
    ...
    /*
     * Prime the vcenc per-instance param block. The frame-done callback is
     * wired directly to this controller via args so the cb can downcast
     * without going through a separate dispatch table.
     */
    os_memset(&control->enc_param, 0, sizeof(control->enc_param));
    control->enc_param.enc_mode = (vcenc_mode_e)BK_H264_ENCODE_FLEXA_MODE_NONE;
