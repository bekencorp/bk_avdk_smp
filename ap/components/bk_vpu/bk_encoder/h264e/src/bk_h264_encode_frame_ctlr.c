...
/*
 * H.264 frame-done callback. Registered directly with the vcenc front-end via
 * vcenc_config_t.frame_done_cb. Used to live behind h264e_driver's
 * h264e_frame_end_cb wrapper; the wrapper was removed and the debug-stats
 * accumulation that lived there was folded directly into this routine.
 */
static void h264e_end_cb(void *buffer, uint32_t size, uint32_t type, uint32_t result, uint32_t param)
{
    ...
    /* Debug stats accumulation (was inside h264e_driver wrapper). */
    if (result == BK_OK) {
...
static avdk_err_t h264_encode_msg_callback(void *param)
{
    ...
    ENCODE_FRAME_START;
    /*
     * Push the per-frame staging fields into the vcenc frame config right
     * before the kick. update_flag is cleared afterwards because vcenc latches
     * any pending IDR-interval/QP changes on this one call.
     */
    ctrl->frame_cfg.in_buffer = ctrl->pending_in_buf;
    ctrl->frame_cfg.in_lines = ctrl->pending_in_lines;
    ctrl->frame_cfg.out_buffer = ctrl->pending_out_buf;
    ctrl->frame_cfg.out_len = ctrl->pending_out_size;
    ctrl->frame_cfg.force_idr_flag = ctrl->force_idr ? 1U : 0U;
    ctrl->force_idr = false;
    ctrl->debug_info.all_frame_count++;
    vcenc_ret_e venc_ret = vcenc_h264_encode_frame(ctrl->handle, &ctrl->frame_cfg);
    ctrl->frame_cfg.update_flag = 0;
    ENCODE_FRAME_END;
...
static avdk_err_t h264_encode_ctlr_open(bk_h264_encode_ctlr_handle_t handle)
{
    ...
    LOGI("Start H.264 encoder %dx%d, mode=%d\r\n",
         control->config.width, control->config.height, BK_H264_ENCODE_FLEXA_MODE_NONE);

    /*
     * Prime the vcenc init configuration. The frame-done callback is wired
     * directly to this controller via args so the cb can downcast without
     * going through a separate dispatch table.
     */
    vcenc_config_t      common_cfg = {
        .mode          = (vcenc_mode_e)BK_H264_ENCODE_FLEXA_MODE_NONE,
        .timeout_ms    = 0,
        .frame_done_cb = h264e_end_cb,
        .slice_done_cb = NULL,
        .args          = control,
    };
    vcenc_h264_config_t h264_cfg = {
        .width        = (uint16_t)control->config.width,
        .height       = (uint16_t)control->config.height,
        .in_type      = VCENC_INPUT_NV12,
        .idr_interval = control->config.gop_frame_count,
        .slice_count  = control->config.input_flexa_cnt,
    };
    ...
    control->frame_cfg.idr_interval = control->config.gop_frame_count;

    control->handle = NULL;
    vcenc_ret_e venc_ret = vcenc_h264_init(&control->handle, &common_cfg, &h264_cfg);
    if (venc_ret != VCENC_OK) {
        LOGE("vcenc_h264_init failed: %d\r\n", venc_ret);
        return AVDK_ERR_GENERIC;
    }
    venc_ret = vcenc_h264_memalloc_register(control->handle, hw_encoder_malloc, hw_encoder_free);
    if (venc_ret != VCENC_OK) {
        LOGE("vcenc_h264_memalloc_register failed: %d\r\n", venc_ret);
        (void)vcenc_h264_deinit(control->handle);
        control->handle = NULL;
        return AVDK_ERR_GENERIC;
    }
    venc_ret = vcenc_h264_open(control->handle);
    if (venc_ret != VCENC_OK) {
    ...
    /*
     * Apply the legacy h264e_open defaults: fixed QP for both I and P, no
     * picture-RC, no bitrate cap. Callers can override later via the
     * SET_RATE_CTRL ioctl.
     */
    vcenc_rate_ctrl_t rc;
    if (vcenc_h264_get_rate_ctrl(control->handle, &rc) == VCENC_OK) {
        ...
        rc.picture_rc = 0;
        rc.bit_per_second = 0;
        (void)vcenc_h264_set_rate_ctrl(control->handle, &rc);
    }
    ...
    if (ret != BK_OK) {
        ...
        (void)vcenc_h264_close(control->handle);
        (void)vcenc_h264_deinit(control->handle);
                                                       ...
                                                       uint32_t gop_frame_count)
{
    ...
    if (control->encoder_inited) {
        control->frame_cfg.idr_interval = gop_frame_count;
        control->frame_cfg.update_flag = 1;
    }
                                                       ...
                                                       uint32_t *gop_frame_count)
{
    ...
    if (control->encoder_inited) {
        *gop_frame_count = control->frame_cfg.idr_interval;
    } else {
                                                 ...
                                                 bk_h264_encode_rate_ctrl_t *rate_ctrl)
{
    ...
    /*
     * Validate against the AVC 0..51 QP range up front; same checks the
     * removed h264e_driver wrapper enforced.
     */
    ...
    if (vcenc_h264_get_rate_ctrl(control->handle, &vcenc_rc) != VCENC_OK) {
        LOGE("vcenc_h264_get_rate_ctrl failed\r\n");
        return AVDK_ERR_GENERIC;
    }

    /*
     * Fixed-QP path is taken when the caller passes bitrate==0, or supplied a
     * QP range that collapses to a single value for both I and P. Otherwise
     * full bitrate-driven RC kicks in with the supplied min/max bracket.
     */
    ...
    if (vcenc_h264_set_rate_ctrl(control->handle, &vcenc_rc) != VCENC_OK) {
...
static avdk_err_t h264_encode_ctlr_ioctl(bk_h264_encode_ctlr_handle_t handle, uint32_t cmd, void *arg)
{
    ...
    switch (cmd) {
        ...
        case BK_H264_ENCODE_IOCTL_SET_FLEXA_LINES_READY: {
            if (control->encoder_inited) {
                (void)vcenc_h264_update_slice_wr_cnt(control->handle, (uint32_t)arg);
            }
            break;
        }
