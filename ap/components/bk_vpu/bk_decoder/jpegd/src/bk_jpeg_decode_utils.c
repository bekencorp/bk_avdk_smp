...
{
    if (img_info == NULL || img_info->input_stream == NULL || img_info->input_stream_length == 0)
    {
    ...
    uint8_t *src_buf;
    uint32_t length;
    src_buf = img_info->input_stream;
    length = img_info->input_stream_length;
    ...
    if (components == 1)
    {
        img_info->format = BK_JPEG_DECODE_IMG_FMT_YUV400;
    }
    else if (msx == 1)
    {
        img_info->format = BK_JPEG_DECODE_IMG_FMT_YUV444;
    }
    else if (msy == 1)
    {
        img_info->format = BK_JPEG_DECODE_IMG_FMT_YUV422;
    }
    else
    {
        img_info->format = BK_JPEG_DECODE_IMG_FMT_YUV420;
    }
