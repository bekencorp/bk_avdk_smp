...
#include "os/os.h"
#include "os/mem.h"
#if (CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED) && !CONFIG_BLE_MESH_AP_HCI_IPC
#include "bk_ble.h"
#endif
...
NET_BUF_POOL_FIXED_DEFINE(hci_cmd_pool, CONFIG_BT_BUF_CMD_TX_COUNT, CMD_BUF_SIZE, NULL);
...
struct net_buf *bt_hci_cmd_create(uint16_t opcode, uint8_t param_len)
{
	...
	BT_VER("opcode 0x%04x param_len %u", opcode, param_len);
...
int bt_hci_cmd_send(uint16_t opcode, struct net_buf *buf)
{
	...
	BT_VER("opcode 0x%04x len %u", opcode, buf->len);
			 ...
			 struct net_buf **rsp)
{
	...
	BT_VER("buf %p opcode 0x%04x len %u", buf, opcode, buf->len);
	...
	BT_VER("rsp %p opcode 0x%04x len %u", buf, opcode, buf->len);
...
{
	BT_VER("opcode 0x%04x status 0x%02x buf %p", opcode, status, buf);
...
static void hci_le_meta_event(struct net_buf *buf)
{
	...
	BT_VER("subevent 0x%02x", evt->subevent);
...
static void bt_dev_show_info(void)
{
	...
	BT_DBG("Identity%s: %s", bt_dev.id_count > 1 ? "[0]" : "",
	...
	if (IS_ENABLED(CONFIG_BT_LOG_SNIFFER_INFO)) {
		...
		BT_DBG("IRK%s: 0x%s", bt_dev.id_count > 1 ? "[0]" : "",
			bt_hex(irk, 16));
#endif
	}
	...
	for (i = 1; i < bt_dev.id_count; i++) {
		...
		if (IS_ENABLED(CONFIG_BT_LOG_SNIFFER_INFO)) {
			...
			BT_DBG("IRK[%d]: 0x%s", i, bt_hex(irk, 16));
#endif
		}
	}
	...
	BT_DBG("HCI: version %s (0x%02x) revision 0x%04x, manufacturer 0x%04x",
		ver_str(bt_dev.hci_version), bt_dev.hci_version,
		bt_dev.hci_revision, bt_dev.manufacturer);
	BT_DBG("LMP: version %s (0x%02x) subver 0x%04x",
...
void bt_finalize_init(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_OBSERVER)) {
		ret = bt_le_scan_update(false);
		if(ret != 0) {
		    BT_ERR("scan err %d, maybe ble scan is already set !!! Don't enable scan before enable mesh !!!", ret);
		}
	}
...
static int bt_init(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		if (!bt_dev.id_count) {
			BT_ERR("No ID address. App must call settings_load()");
			return 0;
		}
...
{
    uint8_t event_code = bt_hci_evt_get_flags(buf[0]);
    struct net_buf *pbuf = NULL;

    #if defined(CONFIG_BT_RECV_IS_RX_THREAD) || (CONFIG_BT_RECV_IS_RX_THREAD == 1)
    if (event_code & BT_HCI_EVT_FLAG_RECV_PRIO) {
        BT_HEXDUMP_DBG(buf,len,"RECV_PRIO");
        pbuf = bt_buf_get_cmd_complete(K_NO_WAIT);
        net_buf_add_mem(pbuf, buf, len);
        bt_buf_set_type(pbuf,BT_BUF_EVT);
        bt_recv_prio(pbuf);
    }

    if (event_code & BT_HCI_EVT_FLAG_RECV) {
        BT_HEXDUMP_DBG(buf,len,"RECV");
        if (!pbuf) {
            pbuf = bt_buf_get_rx(BT_BUF_EVT,K_NO_WAIT);
            net_buf_add_mem(pbuf, buf, len);
            bt_buf_set_type(pbuf,BT_BUF_EVT);
        }
        bt_recv(pbuf);
    }
    #else ///CONFIG_BT_RECV_IS_RX_THREAD
    if (event_code & BT_HCI_EVT_FLAG_RECV_PRIO) {
        pbuf = bt_buf_get_cmd_complete(K_NO_WAIT);
    } else {
        pbuf = bt_buf_get_rx(BT_BUF_EVT,K_NO_WAIT);
...
{
    struct net_buf *pbuf = bt_buf_get_rx(BT_BUF_ACL_IN, K_NO_WAIT);

    BT_HEXDUMP_DBG(buf,len,"[bk]");
    if (pbuf) {
        net_buf_add_mem(pbuf, buf, len);
        bt_recv(pbuf);
    }
...
{
    return bk_ble_reg_hci_recv_callback(ble_hci_to_host_evt_cb,ble_hci_to_host_acl_cb);
}
...
static int hci_driver_send(struct net_buf *buf)
{
    switch (bt_buf_get_type(buf)) {
        case BT_BUF_CMD:
        {
            uint8_t *pbuf = os_malloc(buf->len);
            memcpy(pbuf,buf->data,buf->len);
            BT_HEXDUMP_DBG(pbuf,buf->len,"CMD TX");
            bk_ble_hci_cmd_to_controller(buf->data,buf->len);
            net_buf_unref(buf);
            os_free(pbuf);
        }
        break;
        case BT_BUF_ACL_OUT:
        {
            uint8_t *pbuf = os_malloc(buf->len);
            memcpy(pbuf,buf->data,buf->len);
            BT_HEXDUMP_DBG(pbuf,buf->len,"ACL TX");
            bk_ble_hci_acl_to_controller(buf->data,buf->len);
            net_buf_unref(buf);
            os_free(pbuf);
        }
        break;
        default:
            BT_WARN("Failed to hci_driver_send type (%d)", bt_buf_get_type(buf));
        break;
    }
    return 0;
}
...
#endif //CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED && !CONFIG_BLE_MESH_AP_HCI_IPC
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	BT_INFO("submit work %p %p", &bt_dev.init, bt_dev.init.handler);
