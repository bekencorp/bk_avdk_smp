...
ble_attm_desc_t boarding_service_db[BOARDING_IDX_NB] =
{
    ...
    [BOARDING_IDX_CHAR_OPERATION_VALUE] = {{BOARDING_CHARA_OPERATION_UUID & 0xFF, BOARDING_CHARA_OPERATION_UUID >> 8, 0}, BK_BLE_PERM_SET(WRITE_REQ, ENABLE), BK_BLE_PERM_SET(RI, ENABLE) | BK_BLE_PERM_SET(UUID_LEN, UUID_16), 250},
...
int ble_boarding_notify(uint8_t *data, uint16_t length)
{
    if (s_conn_ind == 0xFF)
    {
        LOGD("BLE is disconnected, can not send data !!!\r\n");
        return BK_FAIL;
    }
    else
    {
...
int ble_boarding_legacy_handle(int sync, int argc, char **argv)
{
    ...
    LOGD("creat boarding service successfully!\r\n");
    ...
    LOGD("set adv param and creat adv successfully!\r\n");
    ...
    LOGD("set adv data successfully!\r\n");
