 ...
 */
#define MEM_STATIC_LOGD( tag, format, ... ) bk_printf_static_block(BK_LOG_DEBUG, tag, format, ##__VA_ARGS__)
...
#endif
#define PSRAM_HEAP_SIZE        CONFIG_AP_PSRAM_HEAP_SIZE   //1MB
#endif

/*-----------------------------------------------------------*/
#if CONFIG_PSRAM_HEAP_INIT_CODE_IN_ITCM
__attribute__((section(".iram"))) void bk_psram_heap_init(void)
#elif CONFIG_PSRAM_HEAP_INIT_CODE_IN_IRAM
...
{
	return (void *)mem_get_heap_start_addr();
}
