...
STATUS setRequestHeader(PRequestInfo pRequestInfo, PCHAR headerName, UINT32 headerNameLen, PCHAR headerValue, UINT32 headerValueLen)
{
    ...
    while (pCurNode != NULL) {
        ...
        pCurrentHeader = (PRequestHeader)(uintptr_t) item;

        if (STRCMPI(pCurrentHeader->pName, pRequestHeader->pName) > 0) {
            if (pPrevNode == NULL) {
                // Insert at the head
                CHK_STATUS(singleListInsertItemHead(pRequestInfo->pRequestHeaders, (UINT64)(uintptr_t) pRequestHeader));
            } else {
                CHK_STATUS(singleListInsertItemAfter(pRequestInfo->pRequestHeaders, pPrevNode, (UINT64)(uintptr_t) pRequestHeader));
            }
    ...
    CHK_STATUS(singleListInsertItemTail(pRequestInfo->pRequestHeaders, (UINT64)(uintptr_t) pRequestHeader));
...
STATUS removeRequestHeaders(PRequestInfo pRequestInfo)
{
    ...
    while (itemCount-- != 0) {
        ...
        pRequestHeader = (PRequestHeader)(uintptr_t) pNode->data;
