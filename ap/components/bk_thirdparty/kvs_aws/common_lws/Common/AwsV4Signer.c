    ...
    while (iterate) {
        ...
        *pCurPtr++ = '=';
        ...
        while (!inserted && pCurNode != NULL) {
            ...
            pParam = (PCHAR)(uintptr_t) item;

            if (STRCMP(pNewParam, pParam) <= 0) {
                if (pPrevNode == NULL) {
                    // Insert at the head
                    CHK_STATUS(singleListInsertItemHead(pSingleList, (UINT64)(uintptr_t) pNewParam));
                } else {
                    CHK_STATUS(singleListInsertItemAfter(pSingleList, pPrevNode, (UINT64)(uintptr_t) pNewParam));
                }
        ...
        if (!inserted) {
            ...
            CHK_STATUS(singleListInsertItemTail(pSingleList, (UINT64)(uintptr_t) pNewParam));
        }
