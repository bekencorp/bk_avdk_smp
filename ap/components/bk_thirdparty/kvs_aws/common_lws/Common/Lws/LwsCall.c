...
INT32 lwsIotCallbackRoutine(struct lws* wsi, enum lws_callback_reasons reason, PVOID user, PVOID pDataIn, size_t dataSize)
{
    ...
    #if 1
    buffer = (PCHAR) MEMCALLOC(1, IOT_LWS_SEND_BUFFER_SIZE);
    ...
    switch (reason) {
        case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
            ...
            DLOGW("Client connection failed. Connection error string: %s %d", pCurPtr,errno);
        ...
        case LWS_CALLBACK_ESTABLISHED_CLIENT_HTTP:
            ...
            #else
            if ((size = lws_hdr_custom_copy(wsi, pBuffer, IOT_LWS_SEND_BUFFER_SIZE, KVS_REQUEST_ID_HEADER_NAME ":",
                                            SIZEOF(KVS_REQUEST_ID_HEADER_NAME) * SIZEOF(CHAR))) > 0)
            #endif
            {
                pBuffer[size] = '\0';
        ...
        case LWS_CALLBACK_RECEIVE_CLIENT_HTTP:
            ...
            size = SIZEOF(buffer);
            #else
            size = (INT32) IOT_LWS_SEND_BUFFER_SIZE;
            #endif
