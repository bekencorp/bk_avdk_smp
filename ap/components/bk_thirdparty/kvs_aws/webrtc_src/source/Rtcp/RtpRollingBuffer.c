...
STATUS freeRtpRollingBufferData(PUINT64 pData)
{
    ...
    pPacket = (PRtpPacket)(uintptr_t)*pData;
    CHK_STATUS(freeRtpPacket(&pPacket));
CleanUp:
    LEAVES();
    return retStatus;
}
...
STATUS rtpRollingBufferAddRtpPacket(PRtpRollingBuffer pRollingBuffer, PRtpPacket pRtpPacket)
{
    ...
    CHK_STATUS(rollingBufferAppendData(pRollingBuffer->pRollingBuffer, (UINT64)(uintptr_t) pRtpPacketCopy, &index));
