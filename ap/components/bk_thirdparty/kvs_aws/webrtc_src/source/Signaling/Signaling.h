...
#define SIGNALING_CLIENT_FROM_CUSTOM_DATA(h) ((PSignalingClient)(uintptr_t)(h))
#define CUSTOM_DATA_FROM_SIGNALING_CLIENT(p) ((UINT64)(uintptr_t)(p))
...
#define TO_SIGNALING_CLIENT_HANDLE(p)   ((SIGNALING_CLIENT_HANDLE)(uintptr_t)(p))
#define FROM_SIGNALING_CLIENT_HANDLE(h) (IS_VALID_SIGNALING_CLIENT_HANDLE(h) ? (PSignalingClient)(uintptr_t)(h) : NULL)
...
#if 0
/* Original name: conflicts with pic Client.c freeClientRetryStrategy(PKinesisVideoClient) when both linked (e.g. Beken). */
STATUS freeClientRetryStrategy(PSignalingClient);
#else
STATUS freeSignalingClientRetryStrategy(PSignalingClient);
#endif
