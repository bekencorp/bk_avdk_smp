                                  ...
                                  PCHAR cacheFilePath)
{
    ...
    #else
    PSignalingFileCacheEntry entries = NULL;
    ...
    #if 1
    /* Avoid ~233KB stack allocation on embedded: allocate entries on heap (was entries[32] on stack). */
    entries = (PSignalingFileCacheEntry) MEMCALLOC(1, SIZEOF(SignalingFileCacheEntry) * MAX_SIGNALING_CACHE_ENTRY_COUNT);
    CHK(entries != NULL, STATUS_NOT_ENOUGH_MEMORY);
    #endif
    ...
    #else
    MEMSET(entries, 0x00, SIZEOF(SignalingFileCacheEntry) * MAX_SIGNALING_CACHE_ENTRY_COUNT);
    #endif
