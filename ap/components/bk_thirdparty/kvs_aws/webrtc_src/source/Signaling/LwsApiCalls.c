    ...
    switch (reason) {
        ...
        case LWS_CALLBACK_CLIENT_APPEND_HANDSHAKE_HEADER:
            ...
            while (headerCount != 0) {
                ...
                pRequestHeader = (PRequestHeader)(uintptr_t) item;
        ...
        case LWS_CALLBACK_CLIENT_HTTP_WRITEABLE:
...
#else
            /* HTTP/2 (ALPN h2): body must use LWS_WRITE_HTTP / LWS_WRITE_HTTP_FINAL — LWS_WRITE_TEXT is WebSocket
             * mode and rops_write_role_protocol_h2 returns 0 (no DATA frame). Single-shot POST uses FINAL + END_STREAM. */
            size = lws_write((struct lws*) wsi, (PBYTE) pBuffer, (SIZE_T) pRequestInfo->bodySize, LWS_WRITE_HTTP_FINAL);
#endif
            ...
            if (size != (INT32) pRequestInfo->bodySize) {
                ...
                if (size > 0) {
                    ...
                    /* wrote==0: common when ALPN negotiated HTTP/2 — body must go on the mux child stream;
                     * this h1-style lws_write path often fails, so no response body is read -> getIceConfigLws CHK fails. */
                    DLOGW("CLIENT_HTTP_WRITEABLE: lws_write returned 0; aborting POST body (check HTTP/2 vs HTTP/1.1 for signaling HTTPS)");
                    // Quit
                    retValue = 1;
                }
            } else {
...
STATUS lwsCompleteSync(PLwsCallInfo pCallInfo)
{
    ...
    #if 1
    path = (PCHAR) MEMCALLOC(1, MAX_URI_CHAR_LEN + 1);
    CHK(path != NULL, STATUS_NOT_ENOUGH_MEMORY);
    #endif
...
STATUS describeChannelLws(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    #if 1
    url = (PCHAR) MEMCALLOC(1, MAX_URI_CHAR_LEN + 1);
    CHK(url != NULL, STATUS_NOT_ENOUGH_MEMORY);
    paramsJson = (PCHAR) MEMCALLOC(1, MAX_JSON_PARAMETER_STRING_LEN);
...
STATUS createChannelLws(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    paramsJson = (PCHAR) MEMCALLOC(1, MAX_JSON_PARAMETER_STRING_LEN);
    CHK(paramsJson != NULL, STATUS_NOT_ENOUGH_MEMORY);
    tagsJson = (PCHAR) MEMCALLOC(1, 2 * MAX_JSON_PARAMETER_STRING_LEN);
...
#else
        // Prepare the json params for the call (use temp buffer to avoid snprintf restrict overlap with tagsJson)
        {
            #if 0
            CHAR tagsParamBuf[MAX_JSON_PARAMETER_STRING_LEN];
            SNPRINTF(tagsParamBuf, ARRAY_SIZE(tagsParamBuf), TAGS_PARAM_JSON_TEMPLATE, tagsJson + MAX_JSON_PARAMETER_STRING_LEN);
            #else
            PCHAR tagsParamBuf = (PCHAR) MEMCALLOC(1, MAX_JSON_PARAMETER_STRING_LEN);
            CHK(tagsParamBuf != NULL, STATUS_NOT_ENOUGH_MEMORY);
            SNPRINTF(tagsParamBuf, MAX_JSON_PARAMETER_STRING_LEN, TAGS_PARAM_JSON_TEMPLATE, tagsJson + MAX_JSON_PARAMETER_STRING_LEN);
            #endif
            STRCPY(tagsJson, tagsParamBuf);
            #if 1
            SAFE_MEMFREE(tagsParamBuf);
            #endif
        }
#endif
    }
    ...
    #else
    SNPRINTF(paramsJson, MAX_JSON_PARAMETER_STRING_LEN, CREATE_CHANNEL_PARAM_JSON_TEMPLATE, pSignalingClient->pChannelInfo->pChannelName,
             getStringFromChannelType(pSignalingClient->pChannelInfo->channelType),
             pSignalingClient->pChannelInfo->messageTtl / HUNDREDS_OF_NANOS_IN_A_SECOND, tagsJson);
    #endif
    // Create the request info with the body
...
STATUS getChannelEndpointLws(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (pSignalingClient->mediaStorageConfig.storageStatus == FALSE) {
        ...
        #else
        SNPRINTF(paramsJson, MAX_JSON_PARAMETER_STRING_LEN, GET_CHANNEL_ENDPOINT_PARAM_JSON_TEMPLATE, pSignalingClient->channelDescription.channelArn,
                 SIGNALING_CHANNEL_PROTOCOL, getStringFromChannelRoleType(pSignalingClient->pChannelInfo->channelRoleType));
        #endif
        } else {
        #if 0
...
STATUS getIceConfigLws(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    #else
    SNPRINTF(paramsJson, MAX_JSON_PARAMETER_STRING_LEN, GET_ICE_CONFIG_PARAM_JSON_TEMPLATE, pSignalingClient->channelDescription.channelArn,
             pSignalingClient->clientInfo.signalingClientInfo.clientId);
    #endif
...
STATUS parseIceConfigResponse(PCHAR pResponseStr, UINT32 responseLen, UINT8 maxIceConfigs, PIceConfigInfo pIceConfigs, PUINT32 pIceConfigCount)
{
    ...
    #else
    tokenCount = jsmn_parse(&parser, pResponseStr, responseLen, tokens, MAX_JSON_TOKEN_COUNT);
    #endif
    CHK(tokenCount > 1, STATUS_INVALID_API_CALL_RETURN_JSON);
    CHK(tokens[0].type == JSMN_OBJECT, STATUS_INVALID_API_CALL_RETURN_JSON);
...
STATUS deleteChannelLws(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    #else
    SNPRINTF(paramsJson, MAX_JSON_PARAMETER_STRING_LEN, DELETE_CHANNEL_PARAM_JSON_TEMPLATE, pSignalingClient->channelDescription.channelArn,
             pSignalingClient->channelDescription.updateVersion);
    #endif
...
STATUS connectSignalingChannelLws(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (pSignalingClient->pChannelInfo->channelRoleType == SIGNALING_CHANNEL_ROLE_TYPE_VIEWER) {
        ...
        SNPRINTF(url, ARRAY_SIZE(url), SIGNALING_ENDPOINT_VIEWER_URL_WSS_TEMPLATE, pSignalingClient->channelEndpointWss,
                 ...
                 pSignalingClient->clientInfo.signalingClientInfo.clientId);
        #else
        SNPRINTF(url, MAX_URI_CHAR_LEN + 1, SIGNALING_ENDPOINT_VIEWER_URL_WSS_TEMPLATE, pSignalingClient->channelEndpointWss,
                 SIGNALING_CHANNEL_ARN_PARAM_NAME, pSignalingClient->channelDescription.channelArn, SIGNALING_CLIENT_ID_PARAM_NAME,
                 pSignalingClient->clientInfo.signalingClientInfo.clientId);
        #endif
    } else {
...
STATUS joinStorageSessionLws(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    if (pSignalingClient->pChannelInfo->channelRoleType == SIGNALING_CHANNEL_ROLE_TYPE_VIEWER) {
        ...
        #else
        SNPRINTF(paramsJson, MAX_JSON_PARAMETER_STRING_LEN, SIGNALING_JOIN_STORAGE_SESSION_VIEWER_PARAM_JSON_TEMPLATE,
                 pSignalingClient->channelDescription.channelArn, pSignalingClient->clientInfo.signalingClientInfo.clientId);
        #endif
    } else {
        #if 0
        ...
        #else
        SNPRINTF(paramsJson, MAX_JSON_PARAMETER_STRING_LEN, SIGNALING_JOIN_STORAGE_SESSION_MASTER_PARAM_JSON_TEMPLATE,
                 pSignalingClient->channelDescription.channelArn);
        #endif
    }
...
STATUS describeMediaStorageConfLws(PSignalingClient pSignalingClient, UINT64 time)
{
    ...
    #else
    SNPRINTF(paramsJson, MAX_JSON_PARAMETER_STRING_LEN, DESCRIBE_MEDIA_STORAGE_CONF_PARAM_JSON_TEMPLATE, pSignalingClient->channelDescription.channelArn);
    #endif
    // Create the request info with the body
    CHK_STATUS(createRequestInfo(url, paramsJson, pSignalingClient->pChannelInfo->pRegion, pSignalingClient->pChannelInfo->pCertPath, NULL, NULL,
                                 SSL_CERTIFICATE_TYPE_NOT_SPECIFIED, pSignalingClient->pChannelInfo->pUserAgent,
...
PVOID reconnectHandler(PVOID args)
{
    ...
    #if 1
    reconnectErrMsg = (PCHAR) MEMCALLOC(1, SIGNALING_MAX_ERROR_MESSAGE_LEN + 1);
    CHK(reconnectErrMsg != NULL, STATUS_NOT_ENOUGH_MEMORY);
    #endif
...
CleanUp:
    ...
    if (pSignalingClient != NULL) {
        ...
        if (STATUS_FAILED(retStatus)) {
            ...
            #else
            if (pSignalingClient->signalingClientCallbacks.errorReportFn != NULL && reconnectErrMsg != NULL) {
            #endif
                      ...
                      PCHAR pCorrelationId, UINT32 correlationIdLen)
{
    ...
    #if 1
    encodedMessage = (PCHAR) MEMCALLOC(1, MAX_SESSION_DESCRIPTION_INIT_SDP_LEN + 1);
...
STATUS parseSignalingMessage(PCHAR pMessage, UINT32 messageLen, PReceivedSignalingMessage pReceivedSignalingMessage)
{
    ...
    for (i = 1; i < tokenCount; i++) {
        if (compareJsonString(pMessage, &tokens[i], JSMN_STRING, (PCHAR) "senderClientId")) {
            ...
            UINT32 statusCodeVal;
            strLen = (UINT32) (tokens[i + 1].end - tokens[i + 1].start);
            CHK(strLen <= MAX_STATUS_CODE_STRING_LEN, STATUS_INVALID_API_CALL_RETURN_JSON);
            CHK_STATUS(STRTOUI32(pMessage + tokens[i + 1].start, pMessage + tokens[i + 1].end, 10, &statusCodeVal));
            pReceivedSignalingMessage->statusCode = (SERVICE_CALL_RESULT) statusCodeVal;
            i++;
