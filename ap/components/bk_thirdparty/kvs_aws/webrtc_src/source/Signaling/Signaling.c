...
#else
    freeSignalingClientRetryStrategy(pSignalingClient);
#endif
...
#else
STATUS freeSignalingClientRetryStrategy(PSignalingClient pSignalingClient)
#endif
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
...
STATUS refreshIceConfiguration(PSignalingClient pSignalingClient)
{
    ...
    #if 1
    iceRefreshErrMsg = (PCHAR) MEMCALLOC(1, SIGNALING_MAX_ERROR_MESSAGE_LEN + 1);
    CHK(iceRefreshErrMsg != NULL, STATUS_NOT_ENOUGH_MEMORY);
    #endif
...
CleanUp:
    ...
    if (pSignalingClient != NULL && STATUS_FAILED(retStatus)) {
        ...
        if (pSignalingClient->signalingClientCallbacks.errorReportFn != NULL && iceRefreshErrMsg != NULL)
        #endif
        {
...
STATUS signalingStoreOngoingMessage(PSignalingClient pSignalingClient, PSignalingMessage pSignalingMessage)
{
    ...
    CHK_STATUS(stackQueueEnqueue(pSignalingClient->pMessageQueue, (UINT64)(uintptr_t) pSignalingMessage));
...
STATUS signalingRemoveOngoingMessage(PSignalingClient pSignalingClient, PCHAR correlationId)
{
    ...
    while (IS_VALID_ITERATOR(iterator)) {
        ...
        pExistingMessage = (PSignalingMessage)(uintptr_t) data;
