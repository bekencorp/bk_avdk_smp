...
#else
int onSctpOutboundPacket(void *addr, void *buffer, size_t length, uint8_t tos, uint8_t set_df)
#endif
{
    UNUSED_PARAM(tos);
    UNUSED_PARAM(set_df);
...
#else
    pSctpSession->sctpSessionCallbacks.outboundPacketFunc(pSctpSession->sctpSessionCallbacks.customData, (PBYTE) buffer, (UINT32) length);
#endif
    return 0;
}
...
INT32 onSctpInboundPacket(struct socket* sock, union sctp_sockstore addr, PVOID data, ULONG length, struct sctp_rcvinfo rcv, INT32 flags,
                          PVOID ulp_info)
#else
int onSctpInboundPacket(struct socket *sock, union sctp_sockstore addr, void *data, size_t datalen, struct sctp_rcvinfo rcv, int flags,
                        void *ulp_info)
#endif
{
    UNUSED_PARAM(sock);
    ...
    switch (rcv.rcv_ppid) {
        case SCTP_PPID_DCEP:
            ...
            #else
            CHK_STATUS(handleDcepPacket(pSctpSession, rcv.rcv_sid, (PBYTE) data, (SIZE_T) datalen));
            #endif
            break;
        ...
        case SCTP_PPID_STRING_EMPTY:
            ...
            pSctpSession->sctpSessionCallbacks.dataChannelMessageFunc(pSctpSession->sctpSessionCallbacks.customData, rcv.rcv_sid, isBinary, data,
                                                            length);
            #else
            pSctpSession->sctpSessionCallbacks.dataChannelMessageFunc(pSctpSession->sctpSessionCallbacks.customData, rcv.rcv_sid, isBinary,
