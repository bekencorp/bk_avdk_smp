                      ...
                      TIMER_QUEUE_HANDLE timerQueueHandle, PConnectionListener pConnectionListener, PIceAgent* ppIceAgent)
{
    ...
    CHK_STATUS(createStateMachineWithName(ICE_AGENT_STATE_MACHINE_STATES, ICE_AGENT_STATE_MACHINE_STATE_COUNT, (UINT64)(uintptr_t) pIceAgent,
                                          iceAgentGetCurrentTime, (UINT64)(uintptr_t) pIceAgent, ICE_STATE_MACHINE_NAME, &pIceAgent->pStateMachine));
...
STATUS freeIceAgent(PIceAgent* ppIceAgent)
{
    ...
    if (pIceAgent->localCandidates != NULL) {
        ...
        while (pCurNode != NULL) {
            pIceCandidate = (PIceCandidate)(uintptr_t) pCurNode->data;
    ...
    if (pIceAgent->iceCandidatePairs != NULL) {
        ...
        while (pCurNode != NULL) {
            ...
            pCurNode = pCurNode->pNext;
            pIceCandidatePair = (PIceCandidatePair)(uintptr_t) data;
    ...
    if (pIceAgent->localCandidates != NULL) {
        ...
        while (pCurNode != NULL) {
            CHK_STATUS(doubleListGetNodeData(pCurNode, &data));
            pCurNode = pCurNode->pNext;
            pIceCandidate = (PIceCandidate)(uintptr_t) data;
...
STATUS iceAgentAddRemoteCandidate(PIceAgent pIceAgent, PCHAR pIceCandidateString)
{
    ...
    CHK_STATUS(doubleListInsertItemHead(pIceAgent->remoteCandidates, (UINT64)(uintptr_t) pIceCandidate));
    ...
    CHK_STATUS(doubleListGetHeadNode(pIceAgent->localCandidates, &pCurNode));
    while (pCurNode != NULL) {
        pLocalIceCandidate = (PIceCandidate)(uintptr_t) pCurNode->data;
        pCurNode = pCurNode->pNext;
...
STATUS iceAgentInitHostCandidate(PIceAgent pIceAgent)
{
    ...
    for (i = 0; i < pIceAgent->localNetworkInterfaceCount; ++i) {
        ...
        if (pDuplicatedIceCandidate == NULL &&
            STATUS_SUCCEEDED(createSocketConnection(pIpAddress->family, KVS_SOCKET_PROTOCOL_UDP, pIpAddress, NULL, (UINT64)(uintptr_t) pIceAgent,
            ...
            CHK_STATUS(doubleListInsertItemHead(pIceAgent->localCandidates, (UINT64)(uintptr_t) pTmpIceCandidate));
...
STATUS iceAgentStartAgent(PIceAgent pIceAgent, PCHAR remoteUsername, PCHAR remotePassword, BOOL isControlling)
{
    ...
    CHK_STATUS(timerQueueAddTimer(pIceAgent->timerQueueHandle, KVS_ICE_DEFAULT_TIMER_START_DELAY,
                                  ...
                                  (UINT64)(uintptr_t) pIceAgent, &pIceAgent->iceAgentStateTimerTask));
...
STATUS iceAgentStartGathering(PIceAgent pIceAgent)
{
    ...
    CHK_STATUS(timerQueueAddTimer(pIceAgent->timerQueueHandle, KVS_ICE_DEFAULT_TIMER_START_DELAY, KVS_ICE_GATHER_CANDIDATE_TIMER_POLLING_INTERVAL,
                                  iceAgentGatherCandidateTimerCallback, (UINT64)(uintptr_t) pIceAgent, &pIceAgent->iceCandidateGatheringTimerTask));
                                                     ...
                                                     PUINT32 pIndex)
{
    ...
    while (pCurNode != NULL) {
        CHK_STATUS(doubleListGetNodeData(pCurNode, &data));
        pCurNode = pCurNode->pNext;
        pCandidate = (PIceCandidate)(uintptr_t) data;
        if (pCandidate->state == ICE_CANDIDATE_STATE_VALID) {
            STRCPY(pSdpMediaDescription->sdpAttributes[attrIndex].attributeName, "candidate");
            CHK_STATUS(iceCandidateSerialize((PIceCandidate)(uintptr_t) data, pSdpMediaDescription->sdpAttributes[attrIndex].attributeValue, &attrBufferLen));
            attrIndex++;
        }
    }
...
STATUS iceAgentShutdown(PIceAgent pIceAgent)
{
    ...
    if (pIceAgent->iceAgentStateTimerTask != MAX_UINT32) {
        CHK_STATUS(timerQueueCancelTimer(pIceAgent->timerQueueHandle, pIceAgent->iceAgentStateTimerTask, (UINT64)(uintptr_t) pIceAgent));
        pIceAgent->iceAgentStateTimerTask = MAX_UINT32;
    }

    if (pIceAgent->keepAliveTimerTask != MAX_UINT32) {
        CHK_STATUS(timerQueueCancelTimer(pIceAgent->timerQueueHandle, pIceAgent->keepAliveTimerTask, (UINT64)(uintptr_t) pIceAgent));
        pIceAgent->keepAliveTimerTask = MAX_UINT32;
    }

    if (pIceAgent->iceCandidateGatheringTimerTask != MAX_UINT32) {
        CHK_STATUS(timerQueueCancelTimer(pIceAgent->timerQueueHandle, pIceAgent->iceCandidateGatheringTimerTask, (UINT64)(uintptr_t) pIceAgent));
        pIceAgent->iceCandidateGatheringTimerTask = MAX_UINT32;
    }
    ...
    CHK_STATUS(doubleListGetHeadNode(pIceAgent->localCandidates, &pCurNode));
    while (pCurNode != NULL) {
        pLocalCandidate = (PIceCandidate)(uintptr_t) pCurNode->data;
        pCurNode = pCurNode->pNext;
...
STATUS iceAgentRestart(PIceAgent pIceAgent, PCHAR localIceUfrag, PCHAR localIcePwd)
{
    ...
    CHK_STATUS(doubleListGetHeadNode(pIceAgent->iceCandidatePairs, &pCurNode));
    while (pCurNode != NULL) {
        pIceCandidatePair = (PIceCandidatePair)(uintptr_t) pCurNode->data;
...
// Runtime control for turning stats calculations on/off can be activated with this compiler flag.
    ...
    while (pCurNode != NULL) {
        CHK_STATUS(doubleListGetNodeData(pCurNode, &data));
        pCurrentIceCandidate = (PIceCandidate)(uintptr_t) data;
        pCurNode = pCurNode->pNext;
...
STATUS insertIceCandidatePair(PDoubleList iceCandidatePairs, PIceCandidatePair pIceCandidatePair)
{
    ...
    while (pCurNode != NULL) {
        pCurIceCandidatePair = (PIceCandidatePair)(uintptr_t) pCurNode->data;
    ...
    if (pCurNode != NULL) {
        CHK_STATUS(doubleListInsertItemBefore(iceCandidatePairs, pCurNode, (UINT64)(uintptr_t) pIceCandidatePair));
    } else {
        CHK_STATUS(doubleListInsertItemTail(iceCandidatePairs, (UINT64)(uintptr_t) pIceCandidatePair));
    }
...
STATUS iceAgentStateTransitionTimerCallback(UINT32 timerId, UINT64 currentTime, UINT64 customData)
{
    ...
    STATUS retStatus = STATUS_SUCCESS;
    PIceAgent pIceAgent = (PIceAgent)(uintptr_t) customData;
...
STATUS iceAgentInitSrflxCandidate(PIceAgent pIceAgent)
{
        ...
        if (pCandidate->iceCandidateType == ICE_CANDIDATE_TYPE_HOST) {
            for (j = 0; j < pIceAgent->iceServersCount; j++) {
                ...
                if (!pIceServer->isTurn &&
                    ...
                    CHK_STATUS(doubleListInsertItemHead(pIceAgent->localCandidates, (UINT64)(uintptr_t) pNewCandidate));
    ...
    for (j = 0; j < srflxCount; j++) {
        ...
        CHK_STATUS(createSocketConnection(pCandidate->ipAddress.family, KVS_SOCKET_PROTOCOL_UDP, &pCandidate->ipAddress, NULL, (UINT64)(uintptr_t) pIceAgent,
...
STATUS turnStateFailedFn(PSocketConnection pSocketConnection, UINT64 data)
{
    ...
    PIceCandidate pNewCandidate = (PIceCandidate)(uintptr_t) data;
...
STATUS iceAgentInitRelayCandidate(PIceAgent pIceAgent, UINT32 iceServerIndex, KVS_SOCKET_PROTOCOL protocol, KVS_IP_FAMILY_TYPE turnServerIpFamily)
{
    ...
    CHK_STATUS(createSocketConnection(turnServerIpFamily, protocol, NULL, pTurnServerAddress, (UINT64)(uintptr_t) pNewCandidate, incomingRelayedDataHandler,
    ...
    callback.customData = (UINT64)(uintptr_t) pNewCandidate;
...
STATUS iceAgentConnectedStateSetup(PIceAgent pIceAgent)
{
    ...
    CHK_STATUS(timerQueueAddTimer(pIceAgent->timerQueueHandle, KVS_ICE_DEFAULT_TIMER_START_DELAY, KVS_ICE_SEND_KEEP_ALIVE_INTERVAL,
                                  iceAgentSendKeepAliveTimerCallback, (UINT64)(uintptr_t) pIceAgent, &pIceAgent->keepAliveTimerTask));
...
STATUS iceAgentReadyStateSetup(PIceAgent pIceAgent)
{
    ...
    CHK_STATUS(timerQueueUpdateTimerPeriod(pIceAgent->timerQueueHandle, (UINT64)(uintptr_t) pIceAgent, pIceAgent->iceAgentStateTimerTask,
...
{
    STATUS retStatus = STATUS_SUCCESS;
    PIceCandidate pRelayedCandidate = (PIceCandidate)(uintptr_t) customData;
    ...
    for (i = 0; i < turnChannelDataCount; ++i) {
        incomingDataHandler((UINT64)(uintptr_t) pRelayedCandidate->pIceAgent, pSocketConnection, turnChannelData[i].data, turnChannelData[i].size,
    ...
    switch (stunPacketType) {
        case STUN_PACKET_TYPE_BINDING_REQUEST:
            ...
            if (pIceCandidatePair->state == ICE_CANDIDATE_PAIR_STATE_FROZEN || pIceCandidatePair->state == ICE_CANDIDATE_PAIR_STATE_WAITING ||
                pIceCandidatePair->state == ICE_CANDIDATE_PAIR_STATE_IN_PROGRESS) {
                CHK_STATUS(stackQueueEnqueue(pIceAgent->triggeredCheckQueue, (UINT64)(uintptr_t) pIceCandidatePair));
            }
