                            ...
                            PConnectionListener pConnectionListener, KVS_IP_FAMILY_TYPE turnServerIpFamily, PTurnConnection* ppTurnConnection)
{
    ...
    CHK_STATUS(createStateMachineWithName(TURN_CONNECTION_STATE_MACHINE_STATES, TURN_CONNECTION_STATE_MACHINE_STATE_COUNT, (UINT64)(uintptr_t) pTurnConnection,
                                          turnConnectionGetTime, (UINT64)(uintptr_t) pTurnConnection, turnStateMachineName, &pTurnConnection->pStateMachine));
...
STATUS freeTurnConnection(PTurnConnection* ppTurnConnection)
{
    ...
    if (timerCallbackId != MAX_UINT32) {
        CHK_LOG_ERR(timerQueueCancelTimer(pTurnConnection->timerQueueHandle, (UINT32) timerCallbackId, (UINT64)(uintptr_t) pTurnConnection));
    }
...
STATUS turnConnectionStart(PTurnConnection pTurnConnection)
{
    ...
    timerCallbackId = ATOMIC_EXCHANGE(&pTurnConnection->timerCallbackId, MAX_UINT32);
    if (timerCallbackId != MAX_UINT32) {
        CHK_STATUS(timerQueueCancelTimer(pTurnConnection->timerQueueHandle, (UINT32) timerCallbackId, (UINT64)(uintptr_t) pTurnConnection));
    }
    ...
    CHK_STATUS(timerQueueAddTimer(pTurnConnection->timerQueueHandle, KVS_ICE_DEFAULT_TIMER_START_DELAY, pTurnConnection->currentTimerCallingPeriod,
                                  turnConnectionTimerCallback, (UINT64)(uintptr_t) pTurnConnection, (PUINT32) &timerCallbackId));
...
STATUS turnConnectionTimerCallback(UINT32 timerId, UINT64 currentTime, UINT64 customData)
{
    ...
    STATUS retStatus = STATUS_SUCCESS;
    PTurnConnection pTurnConnection = (PTurnConnection)(uintptr_t) customData;
