...
#if defined(BEKEN_PLATFORM) && !defined(HAVE_IFADDRS_H)
...
#if defined(BEKEN_PLATFORM) && !defined(HAVE_IFADDRS_H)
#if LWIP_IPV6
/* Append IPv6 unicast addresses from an lwIP netif (same filtering idea as getifaddrs branch). */
static void bekenAppendIpv6FromNetif(struct netif *netif, PCHAR ifName, PKvsIpAddress destIpList, UINT32 destIpListLen,
                                     UINT32 *pIpCount, IceSetInterfaceFilterFunc filter, UINT64 customData)
{
    UINT32 ipCount = *pIpCount;
    int j;
    ...
    for (j = 0; j < LWIP_IPV6_NUM_ADDRESSES && ipCount < destIpListLen; j++) {
        const ip6_addr_t *ip6 = netif_ip6_addr(netif, j);

        if ((netif_ip6_addr_state(netif, j) & IP6_ADDR_VALID) == 0) {
            continue;
        }
        if (ip6_addr_isany(ip6)) {
            continue;
        }
        /* Match ICE guidance: skip link-local and site-local (RFC 8445 §5.1.1.1). */
        if (ip6_addr_islinklocal(ip6) || ip6_addr_issitelocal(ip6)) {
            continue;
        }
        if (ip6_addr_ismulticast(ip6)) {
            continue;
        }
        if (filter != NULL && filter(customData, ifName) == FALSE) {
            continue;
        }

        {
            struct in6_addr in6;
            inet6_addr_from_ip6addr(&in6, ip6);
            destIpList[ipCount].family = KVS_IP_FAMILY_TYPE_IPV6;
            destIpList[ipCount].port = 0;
            destIpList[ipCount].isPointToPoint = FALSE;
            MEMCPY(destIpList[ipCount].address, in6.s6_addr, IPV6_ADDRESS_LENGTH);
            ipCount++;
        }
    }
    *pIpCount = ipCount;
...
static UINT32 getBekenLocalIpAddresses(PKvsIpAddress destIpList, UINT32 destIpListLen,
                                       IceSetInterfaceFilterFunc filter, UINT64 customData)
{
    UINT32 ipCount = 0;
    netif_ip4_config_t config;
    struct in_addr inaddr;
    static const struct { netif_if_t ifx; const char *name; } ifs[] = {
        { NETIF_IF_STA, "STA" },
        { NETIF_IF_AP,  "AP" },
        { NETIF_IF_ETH, "ETH" },
    };
    ...
    for (i = 0; i < sizeof(ifs) / sizeof(ifs[0]) && ipCount < destIpListLen; i++) {
        if (bk_netif_get_ip4_config(ifs[i].ifx, &config) != BK_OK)
            continue;
        if (config.ip[0] == '\0')
            continue;
        if (STRNCMP(config.ip, "0.0.0.0", 8) == 0)
            continue;
        if (filter != NULL && filter(customData, (PCHAR) ifs[i].name) == FALSE)
            continue;
        if (inet_pton(AF_INET, config.ip, &inaddr) != 1)
            continue;
        destIpList[ipCount].family = KVS_IP_FAMILY_TYPE_IPV4;
        destIpList[ipCount].port = 0;
        destIpList[ipCount].isPointToPoint = FALSE;
        MEMCPY(destIpList[ipCount].address, &inaddr, IPV4_ADDRESS_LENGTH);
        ipCount++;
    }

#if LWIP_IPV6
    /* IPv6: read lwIP netif slots (see bk_netif_get_ip6_addr_info / cli "ip6"). */
    bekenAppendIpv6FromNetif((struct netif *) net_get_sta_handle(), (PCHAR) "STA", destIpList, destIpListLen, &ipCount,
                             filter, customData);
    bekenAppendIpv6FromNetif((struct netif *) net_get_uap_handle(), (PCHAR) "AP", destIpList, destIpListLen, &ipCount,
                             filter, customData);
#ifdef CONFIG_ETH
    bekenAppendIpv6FromNetif((struct netif *) net_get_eth_handle(), (PCHAR) "ETH", destIpList, destIpListLen, &ipCount,
                             filter, customData);
#endif
#endif /* LWIP_IPV6 */
...
STATUS getLocalhostIpAddresses(PKvsIpAddress destIpList, PUINT32 pDestIpListLen, IceSetInterfaceFilterFunc filter, UINT64 customData)
{
    ...
    /* use getBekenLocalIpAddresses when HAVE_IFADDRS_H not set */
#else
    (void) filter;
    (void) customData;
#endif
...
#endif
/* Beken without getifaddrs collects IPv6 inside getBekenLocalIpAddresses; no pIpv6Addr here. */
#if defined(_WIN32) || !defined(BEKEN_PLATFORM) || (defined(HAVE_IFADDRS_H) && LWIP_IPV6)
...
#else
#if defined(BEKEN_PLATFORM)
    ipCount = getBekenLocalIpAddresses(destIpList, destIpListLen, filter, customData);
...
STATUS socketBind(PKvsIpAddress pHostIpAddress, INT32 sockfd)
{
    ...
    if (pHostIpAddress->family == KVS_IP_FAMILY_TYPE_IPV4) {
        ...
        CHK(FALSE, STATUS_INVALID_ARG); /* IPv6 not supported when LWIP_IPV6 is disabled */
#endif
    }
...
#else
    pHostIpAddress->port = (UINT16) ipv4Addr.sin_port;
#endif
...
STATUS getIpWithHostName(PCHAR hostname, PDualKvsIpAddresses destIps)
{
    ...
    if (!wasAddressParseSuccessful || retStatus != STATUS_SUCCESS) {
        ...
        if (errCode != 0) {
            //errStr = errCode == EAI_SYSTEM ? (PCHAR) strerror(errno) : (PCHAR) gai_strerror(errCode);
            errStr = (errCode == EAI_SYSTEM) ? (PCHAR) strerror(errno) : (PCHAR) "getaddrinfo failed";
