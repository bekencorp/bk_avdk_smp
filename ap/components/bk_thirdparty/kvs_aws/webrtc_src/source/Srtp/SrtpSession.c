...
STATUS decryptSrtpPacket(PSrtpSession pSrtpSession, PVOID encryptedMessage, PINT32 len)
{
    ...
    CHK(*len > 0, STATUS_INVALID_ARG);
    srtp_len = (size_t)*len;
    out_cap = srtp_len;

    errStatus = srtp_unprotect(pSrtpSession->srtp_receive_session, (const uint8_t *)encryptedMessage, srtp_len,
                               (uint8_t *)encryptedMessage, &out_cap);
    CHK_ERR(errStatus == srtp_err_status_ok, STATUS_SRTP_DECRYPT_FAILED, "Decrypting rtp packet failed with error code %u on srtp session %" PRIu64, errStatus,
            pSrtpSession->srtp_receive_session);
    CHK(out_cap <= (size_t)MAX_INT32, STATUS_INVALID_ARG);
    *len = (INT32)out_cap;
#else
...
STATUS decryptSrtcpPacket(PSrtpSession pSrtpSession, PVOID encryptedMessage, PINT32 len)
{
    ...
    errStatus = srtp_unprotect_rtcp(pSrtpSession->srtp_receive_session, (const uint8_t *)encryptedMessage, srtcp_len,
                                    (uint8_t *)encryptedMessage, &out_cap);
    CHK_ERR(errStatus == srtp_err_status_ok, STATUS_SRTP_DECRYPT_FAILED, "Decrypting rtcp packet failed with error code %u on srtp session %" PRIu64, errStatus,
            pSrtpSession->srtp_receive_session);
    CHK(out_cap <= (size_t)MAX_INT32, STATUS_INVALID_ARG);
    *len = (INT32)out_cap;
#else
...
STATUS encryptRtpPacket(PSrtpSession pSrtpSession, PVOID message, PINT32 len)
{
    ...
    CHK(*len > 0, STATUS_INVALID_ARG);
    rtp_len = (size_t)*len;
    buf_cap = rtp_len + SRTP_MAX_TRAILER_LEN;

    status = srtp_protect(pSrtpSession->srtp_transmit_session, (const uint8_t *)message, rtp_len, (uint8_t *)message, &buf_cap, 0);
    ...
    CHK(buf_cap <= (size_t)MAX_INT32, STATUS_INVALID_ARG);
    *len = (INT32)buf_cap;
#else
...
STATUS encryptRtcpPacket(PSrtpSession pSrtpSession, PVOID message, PINT32 len)
{
    ...
    CHK(*len > 0, STATUS_INVALID_ARG);
    rtcp_len = (size_t)*len;
    buf_cap = rtcp_len + SRTP_MAX_SRTCP_TRAILER_LEN;

    status = srtp_protect_rtcp(pSrtpSession->srtp_transmit_session, (const uint8_t *)message, rtcp_len, (uint8_t *)message, &buf_cap, 0);
