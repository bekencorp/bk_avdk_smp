...
STATUS allocateSctpSortDataChannelsDataCallback(UINT64 customData, PHashEntry pHashEntry)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    PAllocateSctpSortDataChannelsData data = (PAllocateSctpSortDataChannelsData)(uintptr_t) customData;
    PKvsDataChannel pKvsDataChannel = (PKvsDataChannel)(uintptr_t) pHashEntry->value;

    CHK(customData != 0, STATUS_NULL_ARG);

    pKvsDataChannel->channelId = data->currentDataChannelId;
    CHK_STATUS(hashTablePut(data->pKvsPeerConnection->pDataChannels, pKvsDataChannel->channelId, (UINT64)(uintptr_t) pKvsDataChannel));
...
STATUS allocateSctp(PKvsPeerConnection pKvsPeerConnection)
{
    ...
    CHK_STATUS(hashTableIterateEntries(data.unkeyedDataChannels, (UINT64)(uintptr_t) &data, allocateSctpSortDataChannelsDataCallback));
    ...
    sctpSessionCallbacks.customData = (UINT64)(uintptr_t) pKvsPeerConnection;
    ...
    for (; currentDataChannelId < data.currentDataChannelId; currentDataChannelId += 2) {
        ...
        pKvsDataChannel = (PKvsDataChannel)(uintptr_t) hashValue;
        ...
        if (STATUS_FAILED(hashTableUpsert(pKvsPeerConnection->pDataChannels, currentDataChannelId, (UINT64)(uintptr_t) pKvsDataChannel))) {
...
VOID onInboundPacket(UINT64 customData, PBYTE buff, UINT32 buffLen)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    PKvsPeerConnection pKvsPeerConnection = (PKvsPeerConnection)(uintptr_t) customData;
...
STATUS sendPacketToRtpReceiver(PKvsPeerConnection pKvsPeerConnection, PBYTE pBuffer, UINT32 bufferLen)
{
    ...
    while (pCurNode != NULL) {
        ...
        pTransceiver = (PKvsRtpTransceiver)(uintptr_t) item;
...
STATUS onFrameReadyFunc(UINT64 customData, UINT16 startIndex, UINT16 endIndex, UINT32 frameSize)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    PKvsRtpTransceiver pTransceiver = (PKvsRtpTransceiver)(uintptr_t) customData;
    ...
    pPacket = (PRtpPacket)(uintptr_t) hashValue;
    if (retStatus == STATUS_SUCCESS || retStatus == STATUS_HASH_KEY_NOT_PRESENT) {
        retStatus = STATUS_SUCCESS;
    } else {
        CHK(FALSE, retStatus);
...
VOID onNewIceLocalCandidate(UINT64 customData, PCHAR candidateSdpStr)
{
    ...
    if (candidateSdpStr != NULL) {
        ...
        #else
        jsonStrBuffer = (PCHAR) MEMCALLOC(1, MAX_ICE_CANDIDATE_JSON_LEN);
        CHK(jsonStrBuffer != NULL, STATUS_NOT_ENOUGH_MEMORY);
        strCompleteLen = SNPRINTF(jsonStrBuffer, MAX_ICE_CANDIDATE_JSON_LEN, ICE_CANDIDATE_JSON_TEMPLATE, candidateSdpStr);
        #endif
        CHK(strCompleteLen > 0, STATUS_INTERNAL_ERROR);
        #if 0
        CHK(strCompleteLen < (INT32) ARRAY_SIZE(jsonStrBuffer), STATUS_BUFFER_TOO_SMALL);
        #else
        CHK(strCompleteLen < (INT32) MAX_ICE_CANDIDATE_JSON_LEN, STATUS_BUFFER_TOO_SMALL);
        #endif
...
VOID onSctpSessionOutboundPacket(UINT64 customData, PBYTE pPacket, UINT32 packetLen)
{
    ...
    pKvsPeerConnection = (PKvsPeerConnection)(uintptr_t) customData;
...
VOID onSctpSessionDataChannelMessage(UINT64 customData, UINT32 channelId, BOOL isBinary, PBYTE pMessage, UINT32 pMessageLen)
{
    ...
    if (STATUS_FAILED(hashTableUpsert(pKvsPeerConnection->pDataChannels, channelId, (UINT64)(uintptr_t) pKvsDataChannel))) {
        DLOGW("Failed to update entry in hash table with recent changes to data channel");
    }
...
VOID onSctpSessionDataChannelOpen(UINT64 customData, UINT32 channelId, PBYTE pName, UINT32 nameLen)
{
    ...
    CHK_STATUS(hashTablePut(pKvsPeerConnection->pDataChannels, channelId, (UINT64)(uintptr_t) pKvsDataChannel));
...
STATUS rtcpReportsCallback(UINT32 timerId, UINT64 currentTime, UINT64 customData)
{
    ...
    PKvsRtpTransceiver pKvsRtpTransceiver = (PKvsRtpTransceiver)(uintptr_t) customData;
    ...
    CHK_STATUS(timerQueueAddTimer(pKvsPeerConnection->timerQueueHandle, delay * HUNDREDS_OF_NANOS_IN_A_MILLISECOND,
                                  TIMER_QUEUE_SINGLE_INVOCATION_PERIOD, rtcpReportsCallback, (UINT64)(uintptr_t) pKvsRtpTransceiver,
...
STATUS createPeerConnection(PRtcConfiguration pConfiguration, PRtcPeerConnection* ppPeerConnection)
{
    ...
    CHK_STATUS(dtlsSessionOnOutBoundData(pKvsPeerConnection->pDtlsSession, (UINT64)(uintptr_t) pKvsPeerConnection, onDtlsOutboundPacket));
    CHK_STATUS(dtlsSessionOnStateChange(pKvsPeerConnection->pDtlsSession, (UINT64)(uintptr_t) pKvsPeerConnection, onDtlsStateChange));
    ...
    iceAgentCallbacks.customData = (UINT64)(uintptr_t) pKvsPeerConnection;
...
STATUS freeHashEntry(UINT64 customData, PHashEntry pHashEntry)
{
    ENTERS();
    UNUSED_PARAM(customData);
    MEMFREE((PVOID)(uintptr_t) pHashEntry->value);
                      ...
                      PRtcRtpTransceiver* ppRtcRtpTransceiver)
{
    ...
    CHK_STATUS(createJitterBuffer(onFrameReadyFunc, onFrameDroppedFunc, depayFunc, DEFAULT_JITTER_BUFFER_MAX_LATENCY, clockRate,
                                  (UINT64)(uintptr_t) pKvsRtpTransceiver, &pJitterBuffer));
    ...
    CHK_STATUS(doubleListInsertItemHead(pKvsPeerConnection->pTransceivers, (UINT64)(uintptr_t) pKvsRtpTransceiver));
    *ppRtcRtpTransceiver = (PRtcRtpTransceiver) pKvsRtpTransceiver;
    ...
    CHK_STATUS(timerQueueAddTimer(pKvsPeerConnection->timerQueueHandle, RTCP_FIRST_REPORT_DELAY, TIMER_QUEUE_SINGLE_INVOCATION_PERIOD,
                                  rtcpReportsCallback, (UINT64)(uintptr_t) pKvsRtpTransceiver, &pKvsRtpTransceiver->rtcpReportsTimerId));
...
static STATUS twccRollingWindowDeletion(PKvsPeerConnection pKvsPeerConnection, PRtpPacket pRtpPacket, UINT16 endingSeqNum)
{
    ...
    do {
        ...
        if (STATUS_SUCCEEDED(hashTableGet(pKvsPeerConnection->pTwccManager->pTwccRtpPktInfosHashTable, updatedSeqNum, &twccPacketValue))) {
            tempTwccRtpPktInfo = (PTwccRtpPacketInfo)(uintptr_t) twccPacketValue;
...
STATUS twccManagerOnPacketSent(PKvsPeerConnection pKvsPeerConnection, PRtpPacket pRtpPacket)
{
    ...
    CHK_STATUS(hashTableUpsert(pKvsPeerConnection->pTwccManager->pTwccRtpPktInfosHashTable, seqNum, (UINT64)(uintptr_t) pTwccRtpPktInfo));
