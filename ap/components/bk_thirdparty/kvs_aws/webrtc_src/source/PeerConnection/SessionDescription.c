...
STATUS setTransceiverPayloadTypes(PHashTable codecTable, PHashTable rtxTable, PDoubleList pTransceivers)
{
    ...
    while (pCurNode != NULL) {
        ...
        pKvsRtpTransceiver = (PKvsRtpTransceiver)(uintptr_t) data;
...
STATUS findCodecInTransceivers(PKvsPeerConnection pKvsPeerConnection, RTC_CODEC rtcCodec, PBOOL pDidFindCodec, PHashTable pSeenTransceivers)
{
    ...
    while (pCurNode != NULL) {
        ...
        CHK_STATUS(hashTableContains(pSeenTransceivers, (UINT64)(uintptr_t) pKvsRtpTransceiver, &contains));
        if (pKvsRtpTransceiver != NULL && pKvsRtpTransceiver->sender.track.codec == rtcCodec && contains == FALSE) {
            CHK_STATUS(doubleListInsertItemTail(pKvsPeerConnection->pAnswerTransceivers, (UINT64)(uintptr_t) pKvsRtpTransceiver));
            CHK_STATUS(hashTablePut(pSeenTransceivers, (UINT64)(uintptr_t) pKvsRtpTransceiver, 0));
            *pDidFindCodec = TRUE;
            break;
        }
        pCurNode = pCurNode->pNext;
                                           ...
                                           PHashTable pUnknownCodecPayloadTypesTable, PHashTable pUnknownCodecRtpmapTable)
{
    ...
    for (currentMedia = 0; currentMedia < pRemoteSessionDescription->mediaCount; currentMedia++) {
        ...
        if (!foundMediaSectionWithCodec && (streamKind == MEDIA_STREAM_TRACK_KIND_AUDIO || streamKind == MEDIA_STREAM_TRACK_KIND_VIDEO)) {
            ...
            CHK_STATUS(doubleListInsertItemTail(pKvsPeerConnection->pFakeTransceivers, (UINT64)(uintptr_t) pKvsRtpFakeTransceiver));
            CHK_STATUS(doubleListInsertItemTail(pKvsPeerConnection->pAnswerTransceivers, (UINT64)(uintptr_t) pKvsRtpFakeTransceiver));
            ...
            CHK_STATUS(hashTableContains(pUnknownCodecRtpmapTable, (UINT64)(uintptr_t) rtpMapValue, &containsRtpMap));
            if (containsPayloadType == FALSE && containsRtpMap == FALSE) {
                ...
                CHK_STATUS(hashTablePut(pUnknownCodecRtpmapTable, unknownCodecCounter, (UINT64)(uintptr_t) rtpMapValue));
                unknownCodecCounter++;
