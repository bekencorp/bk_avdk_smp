...
#else
INT32 tlsSessionSendCallback(PVOID customData, const unsigned char* buf, size_t len)
#endif
{
    STATUS retStatus = STATUS_SUCCESS;
    ...
    #else
    pTlsSession->callbacks.outboundPacketFn(pTlsSession->callbacks.outBoundPacketFnCustomData, (PBYTE) buf, (UINT32) len);
    #endif
CleanUp:
...
#else
INT32 tlsSessionReceiveCallback(PVOID customData, unsigned char* buf, size_t len)
#endif
{
    STATUS retStatus = STATUS_SUCCESS;
    PTlsSession pTlsSession = (PTlsSession) customData;
    ...
    if (pBuffer->off < pBuffer->len) {
        ...
        #else
        retStatus = ioBufferRead(pBuffer, buf, (UINT32) len, &readBytes);
        #endif
    }
