...
//#include <stddef.h>
//#include <stdint.h>
/* callbacks: signatures must match mbedtls_ssl_set_bio / mbedtls_ssl_set_timer_cb (match Dtls_mbedtls.c #else branch) */
INT32 dtlsSessionSendCallback(PVOID, const unsigned char*, size_t);
INT32 dtlsSessionReceiveCallback(PVOID, unsigned char*, size_t);
VOID dtlsSessionSetTimerCallback(PVOID, uint32_t, uint32_t);
INT32 dtlsSessionGetTimerCallback(PVOID);
#endif
