...
STATUS freeDtlsSession(PDtlsSession* ppDtlsSession)
{
    ...
    if (pDtlsSession->timerId != MAX_UINT32) {
        timerQueueCancelTimer(pDtlsSession->timerQueueHandle, pDtlsSession->timerId, (UINT64)(uintptr_t) pDtlsSession);
    }
...
#else
INT32 dtlsSessionSendCallback(PVOID customData, const unsigned char* pBuf, size_t len)
#endif
{
    STATUS retStatus = STATUS_SUCCESS;
    PDtlsSession pDtlsSession = (PDtlsSession)(uintptr_t) customData;
    ...
    #else
    pDtlsSession->dtlsSessionCallbacks.outboundPacketFn(pDtlsSession->dtlsSessionCallbacks.outBoundPacketFnCustomData, (PBYTE) pBuf, (UINT32) len);
    #endif
...
CleanUp:
    ...
    #else
    return STATUS_FAILED(retStatus) ? -(INT32) retStatus : (INT32) len;
    #endif
}
...
#else
INT32 dtlsSessionReceiveCallback(PVOID customData, unsigned char* pBuf, size_t len)
#endif
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    ...
    if (pBuffer->off < pBuffer->len) {
        ...
        #else
        CHK_STATUS(ioBufferRead(pBuffer, pBuf, (UINT32) len, &readBytes));
        #endif
    }
...
#else
VOID dtlsSessionSetTimerCallback(PVOID customData, uint32_t intermediateDelayInMs, uint32_t finalDelayInMs)
#endif
{
    ENTERS();
    PDtlsSessionTimer pTimer = (PDtlsSessionTimer)(uintptr_t) customData;

    pTimer->intermediateDelay = (UINT64) intermediateDelayInMs * HUNDREDS_OF_NANOS_IN_A_MILLISECOND;
    pTimer->finalDelay = (UINT64) finalDelayInMs * HUNDREDS_OF_NANOS_IN_A_MILLISECOND;
...
STATUS dtlsSessionStart(PDtlsSession pDtlsSession, BOOL isServer)
{
    ...
    CHK_STATUS(timerQueueAddTimer(pDtlsSession->timerQueueHandle, DTLS_SESSION_TIMER_START_DELAY, DTLS_TRANSMISSION_INTERVAL,
                                  dtlsTransmissionTimerCallback, (UINT64)(uintptr_t) pDtlsSession, &pDtlsSession->timerId));
