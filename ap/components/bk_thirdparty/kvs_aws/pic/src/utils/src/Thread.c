...
PUBLIC_API STATUS defaultGetThreadName(TID thread, PCHAR name, UINT32 len)
{
    ...
    retValue = pthread_getname_np((pthread_t)(uintptr_t) thread, name, len);
    ...
    /* Beken has no prctl(); stub: return empty name */
    (void) thread;
    (void) len;
    name[0] = '\0';
    retValue = 0;
...
PUBLIC_API TID defaultGetThreadId()
{
    return (TID)(uintptr_t) pthread_self();
}
...
PUBLIC_API STATUS defaultCreateThreadWithParams(PTID pThreadId, PThreadParams pThreadParams, startRoutine start, PVOID args)
{
    ...
    *pThreadId = (TID)(uintptr_t) threadId;
...
{
    STATUS retStatus = STATUS_SUCCESS;
    INT32 joinResult = pthread_join((pthread_t)(uintptr_t) threadId, retVal);
...
{
    STATUS retStatus = STATUS_SUCCESS;
    INT32 cancelResult = pthread_kill((pthread_t)(uintptr_t) threadId, 0);
...
{
    STATUS retStatus = STATUS_SUCCESS;
    INT32 cancelResult = pthread_cancel((pthread_t)(uintptr_t) threadId);
...
{
    STATUS retStatus = STATUS_SUCCESS;
    INT32 detachResult = pthread_detach((pthread_t)(uintptr_t) threadId);
