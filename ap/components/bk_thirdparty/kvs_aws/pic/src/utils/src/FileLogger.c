                        ...
                        logPrintFunc* pFilePrintFn)
{
    ...
    gFileLogger = (PFileLogger) os_malloc(SIZEOF(FileLogger) + maxStringBufferLen * SIZEOF(CHAR));
    ...
    /* Copy including nul to avoid -Wstringop-truncation (strncpy with length=strlen leaves no nul). */
    STRNCPY(gFileLogger->mainLogger.logFile, FILE_LOGGER_LOG_FILE_NAME, STRLEN(FILE_LOGGER_LOG_FILE_NAME) + 1);
                                          ...
                                          BOOL enableAllLevels, UINT32 level, logPrintFunc* pFilePrintFn)
{
    ...
    CHAR fileIndexBuffer[KVS_COMMON_FILE_INDEX_BUFFER_SIZE];
    #else
    PCHAR fileIndexBuffer = NULL;
    #endif
    UINT64 charWritten = 0, indexFileSize = KVS_COMMON_FILE_INDEX_BUFFER_SIZE;

    #if 1
    fileIndexBuffer = (PCHAR) MEMALLOC(KVS_COMMON_FILE_INDEX_BUFFER_SIZE * SIZEOF(CHAR));
    CHK(fileIndexBuffer != NULL, STATUS_NOT_ENOUGH_MEMORY);
    #endif

    if (enableAllLevels) {
        // allocate the struct and string buffer together
        //gFileLogger = (PFileLogger) MEMALLOC(SIZEOF(FileLogger) + maxStringBufferLen * 2 * SIZEOF(CHAR));
        gFileLogger = (PFileLogger) os_malloc(SIZEOF(FileLogger) + maxStringBufferLen * 2 * SIZEOF(CHAR));
    } else {
        // allocate the struct and string buffer together
        //gFileLogger = (PFileLogger) MEMALLOC(SIZEOF(FileLogger) + maxStringBufferLen * SIZEOF(CHAR));
        gFileLogger = (PFileLogger) os_malloc(SIZEOF(FileLogger) + maxStringBufferLen * SIZEOF(CHAR));
    ...
    CHK(gFileLogger != NULL, STATUS_NOT_ENOUGH_MEMORY);
    ...
    if (gFileLogger->enableAllLevels) {
        ...
        gFileLogger->mainLogger.stringBuffer = (PCHAR) (gFileLogger + 1);
        gFileLogger->mainLogger.stringBufferLen = maxStringBufferLen;
        /* Copy including nul to avoid -Wstringop-truncation. */
        STRNCPY(gFileLogger->mainLogger.logFile, FILE_LOGGER_LOG_FILE_NAME, STRLEN(FILE_LOGGER_LOG_FILE_NAME) + 1);
        gFileLogger->mainLogger.currentOffset = 0;
        gFileLogger->mainLogger.currentFileIndex = 0;
    ...
    if (level < LOG_LEVEL_VERBOSE || level > LOG_LEVEL_PROFILE) {
        ...
        /* Copy including nul to avoid -Wstringop-truncation. */
        STRNCPY(gFileLogger->levelLogger.logFile, FILE_LOGGER_FILTER_LOG_FILE_NAME, STRLEN(FILE_LOGGER_FILTER_LOG_FILE_NAME) + 1);
        ...
        #else
        MEMSET(fileIndexBuffer, '\0', KVS_COMMON_FILE_INDEX_BUFFER_SIZE * SIZEOF(CHAR));
        #endif
...
CleanUp:
    ...
    #if 1
    if (fileIndexBuffer != NULL) {
        MEMFREE(fileIndexBuffer);
    }
    #endif
