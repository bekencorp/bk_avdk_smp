...
STATUS getDirectorySize(PCHAR dirPath, PUINT64 pSize)
{
    ...
    CHK_STATUS(traverseDirectory(dirPath, (UINT64)(uintptr_t) &size, TRUE, getFileDirSize));
...
STATUS traverseDirectory(PCHAR dirPath, UINT64 userData, BOOL iterate, DirectoryEntryCallbackFunc entryFn)
{
    ...
    #if 1
    tempFileName = (PCHAR) MEMCALLOC(1, MAX_PATH_LEN * SIZEOF(CHAR));
    CHK(tempFileName != NULL, STATUS_NOT_ENOUGH_MEMORY);
    #endif
...
STATUS getFileDirSize(UINT64 userData, DIR_ENTRY_TYPES entryType, PCHAR path, PCHAR name)
{
    UNUSED_PARAM(name);
    UINT64 size = 0;
    PUINT64 pSize = (PUINT64)(uintptr_t) userData;
    STATUS retStatus = STATUS_SUCCESS;
