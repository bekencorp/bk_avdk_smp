                          ...
                          PUINT32 pIndex)
{
    ...
    /* Use GETREALTIME so invokeTime matches defaultConditionVariableWait (GETREALTIME + timeout). GETTIME may be wall (e.g. Beken RTC). */
    #if 1
    pTimerEntry->invokeTime = GETREALTIME() + start;
    #else
...
STATUS timerQueueUpdateTimerPeriod(TIMER_QUEUE_HANDLE handle, UINT64 customData, UINT32 timerId, UINT64 period)
{
    ...
    #if 1
    pTimerQueue->pTimers[timerId].invokeTime = GETREALTIME() + period;
    #else
        ...
        if (!ATOMIC_LOAD_BOOL(&pTimerQueue->shutdown)) {
            ...
            for (i = 0, index = 0; index < pTimerQueue->activeTimerCount && i < pTimerQueue->maxTimerCount; i++) {
                if (pTimerQueue->pTimers[i].timerCallbackFn != NULL) {
                    ...
                    #else
                        /* invokeTime/curTime use GETREALTIME for CVAR_WAIT; callbacks compare with SDK fields set via GETTIME() (e.g. ICE
                         * gathering end, RTCP wall clock). Pass wall "now" here when GETTIME/GETREALTIME diverge (e.g. BEKEN_PLATFORM). */
                        retStatus = pTimerQueue->pTimers[i].timerCallbackFn(i, GETTIME(), pTimerQueue->pTimers[i].customData);
                    #endif
