...
MUTEX defaultCreateMutex(BOOL reentrant)
{
    ...
    #else
    PWinLock pLock = (PWinLock) os_malloc(1 * SIZEOF(WinLock));
    #endif
    if (NULL == pLock) {
        return (MUTEX) NULL;
    }
...
CVAR defaultConditionVariableCreate()
{
    ...
    #else
    CVAR pVar = (CVAR) os_malloc(1 * SIZEOF(CONDITION_VARIABLE));
    #endif
    if (NULL == pVar) {
        return (CVAR) NULL;
    }
...
/* Embedded pthread (FreeRTOS POSIX) struct layout differs; init at runtime like Time.c */
static pthread_mutex_t globalKvsReentrantMutex;
static pthread_mutex_t globalKvsNonReentrantMutex;
static volatile UINT8 globalKvsMutexesInited = 0;

static void ensureGlobalKvsMutexesInited(void)
{
    if (globalKvsMutexesInited) {
        return;
    }
    pthread_mutexattr_t attr;
    pthread_mutexattr_init(&attr);
    pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_RECURSIVE);
    pthread_mutex_init(&globalKvsReentrantMutex, &attr);
    pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_NORMAL);
    pthread_mutex_init(&globalKvsNonReentrantMutex, &attr);
    pthread_mutexattr_destroy(&attr);
    globalKvsMutexesInited = 1;
}
#else
...
MUTEX defaultCreateMutex(BOOL reentrant)
{
    ...
    #else
    pMutex = (pthread_mutex_t*) os_malloc(1 * SIZEOF(pthread_mutex_t));
    #endif
    if (NULL == pMutex) {
        return (MUTEX)(uintptr_t)(reentrant ? &globalKvsReentrantMutex : &globalKvsNonReentrantMutex);
    }
...
BOOL defaultTryLockMutex(MUTEX mutex)
{
    return (0 == pthread_mutex_trylock((pthread_mutex_t*)(uintptr_t)mutex));
}

VOID defaultFreeMutex(MUTEX mutex)
{
    pthread_mutex_t* pMutex = (pthread_mutex_t*)(uintptr_t)mutex;
...
CVAR defaultConditionVariableCreate()
{
    ...
    #else
    CVAR pVar = (CVAR) os_malloc(1 * SIZEOF(pthread_cond_t));
    #endif
    if (NULL == pVar) {
...
STATUS defaultConditionVariableWait(CVAR cvar, MUTEX mutex, UINT64 timeout)
{
    ...
    if (time < timeout || INFINITE_TIME_VALUE == timeout) {
        ...
        retVal = pthread_cond_timedwait(cvar, pMutex, &timeSpec);
        CHK(0 == (retVal), STATUS_WAIT_FAILED);
    }
