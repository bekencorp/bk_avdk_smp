...
STATUS readFile(PCHAR filePath, BOOL binMode, PBYTE pBuffer, PUINT64 pSize)
{
...
#if defined(BEKEN_PLATFORM)
    {
        ...
        CHK(fd >= 0, STATUS_OPEN_FILE_FAILED);

        sz = lseek(fd, 0, SEEK_END);
        CHK(sz >= 0, STATUS_READ_FILE_FAILED);
        fileLen = (UINT64) sz;
        ...
        CHK(lseek(fd, 0, SEEK_SET) >= 0, STATUS_READ_FILE_FAILED);

        n = read(fd, pBuffer, (size_t) fileLen);
        close(fd);
        fd = -1;
        CHK(n >= 0 && (UINT64) n == fileLen, STATUS_READ_FILE_FAILED);
    }
#else
    DLOGV("Opening file: %s", filePath);
...
STATUS readFileSegment(PCHAR filePath, BOOL binMode, PBYTE pBuffer, UINT64 offset, UINT64 readSize)
{
...
#if defined(BEKEN_PLATFORM)
    {
        ...
        CHK(lseek(fd, (off_t) offset, SEEK_SET) >= 0, STATUS_READ_FILE_FAILED);

        n = read(fd, pBuffer, (size_t) readSize);
        close(fd);
        fd = -1;
        CHK(n >= 0 && (UINT64) n == readSize, STATUS_READ_FILE_FAILED);
    }
#else
    DLOGV("Opening file: %s", filePath);
    fp = FOPEN(filePath, binMode ? "rb" : "r");
...
STATUS writeFileWithLogging(PCHAR filePath, BOOL binMode, BOOL append, PBYTE pBuffer, UINT64 size)
{
...
#if defined(BEKEN_PLATFORM)
    {
        ...
        fd = bk_vfs_open(filePath, flags);
        CHK(fd >= 0, STATUS_OPEN_FILE_FAILED);

        n = write(fd, pBuffer, (size_t) size);
        close(fd);
        fd = -1;
        CHK(n >= 0 && (UINT64) n == size, STATUS_WRITE_TO_FILE_FAILED);
    }
#else
    DLOGV("Opening file: %s", filePath);
...
STATUS updateFile(PCHAR filePath, BOOL binMode, PBYTE pBuffer, UINT64 offset, UINT64 size)
{
...
#if defined(BEKEN_PLATFORM)
    {
        ...
        CHK(lseek(fd, (off_t) offset, SEEK_SET) >= 0, STATUS_INVALID_OPERATION);
...
STATUS setFileLength(PCHAR filePath, UINT64 length)
{
...
#elif defined(BEKEN_PLATFORM)
    {
        ...
        if (fd >= 0) {
            retVal = ftruncate(fd, (off_t) length);
            errCode = errno;
            close(fd);
        } else {
            retVal = -1;
...
STATUS createFile(PCHAR filePath, UINT64 size)
{
...
#if defined(BEKEN_PLATFORM)
    {
        ...
        fd = bk_vfs_open(filePath, O_RDWR | O_CREAT | O_TRUNC);
        CHK(fd >= 0, STATUS_OPEN_FILE_FAILED);

        if (size != 0) {
            CHK(lseek(fd, (off_t) (size - 1), SEEK_SET) >= 0, STATUS_INVALID_OPERATION);
            CHK(write(fd, &zero, 1) == 1, STATUS_INVALID_OPERATION);
        }
        close(fd);
        fd = -1;
    }
