...
getTime globalGetRealTime = defaultGetRealTime;

#if !(defined _WIN32 || defined _WIN64 || defined __CYGWIN__)

getTmTime globalGetThreadSafeTmTime = defaultGetThreadSafeTmTime;
#if defined(BEKEN_PLATFORM)
/* Embedded pthread may not provide PTHREAD_MUTEX_INITIALIZER; init at runtime */
static pthread_mutex_t globalGmTimeMutex;
static volatile UINT8 globalGmTimeMutexInited = 0;
#else
...
struct tm* defaultGetThreadSafeTmTime(const time_t* timer)
{
    struct tm* retVal;
#if defined(BEKEN_PLATFORM)
    if (!globalGmTimeMutexInited) {
        pthread_mutex_init(&globalGmTimeMutex, NULL);
        globalGmTimeMutexInited = 1;
    }
#endif
...
STATUS generateTimestampStrInMilliseconds(PCHAR formatStr, PCHAR pDestBuffer, UINT32 destBufferLen, PUINT32 pFormattedStrLen)
{
    ...
    /* Same wall clock as CONFIG_NTP_SYNC_RTC (bk_rtc_settimeofday / cli_misc). */
    struct timeval tv;
    CHK(bk_rtc_gettimeofday(&tv, NULL) == BK_OK, STATUS_INVALID_OPERATION);
    timeinfo = GMTIME_THREAD_SAFE(&(tv.tv_sec));
    millisecondVal = (UINT32)(tv.tv_usec / 1000);
#else
...
#elif defined(BEKEN_PLATFORM)
    struct timeval nowTime;
    if (bk_rtc_gettimeofday(&nowTime, NULL) != BK_OK) {
        return 0;
    }
    return (UINT64) nowTime.tv_sec * HUNDREDS_OF_NANOS_IN_A_SECOND + (UINT64) nowTime.tv_usec * HUNDREDS_OF_NANOS_IN_A_MICROSECOND;
#else
