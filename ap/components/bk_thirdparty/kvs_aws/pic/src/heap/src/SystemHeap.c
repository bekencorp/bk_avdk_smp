...
DEFINE_HEAP_ALLOC(sysHeapAlloc)
{
    ...
    *pHandle = (ALLOCATION_HANDLE)(uintptr_t)((PBYTE) pHeader + SYS_ALLOCATION_HEADER_SIZE);
...
DEFINE_HEAP_SET_ALLOC_SIZE(sysHeapSetAllocSize)
{
    ...
    // Setting the return value
    *pHandle = (ALLOCATION_HANDLE)(uintptr_t)((PBYTE) pNewHeader + SYS_ALLOCATION_HEADER_SIZE);
