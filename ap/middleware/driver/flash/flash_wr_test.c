...
/* flash_bypass_test otp <E/R/W/L> <otp_idx> <offset> <len> [data-hex] */
...
static void flash_bypass_otp_test(int argc, char **argv)
{
	flash_bypass_otp_ctrl_t param;
	char op;
	uint32_t length;
	bk_err_t ret;
	...
	op = argv[2][0];
	param.otp_idx = (flash_bypass_otp_idx_t)os_strtoul(argv[3], NULL, 16);
	param.addr_offset = os_strtoul(argv[4], NULL, 16);
	...
	case 'R': {
		if (length == 0 || length > FLASH_BYPASS_OTP_BLOCK_LENGTH) {
			BK_DUMP_OUT("invalid read len 0x%x\r\n", length);
			return;
		}
		param.read_len = length;
		param.read_buf = (uint8_t *)os_malloc(length);
		if (param.read_buf == NULL) {
			BK_DUMP_OUT("malloc fail\r\n");
			return;
		}
	...
	case 'W': {
		if (argc < 7 || length == 0 || length > FLASH_BYPASS_OTP_BLOCK_LENGTH) {
			BK_DUMP_OUT("invalid write len/data\r\n");
			return;
		}
		param.write_len = length;
		param.write_buf = (uint8_t *)os_malloc(length);
		if (param.write_buf == NULL) {
			BK_DUMP_OUT("malloc fail\r\n");
			return;
		}
		for (uint32_t i = 0; i < length; i++) {
...
{
	if (argc >= 2 && os_strcmp(argv[1], "otp") == 0) {
		flash_bypass_otp_test(argc, argv);
		return;
	}
	BK_DUMP_OUT("flash_bypass_test otp <E/R/W/L> <otp_idx> <offset> <len> [data]\r\n");
}
...
#if CONFIG_FLASH_BYPASS_OTP_OPERATION
	{"flash_bypass_test", "flash_bypass_test otp <E/R/W/L> ...", flash_bypass_command_test},
#endif
};
