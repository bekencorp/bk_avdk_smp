...
#if CONFIG_CLI && CONFIG_FLASH_TEST
...
{
	// Simple implementation for flash_client mode
	// In client mode, status register operations may not be fully supported
	// Return a default value or implement via IPC if needed
	return 0;
}
...
bk_err_t bk_flash_write_status_reg(uint16_t status_reg_data)
{
	// Simple implementation for flash_client mode
	// In client mode, status register operations may not be fully supported
	// Return OK to avoid errors, or implement via IPC if needed
