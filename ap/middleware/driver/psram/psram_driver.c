...
psram_write_through_area_t bk_psram_alloc_write_through_channel_with_psram_id(uint8_t psram_id)
{
	...
	if (psram_id > 1) {
		PSRAM_LOGE("invalid psram_id=%u, must be 0 or 1\n", psram_id);
	...
	if (psram_id == 0) {
		start = 0;
		end = PSRAM_WRITE_THROUGH_AREA_PER_PSRAM;
	} else {
		start = PSRAM_WRITE_THROUGH_AREA_PER_PSRAM;
		end = PSRAM_WRITE_THROUGH_AREA_COUNT;
	}

	for (channel = (uint8_t)start; channel < end; channel++) {
		if ((s_psram_channelmap & (0x1 << channel)) == 0) {
			s_psram_channelmap |= (0x1 << channel);
			break;
		}
