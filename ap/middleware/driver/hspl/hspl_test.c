...
static void cli_hspl_help(void)
{
	...
	CLI_LOGD("hspl res_lock {flash|clock|os|user0..user2} {timeout_us}\r\n");
	CLI_LOGD("hspl res_unlock {flash|clock|os|user0..user2}\r\n");
...
static void cli_hspl_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "lock") == 0) {
		...
		if (argc >= 4) {
			...
			/* compatible: hspl raw_sta {ch} -> default hspl_id=0 */
			ch = (uint8_t)os_strtoul(argv[2], NULL, 10);
		}
		...
		ret = rtos_create_thread(&test->thread, 5, "hspl_stress_cpu2",
