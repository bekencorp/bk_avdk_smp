...
#if CONFIG_CLI && CONFIG_GPIO_API_TEST
	int bk_gpio_api_register_cli_test_feature(void);
	bk_gpio_api_register_cli_test_feature();
#endif
...
static void __BK_IRQ gpio_isr(void)
{
...
bk_err_t bk_gpio_ana_register_wakeup_source(gpio_id_t gpio_id, gpio_int_type_t int_type)
{
	...
	GPIO_LOGV("regist wakeup source gpio id: %d type: %d\r\n", gpio_id, int_type);
