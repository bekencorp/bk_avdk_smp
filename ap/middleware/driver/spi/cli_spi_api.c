...
static void cli_spi_api_cmd_handler(void **argtable)
{
    ...
    else if (init_config->count > 0)
    {
        ...
        #if (CONFIG_SPI_DMA)
            config.dma_mode = 0;   /**< SPI whether use dma; DMA channels are owned by the driver */
            if (os_strtoul(bits->sval[0], NULL, 10) == 8) {
...
static void cli_argspi_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    struct arg_str *connect = arg_str0("c", "connect", "<read/write/read_async/write_async>", "Connect with spi");
    ...
    else if (driver->count > 0)
    {
        ...
        if (strcmp(driver->sval[0], "init") == 0) {
            ...
            if (recv_data == NULL || send_data == NULL) {
                CLI_LOGE("duplex buffer malloc failed\r\n");
                if (recv_data) {
                    os_free(recv_data);
                }
                if (send_data) {
...
int bk_spi_api_register_cli_test_feature(void)
{
