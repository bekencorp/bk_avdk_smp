...
enum
{
	...
	//   cpu2 clients
	CPU2_CLIENT_ID_START_test = IPC_CLIENT_ID_START(2),
	reserved_c1,
	TEST1_CLIENT_ID,
	TEST2_CLIENT_ID,
	TEST3_CLIENT_ID,
...
int mb_ipc_test_init(void)
{
	...
	#if IPC_GET_ID_CPU(TEST_SVR3_ID) !=  MAILBOX_CPU2
	#error server cpu configuration error!
	#endif
	rtos_create_thread(NULL, 3, test_svr_name[0], mb_ipc_test_svr1, 2048, (void *)IPC_GET_ID_PORT(TEST_SVR3_ID));
