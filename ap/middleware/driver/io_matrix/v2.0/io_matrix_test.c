...
{
	CLI_LOGD("iomx_driver init\n");
	CLI_LOGD("iomx_driver deinit\n");
	CLI_LOGD("iomx set [id]\n");
	CLI_LOGD("iomx get [id]\n");
}
...
static void cli_iomx_driver_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "init") == 0) {
		...
		CLI_LOGD("iomx driver init\n");
	} else if (os_strcmp(argv[1], "deinit") == 0) {
		BK_LOG_ON_ERR(bk_iomx_driver_deinit());
		CLI_LOGD("iomx driver deinit\n");
	} else {
		cli_iomx_help();
		return;
	}
