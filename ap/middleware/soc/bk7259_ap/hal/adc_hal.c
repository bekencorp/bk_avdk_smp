...
{
	/* ana_reg0x2 */
	uint32_t reg_val = sys_ll_get_ana_reg2_value();
	reg_val |= (BIT(25) | BIT(30) | BIT(31));
	reg_val |= (0x3 << 28);
	sys_ll_set_ana_reg2_value(reg_val);

	/* ana_reg0x4 */
	sys_ll_set_ana_reg4_value(0x9FC9A7F0);

	/* ana_reg0x5 */
	reg_val = sys_ll_get_ana_reg5_value();
	reg_val |= (BIT(12) | BIT(14));
	reg_val &= ~BIT(1);
	sys_ll_set_ana_reg42_en_vout(reg_val);
