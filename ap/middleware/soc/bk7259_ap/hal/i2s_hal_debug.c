...
static void i2s_dump_smb_devid(void)
{
	SOC_LOGI("SMB_DEVID: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0x0 << 2)));
}

static void i2s_dump_smb_verid(void)
{
	SOC_LOGI("SMB_VERID: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0x1 << 2)));
}

static void i2s_dump_smb_clkrst(void)
{
	i2s_smb_clkrst_t *r = (i2s_smb_clkrst_t *)(SOC_I2S_REG_BASE + (0x2 << 2));

	SOC_LOGI("SMB_CLKRST: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0x2 << 2)));
	SOC_LOGI("	soft_reset: %8x\r\n", r->soft_reset);
	SOC_LOGI("	clkg_bypass: %8x\r\n", r->clkg_bypass);
	SOC_LOGI("	reserved_bit_2_31: %8x\r\n", r->reserved_bit_2_31);
}

static void i2s_dump_smb_state(void)
{
	SOC_LOGI("SMB_STATE: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0x3 << 2)));
}

static void i2s_dump_pcm_cfg(void)
{
	i2s_pcm_cfg_t *r = (i2s_pcm_cfg_t *)(SOC_I2S_REG_BASE + (0x4 << 2));
...
static void i2s_dump_pcm_cn(void)
{
	i2s_pcm_cn_t *r = (i2s_pcm_cn_t *)(SOC_I2S_REG_BASE + (0x5 << 2));
...
static void i2s_dump_pcm_stat(void)
{
	i2s_pcm_stat_t *r = (i2s_pcm_stat_t *)(SOC_I2S_REG_BASE + (0x6 << 2));

	SOC_LOGI("PCM_STAT: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0x6 << 2)));
	SOC_LOGI("	rxint: %8x\r\n", r->rxint);
	SOC_LOGI("	txint: %8x\r\n", r->txint);
	SOC_LOGI("	rxovf: %8x\r\n", r->rxovf);
	SOC_LOGI("	txudf: %8x\r\n", r->txudf);
...
static void i2s_dump_pcm_dat(void)
{
	SOC_LOGI("PCM_DAT: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0x7 << 2)));
}

static void i2s_dump_pcm_cn_lt2(void)
{
	i2s_pcm_cn_lt2_t *r = (i2s_pcm_cn_lt2_t *)(SOC_I2S_REG_BASE + (0x8 << 2));

	SOC_LOGI("PCM_CN_LT2: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0x8 << 2)));
...
static void i2s_dump_pcm_stat_lt2(void)
{
	i2s_pcm_stat_lt2_t *r = (i2s_pcm_stat_lt2_t *)(SOC_I2S_REG_BASE + (0x9 << 2));

	SOC_LOGI("PCM_STAT_LT2: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0x9 << 2)));
	SOC_LOGI("	rx2int: %8x\r\n", r->rx2int);
	SOC_LOGI("	tx2int: %8x\r\n", r->tx2int);
	SOC_LOGI("	rx2ovf: %8x\r\n", r->rx2ovf);
	SOC_LOGI("	tx2udf: %8x\r\n", r->tx2udf);
...
static void i2s_dump_pcm_dat2(void)
{
	SOC_LOGI("PCM_DAT2: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0xa << 2)));
}

static void i2s_dump_pcm_dat3(void)
{
	SOC_LOGI("PCM_DAT3: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0xb << 2)));
}

static void i2s_dump_pcm_dat4(void)
{
	SOC_LOGI("PCM_DAT4: %8x\r\n", REG_READ(SOC_I2S_REG_BASE + (0xc << 2)));
}

static i2s_reg_fn_map_t s_fn[] =
{
	{0x0, 0x0, i2s_dump_smb_devid},
	{0x1, 0x1, i2s_dump_smb_verid},
	{0x2, 0x2, i2s_dump_smb_clkrst},
	{0x3, 0x3, i2s_dump_smb_state},
	{0x4, 0x4, i2s_dump_pcm_cfg},
	{0x5, 0x5, i2s_dump_pcm_cn},
	{0x6, 0x6, i2s_dump_pcm_stat},
	{0x7, 0x7, i2s_dump_pcm_dat},
	{0x8, 0x8, i2s_dump_pcm_cn_lt2},
	{0x9, 0x9, i2s_dump_pcm_stat_lt2},
	{0xa, 0xa, i2s_dump_pcm_dat2},
	{0xb, 0xb, i2s_dump_pcm_dat3},
	{0xc, 0xc, i2s_dump_pcm_dat4},
	{-1, -1, 0}
};
