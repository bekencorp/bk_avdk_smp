...
static inline uint32_t can_ll_get_rdata_value(uint32_t i) {
	can_data_t *r = (can_data_t*)(SOC_CAN0_REG_BASE + ((0x2 + i) << 2));
	return r->v;
}
...
static inline void can_ll_set_tdata(uint32_t i, uint32_t v) {
	can_data_t *r = (can_data_t*)(SOC_CAN0_REG_BASE + ((0x16 + i) << 2));
	r->data = v;
}
...
static inline uint32_t can_ll_get_tts_l_value(void) {
	can_tts_t *r = (can_tts_t*)(SOC_CAN0_REG_BASE + (0x26 << 2));
	return r->v;
}

static inline uint32_t can_ll_get_tts_h_value(void) {
	can_tts_t *r = (can_tts_t*)(SOC_CAN0_REG_BASE + (0x27 << 2));
	return r->v;
}
...
static inline void can_ll_set_fd_enable(uint32_t v) {
	can_fd_t *r = (can_fd_t*)(SOC_CAN0_REG_BASE + 0x800 );
	r->can_fd_enable = v;
}
