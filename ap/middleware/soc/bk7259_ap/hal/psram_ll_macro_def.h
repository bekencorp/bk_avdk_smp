...
/* Cover stop: low bits = stop offset (32B unit), BIT31 = enable for this cover range. */
#define PSRAM_REG_COVER_STOP_ENA(id) (PSRAM_LL_REG_BASE + ((0x11 + ((id) << 1)) << 2))
/* per-controller base (PSRAM0 or PSRAM1), id 0~3 */
#define PSRAM_REG_COVER_START_WITH_BASE(base, id)    ((base) + ((0x10 + ((id) << 1)) << 2))
#define PSRAM_REG_COVER_STOP_ENA_WITH_BASE(base, id) ((base) + ((0x11 + ((id) << 1)) << 2))
...
/* value = stop offset in 32B units (low bits) | BIT(31) to enable the cover stop range. */
static inline void psram_ll_set_cover_stop_enable(uint32_t id, uint32_t value)
{
	REG_WRITE(PSRAM_REG_COVER_STOP_ENA(id), value);
}

static inline void psram_ll_set_cover_start_with_base(uint32_t reg_base, uint32_t id, uint32_t value)
{
	(*(volatile uint32_t *)(PSRAM_REG_COVER_START_WITH_BASE(reg_base, id))) = value;
}

/* value = stop offset in 32B units (low bits) | BIT(31) to enable the cover stop range. */
static inline void psram_ll_set_cover_stop_enable_with_base(uint32_t reg_base, uint32_t id, uint32_t value)
{
	(*(volatile uint32_t *)(PSRAM_REG_COVER_STOP_ENA_WITH_BASE(reg_base, id))) = value;
}

/* Read back cover register(s) by reg_base, for dual-PSRAM. */
static inline uint32_t psram_ll_get_cover_start_with_base(uint32_t reg_base, uint32_t id)
{
	return (*(volatile uint32_t *)(PSRAM_REG_COVER_START_WITH_BASE(reg_base, id)));
}

static inline uint32_t psram_ll_get_cover_stop_enable_with_base(uint32_t reg_base, uint32_t id)
{
	return (*(volatile uint32_t *)(PSRAM_REG_COVER_STOP_ENA_WITH_BASE(reg_base, id)));
}
