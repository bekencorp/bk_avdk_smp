...
ARM_MPU_Region_t mpu_regions[] = {
    ...
    /* Interleaved PSRAM NS window; AP PSRAM region macros are already NS aliases. */
    { ARM_MPU_RBAR(0x80000000UL + NS_MEM_OFFSET, ARM_MPU_SH_NON, 0, 1, 0),
      ARM_MPU_RLAR((uint32_t)CONFIG_AP_PSRAM_HEAP_ADDR - 0x20, 1) },
#if (CONFIG_AP_PSRAM_CODE_SECTION_ADDR && CONFIG_AP_PSRAM_CODE_SECTION_SIZE && CONFIG_AP_PSRAM_CODE_SECTION_ADDR > 0x64000000UL + SOC_S_NS_ADDR_DIFF)
    { ARM_MPU_RBAR((uint32_t)CONFIG_AP_PSRAM_HEAP_ADDR, ARM_MPU_SH_NON, 0, 1, 0),
      ARM_MPU_RLAR((uint32_t)CONFIG_AP_PSRAM_CODE_SECTION_ADDR - 0x20, 5) },
    { ARM_MPU_RBAR((uint32_t)CONFIG_AP_PSRAM_CODE_SECTION_ADDR, ARM_MPU_SH_NON, 0, 1, 0),
      ARM_MPU_RLAR((uint32_t)CONFIG_AP_PSRAM_CODE_SECTION_ADDR +
                   (uint32_t)CONFIG_AP_PSRAM_CODE_SECTION_SIZE - 0x20, 3) },
#else
    { ARM_MPU_RBAR(0x81000000UL + NS_MEM_OFFSET, ARM_MPU_SH_NON, 0, 1, 0),
      ARM_MPU_RLAR(0x81FFFFE0UL + NS_MEM_OFFSET, 1) },
#endif
#else
    /* PSRAM0/1 NS aliases (0x70.. / 0x74..); region macros are already NS. */
    { ARM_MPU_RBAR(0x60000000UL + NS_MEM_OFFSET, ARM_MPU_SH_NON, 0, 1, 0),
      ARM_MPU_RLAR(CONFIG_AP_PSRAM_HEAP_ADDR - 0x20, 1) },
#if (CONFIG_AP_PSRAM_CODE_SECTION_ADDR && CONFIG_AP_PSRAM_CODE_SECTION_SIZE && CONFIG_AP_PSRAM_CODE_SECTION_ADDR > 0x64000000UL + SOC_S_NS_ADDR_DIFF)
    { ARM_MPU_RBAR((uint32_t)CONFIG_AP_PSRAM_HEAP_ADDR, ARM_MPU_SH_NON, 0, 1, 0),
      ARM_MPU_RLAR((uint32_t)CONFIG_AP_PSRAM_CODE_SECTION_ADDR - 0x20, 5) },
    { ARM_MPU_RBAR((uint32_t)CONFIG_AP_PSRAM_CODE_SECTION_ADDR, ARM_MPU_SH_NON, 0, 1, 0),
      ARM_MPU_RLAR(0x67FFFFE0UL + NS_MEM_OFFSET - 0x20, 3) },
#else
