...
typedef volatile struct {
	struct {
		...
		uint32_t reserved[0x3FF6]; /**< Reserved to make each group 0x10000 bytes (64KB).
		                            *  Calculation steps:
		                            *  1. group[0] base = TIMER4_BASE = 0x48120000
		                            *  2. group[1] base = TIMER5_BASE = 0x48130000
		                            *  3. Each group size = TIMER5_BASE - TIMER4_BASE = 0x10000 (64KB)
		                            *  4. Used space = 0x28 bytes (10 registers × 4 bytes)
		                            *  5. Reserved = (0x10000 - 0x28) / 4 = 0x3FF6 */ 
