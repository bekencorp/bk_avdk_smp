...
Z_EXC_DECLARE(b_arm_user_string_nlen);

static const struct z_exc_handle exceptions[] = {Z_EXC_HANDLE(b_arm_user_string_nlen)};
#endif
 ...
 *
 * See b_arm_fault_dump() for example.
 *
...
static inline bool b_arm_is_synchronous_svc(struct arch_esf *esf)
{
...
#if defined(CONFIG_ARMV6_M_ARMV8_M_BASELINE)
static inline bool b_arm_is_pc_valid(uintptr_t pc)
{
...
static uint32_t hard_fault(struct arch_esf *esf, bool *recoverable)
{
	...
	if (b_arm_is_pc_valid((uintptr_t)esf->basic.pc) && b_arm_is_synchronous_svc(esf)) {
	...
	*recoverable = false;
	...
	if ((SCB->HFSR & SCB_HFSR_VECTTBL_Msk) != 0) {
		...
		if (b_arm_is_synchronous_svc(esf)) {
			PR_EXC("ARCH_EXCEPT with reason %x\n", esf->basic.r0);
			reason = esf->basic.r0;
 ...
 *
 */
void b_arm_fault(uint32_t msp, uint32_t psp, uint32_t exc_return, _callee_saved_t *callee_regs)
{
	uint32_t reason = K_ERR_CPU_EXCEPTION;
	...
	b_arm_set_fault_sp(esf, exc_return);
	...
	b_arm_fatal_error(reason, &esf_copy);
}
