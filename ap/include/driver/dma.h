...
/**
 * @brief     Set DMA next linked list address
 *
 * @param id DMA channel
 * @param ll_addr Next linked list descriptor address (must be 16-byte aligned)
 *
 * @return
 *    - BK_OK: succeed
 *    - others: other errors.
...
/**
 * @brief     Get DMA next linked list address
 *
 * @param id DMA channel
 *
 * @return Next linked list descriptor address
 */
 ...
 *
 * This API allocates and initializes a descriptor table for DMA linked list transfer.
 * Each descriptor is 16 bytes and already 16-byte aligned.
 *
 * @param link_cnt Number of descriptors in the linked list
 *
 * @return Descriptor table pointer on success, NULL on failure
 */
void *bk_dma_link_init(uint32_t link_cnt);

/**
 * @brief     Deinitialize descriptor table
 *
 * This API frees the memory allocated by bk_dma_link_init.
 *
...
/**
 * @brief     Set descriptor for linked list transfer
 *
 * This API configures a single descriptor in the descriptor table.
 *
 * @param desc_table Descriptor table pointer
 * @param index Descriptor index in the table
 * @param config Link configuration (source address, destination address, length, interrupt enables)
 *
 * @return
 *    - BK_OK: succeed
 *    - BK_ERR_NULL_PARAM: invalid parameters
 *    - BK_ERR_PARAM: address not 16-byte aligned
 *    - others: other errors.
 */
bk_err_t bk_dma_link_set_desc(void *desc_table, uint32_t index, const dma_link_config_t *config);

/**
 * @brief     Set multiple descriptors for linked list transfer
 *
 * This API configures multiple descriptors in the descriptor table.
 *
 * @param desc_table Descriptor table pointer
 ...
 *    - others: other errors.
 */
bk_err_t bk_dma_link_set_descs(void *desc_table, const dma_link_config_t *configs, uint32_t link_cnt);

/**
 * @brief     Execute linked list transfer (synchronous)
 *
 * This API configures the DMA channel and starts the linked list transfer.
 * The DMA channel ID must be allocated by the application layer using bk_dma_alloc().
 ...
 *    - BK_ERR_DMA_ID: invalid DMA channel ID
 *    - others: other errors.
 */
