...
/**
 * @brief     Allocate a write-through channel (area) from the default pool.
 *
 * Returns the next available area (0~7) for configuring PSRAM write-through.
 * Prefer bk_psram_alloc_write_through_channel_with_psram_id() when the buffer
 * is known to lie in PSRAM0 or PSRAM1 so that the area matches the chip.
 *
 * @return    Allocated area (PSRAM_WRITE_THROUGH_AREA_0 ~ _AREA_7), or
 *            PSRAM_WRITE_THROUGH_AREA_COUNT if no channel is available.
 */
psram_write_through_area_t bk_psram_alloc_write_through_channel(void);

/**
 * @brief     alloc write_through_area for specific PSRAM
 *
 * Use when buffer is in a known PSRAM: psram_id 0 -> returns area 0~3, psram_id 1 -> returns area 4~7.
 *
 * @param psram_id 0=PSRAM0, 1=PSRAM1
 * @return allocated area (0~7), or PSRAM_WRITE_THROUGH_AREA_COUNT on failure
 */
