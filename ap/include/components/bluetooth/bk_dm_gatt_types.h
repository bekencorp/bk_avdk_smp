 ...
 * @{
 */
#define BK_GATT_MAX_READ_MULTI_HANDLES     10           /* Max attributes to read in one request */
...
typedef enum
{
    ...
    BK_GATT_UUID_CHAR_DECLARE =                  0x2803,          ///  Characteristic Declaration

    /// Descriptors
    BK_GATT_UUID_CHAR_EXT_PROP =                 0x2900,          ///  Characteristic Extended Properties
    BK_GATT_UUID_CHAR_USER_DESCRIPTION =         0x2901,          ///  Characteristic User Description
    ...
    BK_GATT_UUID_CHAR_PRESENT_FORMAT =           0x2904,          ///  Characteristic Presentation Format
    BK_GATT_UUID_CHAR_AGG_FORMAT =               0x2905,          ///  Characteristic Aggregate Format
    ...
    BK_GATT_UUID_GAP_APPEARANCE =                0x2A01,
    BK_GATT_UUID_GAP_PREF_CONN_PARAM =           0x2A04,
    BK_GATT_UUID_GAP_CENTRAL_ADDR_RESOL =        0x2AA6,
    BK_GATT_UUID_GAP_RESOLV_RPVIATE_ADDR_ONLY =  0x2AC9,
    BK_GATT_UUID_GAP_ENCRYPT_DATA_KEY_MATERIAL = 0x2B88,
    BK_GATT_UUID_GAP_BLE_GATT_SECURITY_LVL =     0x2BF5,

    /// Attribute Profile Attribute UUID
    BK_GATT_UUID_GATT_SRV_CHGD =                 0x2A05,
    BK_GATT_UUID_GATT_CLI_SUPPORT_FEAT =         0x2B29,
    BK_GATT_UUID_GATT_DATABASE_HASH =            0x2B2A,
    BK_GATT_UUID_GATT_SRV_SUPPORT_FEAT =         0x2B3A,
...
typedef enum
{
    ...
    // beken custom gatt err code
    BK_GATT_UNKNOWN_ERROR          =   0x0100, ///unknown err
} bk_gatt_status_t;
 ...
 * @attention When MITM is enable, key generation method can not be Just Work. This require both local and peer iocap configuration suitable.
 *            See bluetooth core specification Version 5.4 | Vol 3, Part H, 2.3.5.1 Selecting key generation method.
 *
 */
typedef enum
{
    BK_GATT_PERM_READ =                  (1 << 0),
    BK_GATT_PERM_READ_ENCRYPTED =        (1 << 1),
    BK_GATT_PERM_READ_ENC_MITM =         (1 << 2),
    BK_GATT_PERM_WRITE =                 (1 << 4),
    BK_GATT_PERM_WRITE_ENCRYPTED =       (1 << 5),
    BK_GATT_PERM_WRITE_ENC_MITM =        (1 << 6),
    BK_GATT_PERM_WRITE_SIGNED =          (1 << 7),//  not used now
    BK_GATT_PERM_WRITE_SIGNED_MITM =     (1 << 8),//  not used now
    BK_GATT_PERM_READ_AUTHORIZATION =    (1 << 9),//  not used now
    BK_GATT_PERM_WRITE_AUTHORIZATION =   (1 << 10),// not used now
} bk_gatt_perm_enum_t;
...
typedef enum
{
    BK_GATT_SERVICE_FROM_REMOTE_DEVICE         = 0,
    BK_GATT_SERVICE_FROM_NVS_FLASH             = 1,
    BK_GATT_SERVICE_FROM_UNKNOWN               = 2,
} bk_service_source_t;
...
typedef struct
{
    ...
    bk_bt_uuid_t attr_content;                   /// attr content if attr_type is service or char
    ...
    bk_gatt_perm_enum_t perm;                     /*!< Attribute permission*/
} bk_attr_desc_t;
...
typedef enum
{
    BK_GATT_WRITE_TYPE_NO_RSP  =   1,                      /*!< Gatt write attribute need no response */
    BK_GATT_WRITE_TYPE_RSP,                                /*!< Gatt write attribute need remote response */
} bk_gatt_write_type_t;
...
typedef struct
{
    ...
    uint16_t handles[BK_GATT_MAX_READ_MULTI_HANDLES];      /*!< The handles list */
} bk_gattc_multi_t;                                        /*!< The gattc multiple read element */
///@}
