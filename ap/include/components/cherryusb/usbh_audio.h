...
/* v1.6 host audio layout. struct usbh_audio is allocated by the (separately
 * compiled) CherryUSB v1.6 library, so this MUST stay byte-for-byte identical to
 * CherryUSB/class/audio/usbh_audio.h, otherwise BK glue code (which reads
 * these fields through the pointer) would read at the wrong offsets. The former
 * v0.7 (pipe-based) usbh_audio layout was removed with the legacy stack. */
