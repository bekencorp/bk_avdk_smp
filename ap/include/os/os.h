...
/** @brief Creates and starts a new thread on hsram and not specify core
  *
  * @param thread     : Pointer to variable that will receive the thread handle (can be null)
  * @param priority   : A priority number.
  * @param name       : a text name for the thread (can be null)
  ...
  * @return    kGeneralErr     : if an error occurred
  */
bk_err_t rtos_create_hsram_thread(beken_thread_t *thread, uint8_t priority, const char *name, beken_thread_function_t function, uint32_t stack_size, beken_thread_arg_t arg);
...
/** @brief    get the total heap size in hsram 
  * 
  * @return   the amount of total heap space in hsram
  */
size_t rtos_get_hsram_total_heap_size(void);

/** @brief    get the free heap size in hsram 
  * 
  * @return   the amount of left heap space in hsram
  */
size_t rtos_get_hsram_free_heap_size(void);

/** @brief    get the minimum heap size in hsram 
  * 
  * @return   the minimum value remaining  in hsram during 
  *           the system heap space usage
  */
size_t rtos_get_hsram_minimum_free_heap_size(void);
...
/** @brief    setup rtos systick
  * 
  * @param    cpu_clock_hz : the cpu clock frequency
  */
void rtos_setup_systick(uint32_t cpu_clock_hz);
...
#define CPU0_CORE_ID              (0)
#define CPU1_CORE_ID              (1)
#define CPU2_CORE_ID              (2)
#define CPU3_CORE_ID              (3)

static inline uint32_t rtos_get_core_id(void)
{
    uint32_t bk_multicore_get_cpu_id(void);
    return bk_multicore_get_cpu_id();
}

#ifndef portGET_CORE_ID
#define portGET_CORE_ID()       (rtos_get_core_id() & 0x00000001)
#endif

#if (CONFIG_SOC_SMP)

/*CPU2 is the main core and the core id is 0
  CPU3 is the sub core and the core id is 1
  So the offset is 2*/
#define SMP_CORE0_ID              (0)   // CPU2 is smp core 0, (CPU2_CORE_ID - CONFIG_CPU_ID_OFFSET)
#define SMP_CORE1_ID              (1)   // CPU3 is smp core 1, (CPU3_CORE_ID - CONFIG_CPU_ID_OFFSET)
...
/** @brief Creates and starts a new thread specify to run on cpu0 which use hsram memeory
  *
  * @param thread     : Pointer to variable that will receive the thread handle (can be null)
  * @param priority   : A priority number.
  * @param name       : a text name for the thread (can be null)
  ...
  * @return    kGeneralErr     : if an error occurred
  */
bk_err_t rtos_core0_create_hsram_thread( beken_thread_t* thread, uint8_t priority, const char* name,
                        beken_thread_function_t function, uint32_t stack_size, beken_thread_arg_t arg );
...
/** @brief Creates and starts a new thread specify to run on cpu1 which use hsram memeory
  *
  * @param thread     : Pointer to variable that will receive the thread handle (can be null)
  * @param priority   : A priority number.
  * @param name       : a text name for the thread (can be null)
  ...
  * @return    kGeneralErr     : if an error occurred
  */
bk_err_t rtos_core1_create_hsram_thread( beken_thread_t* thread, uint8_t priority, const char* name,
                        beken_thread_function_t function, uint32_t stack_size, beken_thread_arg_t arg );
